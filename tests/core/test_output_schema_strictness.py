"""F24 regression guard: every provider ``output_schema`` constant must be strict.

codex CLI enforces ``--output-schema`` through OpenAI Structured Outputs, which rejects a schema
with a 400 (``'additionalProperties' is required to be supplied and to be false``) unless **every**
object node — top-level and nested — carries ``additionalProperties: false``. F19 introduced
``_FINDINGS_SCHEMA`` without it and every codex evaluator turn crashed deterministically (F24). The
F19 smoke test validated its own simplified example schema, not the real constant, so it missed the
regress. This test walks the literal battle constants that reach a provider as ``output_schema`` and
fails if any object node is non-strict — so a future schema addition cannot repeat F24.
"""
