...
def test_worc_packaged_data_discoverable() -> None:
    ...
    assert _worc_root().joinpath("skills", "worc-deco-task", "SKILL.md").is_file()
    assert _worc_root().joinpath("config", "README.md").is_file()
    assert _worc_root().joinpath("config", "reference.md").is_file()
    ...
    assert _worc_root().joinpath("skills", "worc-night-run", "SKILL.md").is_file()
