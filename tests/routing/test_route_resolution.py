...
# Every stage routes the same way now — routing is by the node's ``provider``, not the stage.
_STAGES = [Stage.REFINEMENT, Stage.PLANNING, Stage.IMPLEMENTATION, Stage.REVIEW, Stage.FIXING]
...
@pytest.mark.parametrize("stage", _STAGES, ids=lambda s: s.value)
def test_no_provider_defaults_to_global_primary(router: AgentRouter, stage: Stage) -> None:
...
def test_node_provider_equal_to_primary_has_no_fallback(router: AgentRouter) -> None:
    ...
    route = router.resolve_route(Stage.IMPLEMENTATION, ProviderId.CLAUDE)
    assert (route.primary, route.fallback) == (ProviderId.CLAUDE, None)
    assert route.source is RouteSource.FLOW_NODE
...
def test_node_provider_not_allowlisted_is_rejected(
    ...
    with pytest.raises(ConfigError):
        router.resolve_route(Stage.PLANNING, ProviderId.CODEX)
...
def test_route_carries_provider_command_and_args(
    ...
    route = router.resolve_route(Stage.REVIEW, ProviderId.CLAUDE)
