"""Safety drill 05.2 — low-trust memory never auto-promotes or outranks trusted (AC-SF2/SF5).
...
repo-backed record, and never silently overwrite active memory on a contradiction (NFR2).
"""
...
def test_low_trust_contradiction_never_overwrites_active_memory(tmp_path: Path) -> None:
    ...
    assert len(active) == 1
    assert active[0]["statement"] == "use `make build`"  # untouched — no silent overwrite (NFR2)
