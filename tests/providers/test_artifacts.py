...
def test_attempt_dir_layout(tmp_path: Path) -> None:
    paths = create_attempt_dir(tmp_path, "task-001", Stage.PLANNING, 1, "codex")
    expected = tmp_path / "logs" / "task-001" / "stages" / "planning" / "1-codex"
...
def test_subtask_dir_is_zero_padded(tmp_path: Path) -> None:
    paths = create_attempt_dir(tmp_path, "t", Stage.IMPLEMENTATION, 2, "codex", subtask=2)
    expected = tmp_path / "logs" / "t" / "stages" / "implementation" / "sub-02" / "2-codex"
    assert Path(paths.attempt_dir) == expected
    assert expected.is_dir()


def test_never_overwrites_existing_attempt_dir(tmp_path: Path) -> None:
    create_attempt_dir(tmp_path, "task-001", Stage.PLANNING, 1, "codex")
    with pytest.raises(FileExistsError):
        create_attempt_dir(tmp_path, "task-001", Stage.PLANNING, 1, "codex")


def test_distinct_attempts_get_distinct_dirs(tmp_path: Path) -> None:
    a = create_attempt_dir(tmp_path, "task-001", Stage.PLANNING, 1, "codex")
    b = create_attempt_dir(tmp_path, "task-001", Stage.PLANNING, 2, "codex")
...
def test_write_result_artifact_serializes_enums_and_error(tmp_path: Path) -> None:
    paths = create_attempt_dir(tmp_path, "task-001", Stage.REVIEW, 1, "codex")
