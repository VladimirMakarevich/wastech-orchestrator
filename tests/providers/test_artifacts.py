...
def test_attempt_dir_layout(tmp_path: Path) -> None:
    paths = create_attempt_dir(
        tmp_path, "task-001", Stage.PLANNING, 1, "codex", stage_run_id=42
    )
    expected = (
        tmp_path / "logs" / "task-001" / "stages" / "planning" / "run-000042" / "1-codex"
    )
...
def test_never_overwrites_existing_attempt_dir(tmp_path: Path) -> None:
    create_attempt_dir(
        tmp_path, "task-001", Stage.PLANNING, 1, "codex", stage_run_id=1
    )
...
def test_distinct_attempts_get_distinct_dirs(tmp_path: Path) -> None:
    a = create_attempt_dir(
        tmp_path, "task-001", Stage.PLANNING, 1, "codex", stage_run_id=1
    )
    b = create_attempt_dir(
        tmp_path, "task-001", Stage.PLANNING, 2, "codex", stage_run_id=1
    )
...
def test_distinct_stage_runs_get_distinct_dirs_when_attempt_resets(tmp_path: Path) -> None:
    a = create_attempt_dir(
        tmp_path, "task-001", Stage.FIXING, 1, "claude", stage_run_id=10
    )
    b = create_attempt_dir(
        tmp_path, "task-001", Stage.FIXING, 1, "claude", stage_run_id=11
    )
    assert a.attempt_dir != b.attempt_dir
...
def test_write_result_artifact_serializes_enums_and_error(tmp_path: Path) -> None:
    paths = create_attempt_dir(
        tmp_path, "task-001", Stage.REVIEW, 1, "codex", stage_run_id=1
    )
