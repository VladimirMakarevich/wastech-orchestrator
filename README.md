...
- **Runs unattended.** A watch loop periodically syncs your base branch, so a teammate can hand off a task just by committing it and pushing (that route needs the task lifecycle tree tracked — one install answer). For a night where something *will* go wrong, `install` also delivers a copy-ready `worc-night-run` skill: an agent that takes the queue one task at a time, clears the blockers a watch loop would stop at, gives up on a task after four starts rather than on the whole night, and leaves a per-task write-up to read in the morning.
