...
- the git footprint: the changed-code paths drop everything under the runtime homes (`.worc/`, `.worc-io/`) and under the configured `paths.tasks_dir`, which also drives the audit commit; the scoped `git add` pathspec guards that lifecycle dir with `:(exclude)` **only** while it is tracked, because beside a gitignored one at the repository root git refuses a root-level code path outright; the `.git/info/exclude` append is anchored, written once, idempotent, and never touches the tracked `.gitignore`; the audit commit is orchestrator-only, stages the deletion side of a lifecycle move out of **every** tracked lifecycle folder (its pathspec is derived from that one enumeration, not restated — a folder `install` scaffolds but the pathspec misses commits the task file twice and leaves a dangling `D` on base), and refuses either a foreign task's lifecycle file or a task file rewritten since it was frozen;
