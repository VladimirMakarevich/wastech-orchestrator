...
- the settled-leftover decision and what a reject of an already-terminal id is allowed to do: a finished task's own file is recognised by content (the recorded path is rewritten by the very move that makes the file resurface, and a checkout may rewrite its newlines), a reworded file reusing that id is not, and a reject of a terminal id appends no second ledger record and sends no notification — quarantining only the file proven to be a different task;
- `init` idempotency (a second run is all-skipped; never overwrites `config.yaml`; `--dry-run` is a no-op);
- the git footprint: the changed-code paths drop everything under the runtime homes (`.worc/`, `.worc-io/`) and under the configured `paths.tasks_dir`, which also drives the audit commit; the scoped `git add` pathspec guards that lifecycle dir with `:(exclude)` **only** while it is not ignored, because beside a gitignored one at the repository root git refuses a root-level code path outright; the ignore question is asked with `check-ignore --no-index` against a probe path _under_ the dir, so neither a `tasks/*`-plus-negation scheme nor one already-tracked task file can flip the answer for the tree; the `.git/info/exclude` append is anchored, written once, idempotent, never touches the tracked `.gitignore`, and never covers the lifecycle tree (whose rule `install` seeds in the tracked file, where an operator can delete it); the audit commit is orchestrator-only, is a no-op that touches no branch when the tree is ignored and holds nothing tracked (`install`'s default), stages the deletion side of a lifecycle move out of **every** tracked lifecycle folder (its pathspec is derived from that one enumeration, not restated — a folder `install` scaffolds but the pathspec misses commits the task file twice and leaves a dangling `D` on base), force-adds only paths git already tracks and never an ignored untracked one, and refuses either a foreign task's lifecycle file or a task file rewritten since it was frozen;
- `install`'s two lifecycle answers: the wizard question / `--track-tasks` (default **no**, in both the interactive and the `--non-interactive` path) and `--tasks-dir`, which one value drives — the scaffolded folders, the generated `paths.tasks_dir`, and the anchored ignore line — with an unsafe value refused by the config validator rather than scaffolded; and the override above both, that a lifecycle dir git already tracks something in is never ignored (a repository using `tasks/` for its own files, or an operator who deleted the seeded line — ignoring it would hide every _new_ file there from `git status`);
...
- the audit record closes: a `node_runs` row's lifetime belongs to the node layer, so a node that stops **before** its provider call — a containment breach, an integrity failure, an unresolvable tool — leaves the row `aborted` with a `finished_at` and its reason in `abort_reason`, on any path including one that never reaches the task driver's terminal (the merge flow is the one such path today); a node that recorded its own verdict is never reclosed over. Quarantined exchange evidence is written only when there is something in it, and a bundle carries `created_at` / `node_id` / `attempt` plus a `quarantine_refs` pointer on the task row that outlives the contamination flag an operator continue clears;
