...
- **Flows are fully operator-authorable — no node is privileged.** The engine dispatches a runner by `node.kind` (`agent`, `evaluator`, `checks`, `tool`, `hitl`, `publish`) and **never** branches on a specific node **id**. No packaged id is mandatory — an operator may rename, drop, reorder, or duplicate nodes freely. Behavior attaches to a node **kind** or a declared fact, never to an id.
