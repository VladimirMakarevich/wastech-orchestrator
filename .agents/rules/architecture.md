...
- **Detection brackets and the diff gate**: the Git-control fingerprint around an attempt covers the local control surfaces (HEAD, the task ref, the index, repo-local config, hooks and `core.hooksPath`, operation markers), the configuration the publishing processes read (the clone's agent-CLI config and the user git config as _this_ orchestrator's git resolves it, `GIT_CONFIG_GLOBAL` included, by digest), and — of the **remote** side — only where a push would go (`remote get-url --push`, by digest, so a rewritten `insteadOf`/`pushurl` cannot send the branch and our credentials elsewhere unseen). What `origin` holds for the task branch and whether that branch has an open PR are deliberately **not** watched: a branch or PR without one of our rows is not evidence of foreign ownership — a recreated `state.db` has no rows while the branch and its PR are still ours — and publishing already recovers from a diverged remote by merging it in locally first, so parking here was that same recovery spent earlier and less usefully. The base branch was never watched either: it moves legitimately. The bracket keys on whether that attempt actually gets a **shell** at that provider on that host — not on the permission profile, not on a declared grant — because command execution is what makes a working-tree write or a `.git` mutation reachable. A workspace-write agent attempt that drifts parks the task; every other node class (a shell-bearing read-only attempt, an `evaluator`, a `tool`) warns and continues. The dangerous-diff gate runs at a writing node after its edit, at that node's `hitl` round-trip, and once more immediately before the publishing commit — so **every** path that can edit the tree is asked about, including the classes that only warn (a `tool`, an `evaluator`, a read-only attempt with a shell) and a flow with no writing node at all. It measures from the last commit the orchestrator itself made for the task (the task's start point until it makes one) — never from `HEAD`, which a commit made inside the task would empty — and an approval already given for the same change in the same task is not requested again. At the publishing gate a denial is a `manual_action_required` stop with nothing committed: the agent is gone, so there is no reconsideration to hand it to.
...
- **The orchestrator never delegates publication to the agent:** no node is given a mandate to commit, push, or open a PR, and no product mechanism expects the agent to. Mechanical impossibility is guaranteed only where a sandbox exists, and only for the local half (`.git` and `.worc` are immutable); the remote half is **de jure only** — stated in the prompt every node receives, neither prevented nor detected, on our `origin` or anywhere else.
- **Publishing recovers, it never adopts.** What `origin` holds for the task branch decides the push: matching us (nothing sent), behind us (an ordinary push), diverged from the commit we recorded pushing (a lease-guarded force-push over our own stale push, and only that), or diverged from something we never pushed (merge it in **locally**, re-run the checks over the combination, and only then push and declare the adopted commits in the PR; a conflict parks with the tree restored). That order is the invariant below — publishing happens only when checks succeed — so the merge, which is local and undoable, comes before the push, which is not. The adopted content moves the dangerous-diff gate's reference point with it: it rides in on a merge commit the orchestrator made, so the human is not asked again about deletions the merge brought along. An open pull request on the task head is appended to and adopted whether or not one of our rows already names it — absence of a row is not evidence someone else owns it, and reusing the PR records it, so the question is settled from then on; the cost, stated plainly, is that a PR a person opened on that same head is retitled and appended to. An existing remote branch is still never taken as proof of our own earlier publication: what decides the push is the four cases above, not the branch's mere existence.
