...
| Final rendered prompt the agent received | `rendered-prompt.md` is the literal text = role file (or the node's `resume_role_file` on a continuation turn) + Core-built instruction + context footer (`providers.base.build_effective_prompt`). The editable part is the **role file**; the scaffolding is code |
...
- `logs/<task-id>/`:
  ...
  - `stages/<node>/rendered-prompt.md` — the **final prompt** each node received. The primary input for prompt-quality findings. A looping node may have two role files: the short one it gets on a re-entry is deliberate, not a truncated prompt — check the flow's `resume_role_file` before filing a finding about missing rules.
  ...
  - `prompt-audit/` — present only when `config.prompt_audit: true`: `timeline.jsonl` (chronological per-node prompt + provider/model/attempt/fallback/status, one whole JSON record per line — parse this half) and `<NNNNNN>-<node>.md`, the same record rendered for reading: a fenced `json` metadata header followed by the prompt as the document's body. `model` / `reasoning` are the effective values the settled attempt ran with, with the flow node's own override beside them as `model_configured` / `reasoning_configured` and a per-attempt pair inside `agents` — so read the plain names for "what ran" and the `_configured` pair for "what was pinned", and cite the attempt row when a stage fell back. A node with a `resume_role_file` has two prompts, so the document carries a second body section and each `agents` row names which text that attempt received (`resumed` / `prompt_variant`) — read that per attempt, because a stage that opened as a continuation and then lost its session ran on the full text. If absent, note it as a **data gap** and recommend enabling it.
  - `publish/terminal-cleanup.json` — branch/cleanup outcome.
- `runs/exchange-seals/<task-id>/seal-<NNNNNN>/` — the sealed, checksum-verified snapshot of the agent-readable **exchange** as it was at each terminal, with a `manifest.json` (the in-repo `.worc-io/<task-id>/` is removed at terminal, so this is where a terminal task's curated agent-facing plan/diff/findings live). Read the newest `seal-*` to see exactly what the agent last saw. `runs/exchange-quarantine/<task-id>/<NNNNNN>/` holds a tree quarantined because mutation detection flagged an agent-side change (`evidence.json` = expected vs observed) — a signal the agent tried to write the read-only surface. `runs/control-bundles/<task-id>/` and `runs/instruction-bundles/<task-id>/` are the frozen control plane and frozen agent inputs the run was bound to.
  - **A successful run may have none of these.** `logging.clean_runs_on_success` defaults to `true`, so a `done` task evicts its own `runs/` subtree at the terminal transition (quarantine excepted). If they are missing on a successful task that is expected, not a data gap — note it and fall back to `logs/<task-id>/`. To analyze future runs from the seals, have the operator set `logging.clean_runs_on_success: false` before the run.
