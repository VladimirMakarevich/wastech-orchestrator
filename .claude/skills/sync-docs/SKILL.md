---
name: sync-docs
description: After a behavior/CLI/config/architecture change in wastech-orchestrator, bring the docs (including the docs/functional map and the docs/likec4 C4 model), CHANGELOG, and follow-ups tracker in sync with the code. Use after implementing a change (the Stop docs-sync gate reminds you), before a commit, or whenever the docs no longer match the code.
---
...
2. **Update the docs that match the change** (only the ones it actually affects):
   ...
   - **Architecture / invariants / contracts** → [docs/rules/architecture.md](../../../docs/rules/architecture.md), the [Functional Map](../../../docs/functional/index.md), and [docs/worc_architecture.md](../../../docs/worc_architecture.md).
   - **Persisted state / schema versions** (config `schema_version`, `state.db` `user_version`, registry `version`) → the schema-versions note at the top of [CHANGELOG.md](../../../CHANGELOG.md) and [docs/operations.md](../../../docs/operations.md#upgrading-the-orchestrator).
