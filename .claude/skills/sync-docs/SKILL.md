...
2. **Update the docs that match the change** (only the ones it actually affects):
   ...
   - **Packaged operator guide docs** (the prose docs under `src/wastech_orchestrator/packaged/guide/` — `guide/README.md`, `guide/best-practices.md`, `guide/decision-guide.md`, `guide/flows/*.md`, `guide/config/*.md`, `guide/tasks/*.md`) → these mirror `docs/` for the operator who reads them after `install`; when a behavior/CLI/config/flow/vocabulary change touches something they describe, update them in lockstep with the `docs/` files above. **Note:** `packaged/guide/` is `.prettierignore`d — match the existing one-paragraph-per-line style by hand and do **not** run Prettier on it.
