...
2. **Update the docs that match the change** (only the ones it actually affects):
   ...
   - **Config schema** (`config.yaml` fields, defaults, validation) → [docs/configuration.md](../../../docs/configuration.md), the packaged `src/wastech_orchestrator/packaged/config.example.yaml` (the single example, validated by the round-trip test).
