...
### F41 · finalize-схема structured-output супервизора (`DELTA_OUTPUT_SCHEMA` + `_FOLLOW_UPS_SCHEMA`) не OpenAI-strict-совместима → supervisor-finalize крашит codex (тот же класс, что F24) · **MEDIUM** · уверенность HIGH · зона **orchestrator** · статус **RESOLVED (в коде; empirical codex re-run — за оператором)**
