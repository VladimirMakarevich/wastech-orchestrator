...
The task may be decomposed into several sequential subtasks (run on one branch, landing in one PR). When a step belongs to a subtask, judge it as progress on *that subtask*, not the whole task, and say which subtask it is. The whole task closes only after the *last* subtask's final step — so an early subtask finishing its own implementation/review is not the run closing; do not describe it as such.
