...
- Keep `strict_isolation: true` unless you are choosing the advanced mode on purpose. `false` **is** that mode — the parent environment goes to the agent whole and read-isolation is forced off — so read [`security.md`](security.md) before setting it. It unlocks no provider full-access mode either way: those are forbidden outright.
