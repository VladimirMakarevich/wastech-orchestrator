...
- **Install vs dataclass defaults differ** for a few fields: `security.strict_isolation` (install `false` — the advanced mode; an omitted key still means the fail-closed `true`), `security.allow_git_evidence` (install `true`), `logging.level` (install `warning`), `skills.dynamic` (`false`), provider `model`/`reasoning`, and `supervisor` (provider and every phase model pinned to the primary; `observe.reasoning` delivered as `low`, the other phases as `high`). The table shows both.
