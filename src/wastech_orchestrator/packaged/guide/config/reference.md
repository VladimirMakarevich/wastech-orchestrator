...
- **Unknown keys fail closed.** A key the schema does not know — at the top level or inside any block — is a hard load error (a handful of dead keys are silently tolerated so an older file still loads). Do not invent fields.
- **Three list fields _replace_, never extend, their defaults:** `security.allowed_environment`, `security.denied_read_paths`, `security.denied_commands`. If you write one, write the whole list you need. Note what that means for `allowed_environment` in particular: `config.example.yaml` ships the cross-platform **union** of names, while the generated `config.yaml` omits the key entirely and lets the loader resolve the **host OS** default at load — so writing the list is what freezes it, and copying a written list between machines can drop a name the new host needs.
...
- **[runtime.md](runtime.md)** — `telegram`, `logging`, `memory`, `tools`, `prompt_audit`: notifications, artifact retention, memory, and the remaining top-level keys.
...
- **It is behind us** — an ordinary push. (The mere existence of the remote branch is never taken as proof of an earlier push of ours, which matters most when `branch_name` came from the task file or you chose the branch yourself.)
...
- **Replace-not-extend.** `allowed_environment`, `denied_read_paths`, `denied_commands` replace their defaults wholesale. `extra_environment` is **not** one of them — it has no default to replace, so writing it only ever adds the names you list. For `allowed_environment` that also means the generated `config.yaml` (key absent — the host OS default is resolved at load) and the shipped `config.example.yaml` (cross-platform union) differ by design; `PATH` is mandatory in either, and on Windows `SystemRoot` is a launch-critical FAIL in preflight and at `run`/`watch`/`rerun` start. Advanced mode widens agent-side children only: orchestrator-owned `git`/`gh` keeps this list.
- **Full access is not available at all.** Codex `--sandbox danger-full-access` and Claude `--permission-mode bypassPermissions` are rejected wherever they can appear — a provider's `extra_args`, a flow node's `extra_args`, and the argv build — at every value of `strict_isolation`. There is no `agents.providers.<provider>.sandbox` key: a config that carries one fails to load as an unknown key, and `upgrade-config` does not remove it, so delete the line by hand.
- **The generated `config.yaml` is deliberately small.** `install` writes what it resolved (`repo`, the selected providers, `git`, the `auto_mode` answer), the one block that deviates from the defaults, and three affordances (`paths.tasks_dir`, an empty `checks.command_sets`, `telegram.enabled`). Every other block is absent and runs at the default in these tables, so nothing here is a knob you have to confirm in your own file. Add a block only to change something.
- **Install vs dataclass defaults differ** for exactly two things, and both are written out for that reason: the `security` posture — `strict_isolation` (install `false`, the advanced mode; an omitted key still means the fail-closed `true`) and `allow_git_evidence` (install `true`) — and each provider's `model` / `reasoning`, whose fallbacks are empty. Everything else `install` used to spell out has since been aligned with its default, `logging.level` (`warning`) and `supervisor.observe.reasoning` (`low`) included.
- **Comments are stripped on upgrade, and omitted keys come back.** `worc upgrade-config` preserves your values but re-emits the file from parsed YAML: inline comments are lost, and its add-missing-only merge writes out every key the packaged template has — including the ones a small config deliberately left at their defaults. It confirms before writing and keeps a timestamped backup (`-y` skips the prompt). Keep the _reason_ for an unusual value recoverable elsewhere.
