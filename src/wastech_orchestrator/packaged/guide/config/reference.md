...
Preflight owns every verdict that depends on **this host**, which is why one config file can be valid everywhere and still not run here. `allowed-environment: FAIL` is that shape: on a Windows host it reports an `allowed_environment` missing `SystemRoot` and names the exit code (`0xC0000409`) you would otherwise have to recognize from a CLI that printed nothing. The host-independent half of the same rule — `PATH` must be covered — is a load error instead, so that the same file gets the same answer on every machine. The `allowed-environment:` pattern report is host-specific too: it says what each pattern matched **here**, names anything dropped as secret-bearing, and shows a pattern that matched nothing. Never a FAIL on its own; the run log repeats it once per run.
