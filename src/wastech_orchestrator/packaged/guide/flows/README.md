...
- Network is off by default; declare `network_policy` for a flow-wide grant or `network_access: true` on one node. A Codex `workspace-write` node with network is rejected — split external fetches into a `read-only` node. That refusal belongs to the shipped default: under `security.strict_isolation: false` (advanced mode) the validator no longer rejects the combination, because the mode has already granted every node both the write and the network.
