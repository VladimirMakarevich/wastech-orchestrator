...
The supervisor is a constant read-only layer above the flow — it observes the executed nodes (all but the deterministic `tool` / `checks` ones and the terminal `publish`) and writes the final summary (the PR body). A flow can reshape **its wording** (never the machine contract) with an optional `supervisor:` block:
