...
| `{conflicts_path}` | agent, evaluator, `tool` (in its stdin `paths`) | the flow is not resolving a conflicted base merge — i.e. everywhere except a `worc merge-task` run whose base merge conflicted |
| `{report_dir}` | agent, evaluator, `tool` (in its stdin `paths`) | the flow's `output_policy` is `code_change` — it has no report directory, so a `tool` node's stdin carries `null` and a **role prompt that names it is a fatal flow-load error** rather than an empty render |
...
`{conflicts_path}` is the merge flow's **conflict inventory**: one document listing every conflicted path, what kind of conflict it is (both sides modified it, one side deleted it, both sides added a binary, a submodule pointer), which of the three sides exist, what the working tree holds for it right now, and what would count as a decision. It exists because conflict markers are only one shape of conflict — Git resolves a modify/delete or a binary conflict by dropping one side into the working tree with no marker at all, so a role prompt that says "remove every marker" describes a subset of the job. The orchestrator refuses to commit a conflicted path whose working tree is byte-identical to what the merge left there, so a merge role prompt should send the agent to this list rather than to a marker search; wrap it in `{?conflicts_path}…{/conflicts_path}`.

`{report_dir}` is the one variable that is not an artifact the orchestrator wrote for you to read: it is the directory this flow's writing nodes are **confined to**, repo-relative and already carrying the per-task segment (`docs/research/<task_id>`, or whatever base the flow's `report_dir` names — see [reference.md](reference.md#report_dir--moving-the-deliverables-home)). Name it instead of a literal path in the prompt of any node that writes the deliverable: a hardcoded directory keeps being written to after an operator moves `report_dir`, and the after-stage guard then hard-stops the task on the first write.
...
- A node id that collides with a reserved core-variable prefix (`task`, `plan`, `diff`, `checks`, `review`, `repo`, `memory`, `stage`, `conflicts`, or anything starting with `subtask`) — a fatal flow-load error, because `{plan_path}` etc. already mean the core variable.
