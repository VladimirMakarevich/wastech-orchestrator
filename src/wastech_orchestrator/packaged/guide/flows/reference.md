...
A node's `network_access` overrides this for that node alone (`true` grants even with no flow policy; `false` forces one node offline). **A Codex `workspace-write` node with network is rejected** — split external fetches into a `read-only` node. Both sentences describe the shipped default: under `security.strict_isolation: false` every node is online whatever the flow said, that rejection does not apply (the operator has already granted both halves), and `false` on a node no longer forces it offline — a shell reaches the network with or without a tool list.
...
The same holds if such a node changes **Git control state** — a hook, `.git/config`, the index: warning plus a ⚠️ `done (node changed git control state)` trace, naming the drifted aspect, and the run continues. **Treat that one as a stop-the-run signal, not a note.** The two events differ in what they cost you: a stray file is inert (nothing stages it, nothing downstream reads it), while a planted `.git/hooks/post-commit` is executed by the next git command in that clone — and the next one is the orchestrator's own commit or push. Do not let the run finish; kill it, discard the clone, and look at what was planted. **Every** node class gets this verdict, the writing one included (that changed on 2026-08-24; before it, a `workspace-write` agent node parked the task instead). A `tool` node, an `evaluator`, the supervisor's own read-only turn and a writing agent node are all watched by the same bracket, because what makes the write reachable is the shell, not the node kind. Two things follow that are worth having straight. The reason the verdict softened is that the fingerprint reads "the state moved" and never whose hand moved it: what it stopped in practice was an operator committing in their own repository mid-run, after the node had finished its work, which was then discarded. And a commit that lands inside a run — anyone's — is still asked about, because the dangerous-diff gate measures from the orchestrator's own last commit rather than from `HEAD`. So this warning is the signal about the *clone*; the gate is the guard on the *content*.
