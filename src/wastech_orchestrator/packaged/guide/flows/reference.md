...
- **Codex** needs nothing added: its `read-only` sandbox already permits commands, so `git log` works there today. What forbids mutation is not a verb list but the sandbox — the workspace is mounted `read` and, on the shipped default, the network is off, so `git commit` has nothing to write to and `git push` has nowhere to go. That is the stronger guarantee of the two, and no prompt, task or flow can argue with it. Under `security.strict_isolation: false` only the first half survives: the workspace is still mounted `read`, but the node is online and the CLI picks up your credentials by itself, so publication there is a mandate plus detection rather than an impossibility.
