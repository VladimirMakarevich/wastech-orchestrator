...
The same holds if such a node changes **Git control state** — a hook, `.git/config`, the index: warning plus a ⚠️ `done (node changed git control state)` trace, naming the drifted aspect, and the run continues. **Treat that one as a stop-the-run signal, not a note.** The two events differ in what they cost you: a stray file is inert (nothing stages it, nothing downstream reads it), while a planted `.git/hooks/post-commit` is executed by the next git command in that clone — and the next one is the orchestrator's own commit or push. Do not let the run finish; kill it, discard the clone, and look at what was planted. A `workspace-write` agent node doing the same still parks the task in `manual_action_required`. Everything else warns and continues, and "everything else" is wider than the read-only agent class: a `tool` node, an `evaluator`, and the supervisor's own read-only turn are watched by the same bracket and get the same verdict, because what makes the write reachable is the shell, not the node kind.
