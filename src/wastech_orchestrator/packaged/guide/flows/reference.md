...
An audit node that treats delivery history as prime evidence needs to be able to read it. `git_evidence: true` on an agent or evaluator node asks for that; the operator's `security.allow_git_evidence` decides whether the request is honored. Both halves are required — a flow can express the need but cannot grant itself the capability, and turning the switch on hands the verbs only to the nodes that asked, never to every read-only node in the run. That is true of the shipped default. Under `security.strict_isolation: false` (the advanced mode) the grant does nothing at all: every node already has an unscoped shell, so there is no capability left to hand out and no verb list to be confined by. With the switch off (the default) a declaring flow still loads and validates; it simply runs as it does today.
