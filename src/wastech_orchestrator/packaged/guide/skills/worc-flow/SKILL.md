...
- Do not raise `permission_ceiling` above what the flow needs, and do not grant `workspace-write` to a node that only reads. A read-only node that needs to read git history takes `git_evidence: true`, not `workspace-write` — on the shipped default; in advanced mode (`security.strict_isolation: false`) that key is inert, because every node already has an unscoped shell. A Codex `workspace-write` node with network access is rejected — split external fetches into a `read-only` node. That refusal belongs to the shipped default: under `security.strict_isolation: false` (advanced mode) the validator no longer rejects the combination, because the mode has already granted every node both the write and the network.
