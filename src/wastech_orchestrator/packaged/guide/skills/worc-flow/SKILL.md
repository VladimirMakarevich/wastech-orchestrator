...
2. Choose the three flow-level contracts up front (see `.worc/guide/flows/reference.md` for each variant):
   ...
   - `report_dir` — optional, and only for the two report policies: the base directory their deliverable lands in (`<report_dir>/<task_id>/`), replacing `docs/research` / `.worc/security-reports`. Repo-relative POSIX, no `..`, no backslash, no drive letter, and never inside `.worc/`, `.worc-io/`, the lifecycle tasks tree or `.git/`. A private report under a base outside `.worc/` must be gitignored, or the run ends at `manual_action_required` rather than committing it.
...
- Do not pick `output_policy` by what the deliverable "sounds like". A brand-new document that is not a report-plus-sources bundle (e.g. a blog post under `blog/`) is a `code_change`, **not** a `repository_document` — choosing `repository_document` confines every write to one directory (`docs/research/<task_id>/`, or the flow's `report_dir`) and the flow hard-stops at `manual_action_required` on the first real write.
- Do not write a report's directory into a role prompt as a literal path. Use `{report_dir}` — it resolves to the same directory the flow's write guard enforces, so moving `report_dir` moves the instruction with it.
...
- Do not raise `permission_ceiling` above what the flow needs, and do not grant `workspace-write` to a node that only reads. A read-only node that needs to read git history takes `git_evidence: true`, not `workspace-write` — under `security.strict_isolation: true`; in advanced mode (`strict_isolation: false`, what a fresh install writes) that key is inert, because every node already has an unscoped shell. A Codex `workspace-write` node with network access is rejected — split external fetches into a `read-only` node. That refusal also belongs to strict isolation: under `strict_isolation: false` the validator accepts the combination, because the mode has already granted every node both the write and the network.
