...
3. Reference artifacts only by **allowlisted `{name}` path variables** (e.g. `{task_path}`, `{plan_path}`, `{diff_path}`, `{repo_path}`, `{conflicts_path}` for a merge role, and node-chaining `{<node_id>_path}`) — never inline task bodies, diffs, env, or secrets. Wrap every _optional_ variable in a `{?name}…{/name}` block so a missing value never leaves a dangling fragment.
