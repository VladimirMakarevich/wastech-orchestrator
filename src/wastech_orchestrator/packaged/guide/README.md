...
- **[footprint.md](footprint.md)** — what the orchestrator leaves in the repository: every directory under `.worc/`, whether its presence is normal, what is safe to delete, and how retention works.
...
| `task_type` | no | string | Flow selector — which pipeline runs the task. Omit ⇒ `implementation` (the default coding pipeline). Built-ins: `implementation`, `deep_research`, `security_audit`, `content_chapter`, `content_translate`, `blog_article`, `blog_article_revise`; an operator may add others as `<repo>/.worc/flows/<task_type>.yaml`. (`merge` also ships but is never task-dispatched — the orchestrator selects it via `git.merge_flow` when a base-merge conflicts.) An unknown `task_type` (no matching flow) fails the task before any branch is created. The task only _names_ the flow — it never edits the graph. See the decision guide. |
...
| `nodes` | no | mapping | Per-node overrides keyed by flow node id. Four sub-keys: `enabled: false` (the disable toggle) plus `model` / `reasoning` / `provider`, which overlay that node's executor for this run only. See the decision guide. |

Provider, model, and reasoning are **not** top-level task fields — a node's defaults live on the flow node (the operator's flow + `config.yaml providers`). A task may *overlay* them per run inside its `nodes.<node-id>` block, and that overlay is **best-effort**: the gate checks only shape, and a value invalid for the resolved flow/config (a `provider` not in `agents.allowed`, a `reasoning` the provider does not support, or an overlay on a node that runs no agent) is warned and skipped at run time — the flow's declared value stands and the task is never aborted. `model` is passed through unchecked. A task still cannot change provider commands, credentials, sandbox settings, `extra_args`, or any security policy.
...
5. `nodes` overrides carry only `enabled`, `model`, `reasoning`, `provider`, keyed by flow **node id**. Any node in the task's resolved flow may be disabled (the gate checks shape only); an id absent from the flow ends the task `failed` at flow resolution. Which nodes are safe to disable is the operator's flow-authoring call — there is no `agents.allow_review_skip` gate. The three executor overrides are best-effort (warned + skipped when invalid), never fatal.
