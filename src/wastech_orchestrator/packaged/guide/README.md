...
| `commit_type` | no | one of `feat` `fix` `docs` `style` `refactor` `perf` `test` `build` `ci` `chore` `revert` | The Conventional-Commits **type** for every commit the task lands: the code commit on the task branch, each subtask commit of a decomposition, and the squash commit on the base branch. Omit ⇒ `feat`. The scope is always the task id, so a subject stays greppable back to its task. **Fail-closed** — an unknown type rejects the task, since the value reaches permanent history. This is the only thing a task contributes to its commit message; no node can write one. |
