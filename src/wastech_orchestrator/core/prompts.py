...
# The token shape is an all-lowercase word plus digits and ``-`` (so a node id like ``static-scan``
# / ``pass2`` resolves as ``{static-scan_path}``), kept lowercase-only so camelCase braces in
# code/JSON (``{someVar}``) still fail to match and pass through untouched. ``-`` is last in the
...
#: The name pattern matches :data:`_VAR_RE` so both accept the same token shape.
_BLOCK_RE = re.compile(r"\{\?([a-z0-9_-]+)\}(.*?)\{/\1\}", re.DOTALL)
...
def render_prompt(
    ...
    *,
    ...
    Only names in *allowed* are replaced — the default :data:`ALLOWED_PROMPT_VARS`, but a caller may
    pass a **wider effective set** (the flow-derived ``ALLOWED_PROMPT_VARS ∪ {<node_id>_path}``).
    The renderer stays the fixed security core regardless: it substitutes only names in the set it
    is given, and only the *path* values the caller placed in ``variables`` — never inlined content.
    A ``None`` value renders empty. Any other ``{...}`` (an unknown name, or literal braces in
    code/JSON) passes through unchanged — no ``KeyError``, no breakage on stray braces.

    A conditional block ``{?name}body{/name}`` is resolved first: ``body`` is kept (and then
    flat-substituted in the normal pass) only when ``name`` is in *allowed* and its value is present
    and not the empty string; otherwise the entire block is removed. A non-allowlisted name or an
    unclosed/unbalanced block is left verbatim, like an unknown ``{name}``.
    """
