...
**One executor per clone, and the marker decides it — not a check.** `run` and `watch` each claim their liveness marker (`orchestrator.run` / `orchestrator.pid`) with an exclusive create, so of two processes racing for the same marker the kernel picks the winner and the loser refuses, naming the holder's PID. The pre-flight owner check stays in front for the better message, but it cannot be the guard on its own: the engine build and the dependency scan sit between the question and the write, and two processes that both saw the slot free there would both proceed — the loser's marker silently replacing the winner's, after which `stop` addresses the wrong process and two engines interleave their audit rows. A marker whose holder is dead, or alive with a different start-time token, is reclaimed; where liveness cannot be probed (Windows) the marker's presence is the signal and the exclusive create the whole guard.
...
- **Security ceiling** — a node's `permission_profile` may not exceed the flow `permission_ceiling`; evaluators are forced `read-only`; `extra_args` pass the forbidden-args screen; `role_file` paths contain no traversal; unknown fields fail closed; an optional `report_dir` is a repo-relative POSIX directory with portable segments that never overlaps `.worc/`, `.worc-io/` or `.git/`, is refused on a `code_change` flow, and no `code_change` role prompt names `{report_dir}`.
- **Config consistency** — a node's pinned `provider` is in `agents.allowed`, its `reasoning` is a known level, the ceiling is reachable by at least one configured provider, and a `report_dir` does not overlap the configured `paths.tasks_dir` tree. On resume the live flow is re-validated against the live config, so a config change can only ever _narrow_ what a task may do.
...
When a fix loop cannot make progress — a real environmental blocker (a sandbox/permission wall, a missing host toolchain) leaves the author unable to change the tree — the engine's **no-file-change stall guard** cuts the loop short to `manual_action_required` after a couple of unchanged rework rounds (rather than burning the whole `max_fix_cycles` budget), and the operator-facing terminal reason explains the no-progress stall. A second guard catches the opposite shape — a fixer that keeps working while the critic keeps asking for something that cannot be delivered: three consecutive **identical** non-empty finding sets from one node end the loop as `repeated_findings`. The two are deliberately separate, because "the agent did nothing" and "the critic asked for the impossible" call for opposite responses, and the failure report's `limit_exhausted` names which fired. They also differ in memory: the tree fingerprint is in-memory and resets on resume, which is right for it, while the repeated-verdict count is **derived** from the recorded `evaluations` rows (joined to the current attempt's `node_runs`, so a fresh `rerun` does not inherit it) — the loop counters it guards are persisted and outlive the process, and a guard that did not would never fire on a run that restarts more often than it repeats. A `tool` node's identical `fail` is bounded by the channel it reported through: top-level `findings` switch the detector off, a `data` object alone defers it to the fix budget the flow declares for that loop, and neither parks on the second repeat; `worc validate-flow` warns once where such a budget is written, since which of the three applies is a property of the operator's program, not of the flow. This is a deterministic, flow-agnostic backstop that needs nothing in the operator's role prompts. To keep that decision informed, on a rework re-entry the reviewer's context carries the rework-target author node's own last report (`prior_fix`), so it judges "was the finding addressed" with the implementer's account — including any stated blocker described in prose — in hand, not the diff alone.
...
**The finalize packet carries each gate's verdict on its own step, inline.** A `tool` step carries its reported `data` and a `stdout_head`, an `evaluator` step the `findings` of that pass, each under its own character cap beside the message cap (1 000 / 800 / 2 000, cut with `…`) so one chatty gate cannot inflate every packet — a flow with two lenses and ten rework rounds is summarized from all ten rather than from the last. Inline is the only form the turn can read: the packet's `diff_path` / `findings_path` now name the private copies under `.worc/logs/<task-id>/`, which survive terminal cleanup but stay read-denied to every agent at either value of `security.disable_read_isolation` — they are for the human reading the post-mortem. The `publish` step, whose own hook builds the packet and is therefore always `running` in it, is relabelled `pending` with a `note` so the turn does not report it as unfinished work.
...
`summary.json` carries **one key set on every terminal**: `{what, summary, [follow_ups], [supervisor_usage], [cost_not_accounted], [degraded]}`. `cost_not_accounted` exists because only Claude's CLI reports dollars: the attempts with no recorded cost are named per provider with their input tokens, never estimated from a price table, and `worc status` / `worc list` print the same sentence. The follow-ups section has two sources merged at close, and the composition rule is gate-aware: every evaluator finding is persisted with a `gating` flag, so only the findings a gate actually **let past** become PR follow-ups — plus a gating finding still open because a _non-blocking_ evaluator spent its rework budget, worded as still open. Each mechanically derived record carries the evaluator's own `fix` as its `action_hint` and a `title` that is the finding's first sentence, with the remainder in `rationale`. The persisted finding shape is `{severity, reason, paths, gating, fix}`; `failure_report.json` findings gain the same additive keys. The finalize turn is told not to restate the accepted findings merged in for it.
...
The **only** component that runs commit / push / PR. Before a task it prepares the branch (`worc/<epoch>-<task-id>-<slug>` by default — an epoch prefix makes every fresh run unique, and the full name is capped at 50 chars by truncating the slug — or the task's validated `branch_name`); on publish it makes a **scoped code commit** (an explicit pathspec that excludes `.worc/`, `.worc-io/`, the `paths.tasks_dir` tree and — for a `private_control_workspace_report` flow — the resolved private report directory, declared at each commit from the task's frozen flow; never `git add .`/`-A`) plus a separate **task-scoped audit commit** of just that task's `<id>.md` + `<id>.summary.md`, with a pathspec spanning every lifecycle folder so the file's removal from `preparing/` or `pending/` is staged along with its appearance in `done/`/`failed/`, pushes, and (when enabled) opens the PR with the summary as its body — an adoption notice on top and the task's `references:` lines under `## References` at the foot, when either exists — all idempotent via `publish_operations` fingerprints. The audit commit exists only when the lifecycle tree is git-tracked: with it gitignored (the `install` default) and nothing tracked under it, `commit_audit` returns before any branch is touched. A per-task `publish` is a **downgrade-only cap** over `commit < push < pull_request`: the effective scope is `min(flow policy, task.publish)`, so a task can stop the sequence early but never manufacture a PR the flow's graph does not publish. Optional **auto-merge** (off by default) merges the PR; a blocked merge ends `manual_action_required` with the PR left open, never a forced merge.

**`worc merge-task` refuses a conflict nobody decided.** Conflict markers are only one shape of conflict: Git resolves a modify/delete, an add/add or a binary conflict by leaving one side in the working tree with no marker at all. So before the merge flow runs the Git Manager reads the unmerged index entries (`ls-files -u`, index-only, so the set is identical before and after a flow that may not run Git) into a typed `ConflictedPath` inventory — its code derived from which stages exist, whether a decision could be expressed by the file's bytes at all (`ConflictEvidence`: worktree bytes, nothing to decide for a both-deleted path, or out of band for a submodule gitlink), and a whole-file digest. The rendered inventory is published as `{conflicts_path}`. After the flow, a conflicted path whose bytes are unchanged, unreadable, or out of band makes the merge abort with nothing committed and the PR open — a manual-action refusal (exit `2`) that leaves the task itself `done`. The core compares only digests and the evidence class, never Git's codes. The honest limitation is stated where it is implemented: "keep our side, unchanged" leaves the same tree as "nobody looked", and both are refused — a refused correct resolution costs one manual merge, an accepted undecided one ships a merge nobody made.
...
SQLite (`state.db`, schema **v28**). It holds the task status, the flow checkpoint (`current_node` + counters + fingerprint), the B-lite soft-pause markers (`blocked_since`, plus `blocked_until` when a provider named its own reset instant), per-node audit (`node_runs`, `provider_attempts`), checks, artifacts (each with a sha256), publish idempotency, subtasks, advisory `evaluations`, and the durable editing/own sessions (`editing_lineage` / `node_lineage` — the only place a raw session id is ever stored). `editing_lineage` is keyed `(task_id, subtask_order, lineage_key)`, so one execution unit can carry more than one durable editing session — one per lineage, keyed `lineage_affinity or <node id>`. It is also the **audit record** of what each attempt actually ran: `provider_attempts` carries the effective `model` / `reasoning` (from the same `ProviderAttempt` the prompt audit renders, so the two cannot disagree), `node_runs.abort_reason` records why a run was closed `aborted` (so `skip_reason` means only a `when`-false skip), the task row carries `recovered_loop` and the append-only `quarantine_refs`, and `artifacts.path` is POSIX and relative to the worc home. A `node_runs` row's lifetime belongs to the node layer — every runner opens it through a context manager that closes an unfinished row `aborted` with the raised reason on any exit — because a node that raises before its provider call on a path the task driver's terminal does not reach (the merge flow was one) otherwise strands the row `running` forever. Because the orchestrator is **greenfield**, the store does not migrate across destructive versions: a brand-new database is created at the current shape, and an older-versioned one is refused fail-closed (recreate it). A newer one is also refused.
...
**Two things about `strict_isolation` are worth stating precisely, because both changed.** First, it is **not** a full-access gate: the provider full-access selectors (Codex `--sandbox danger-full-access`, Claude `--permission-mode bypassPermissions`) are refused at **every** value of it, in both argv spellings, by three independent layers — the config validator, the flow validator's config-independent ceiling, and each adapter's argv builder. There is no `agents.providers.<id>.sandbox` key any more either. Second, what it checks is whether a provider's configured isolation is **legal**, at either value; whether an OS write floor exists for the run at all is a separate, **advisory** verdict — one `isolation-floor: NONE (<provider>: <cause>)` status line per provider without a floor, and the run continues; what the missing floor costs is stated once in the shipped security guide rather than recited into every report. Two different things reach that line: a **host** that cannot sandbox (native Windows; Linux/WSL2 without `bubblewrap`+`socat`), at either value of the key, and `strict_isolation: false` itself, on **every** host, because the mode raises no OS sandbox for Claude anywhere (owner decision 2026-09-03). Codex is not symmetric and the mode does not pretend otherwise: it always gets a generated permission profile, so its floor does not move with the key. Under `strict_isolation: true` the refusal happens later and narrower, as a per-node `CAPABILITY_UNAVAILABLE` raised for the attempt that actually needs a sandboxed shell, which a fallback provider can cover.

**`strict_isolation: false` is the advanced mode** — one door, not a matrix. It forwards the parent environment whole to every process run on the agent's behalf, hands every node an unscoped shell and the full tool surface (the skill tool included, which is why this is the only mode in which a flow node may name a skill), widens writes to wherever the operator's own account can write rather than the clone, raises **no OS sandbox for Claude on any host**, and puts every node online whatever its flow granted. Under it, four floor levels are what remain, and the first is mechanical on one provider only: (1) `.git` and `.worc` stay unwritable — re-proved by Codex before every attempt under its generated profile, but on Claude held by the tool-level write denies (`Write`, `Edit`, `MultiEdit`, `NotebookEdit`) plus the prompt contract and after-the-fact detection, since nothing binds the subprocesses a command starts there; (2) publication to your `origin` is neither held nor detected — it is _recovered_ from, per §4.6; (3) publication anywhere else is held by nothing and is asked for only in the prompt every node receives; (4) publication _as_ the orchestrator is **reported** by detection, never held. `allow_git_evidence` and a node's `permission_profile` / `network_policy` are inert in this mode: there is no capability left for them to hand out or withhold. The mode is recorded in the frozen control bundle's `manifest.json` (the mode the task started in) and in the completed ledger record's `advanced_mode` (a task that finished) — deliberately never in the pull request.
...
As **defense in depth** (not enforcement), the orchestrator also prepends a short, fixed, Core-owned **security preamble** to every provider prompt (agent/evaluator/supervisor) — "change only what the task requires and only inside your clone; don't read or write `.worc/`; `.worc-io/` is read-only **input** — the paths you were given under it are yours to read, and only writing it is banned; don't touch git control state or `tasks/`; never commit/push/merge or open a PR; never read credential files or provider auth homes" — built once from the layout constants and carried on `AgentRunRequest.security_preamble`, prepended at the single neutral seam `build_effective_prompt` (order `preamble → role prompt → skills block → context footer`). The **skills block** is the second Core-built text at that seam, present only for a node that declared [`skills:`](flow-authoring.md#node-declared-skills); it sits before the footer so its "the instructions above win" covers both the preamble and the role prompt, and it states out loud that no skill grants any right to publish. The seam is also where a node's **continuation prompt** is chosen: Core decides what _may_ be a continuation (the node has already completed a run under the provider that owns the session, scoped to the unit), and the seam decides what _is_, per attempt, from the same field that decides the resume argv — so an attempt whose session the router dropped is handed the full text back with no router change. The repo's own root instruction files (`AGENTS.md`, `AGENTS.override.md`, `CLAUDE.md`) are deliberately **not** in that deny list — the preamble names them as ordinary repository files to edit when the task calls for it, forbidding only the opportunistic rewriting of the agent's own rules. Together with `.agents/rules/**` they are the governance **floor**: a diff that touches one is not blocked, it is _reported_ to the operator (console, PR summary, ledger, Telegram). The floor cannot be narrowed or switched off, but a repository that keeps its rules elsewhere names them in `repo.governance_paths`, which is unioned with it — the key has no exclusion syntax, and a value that tries to invent one (`!`, absolute, `~`, `..`) is refused at load. It gains a read-restraint reinforcement when read-isolation is off, and — on a run with no OS sandbox under it (a host where none exists, or the advanced mode on any host) — a paragraph that says so outright: nothing but the node's own compliance keeps a write out of `.git/` or `.worc/`. That paragraph is rendered **only** when it is true; claiming it everywhere would be false under strict isolation on a capable host and would teach the reader to discount the whole block. `.worc-io/` is the one asymmetric root, and the wording that once folded it into the read ban is gone: it made reviewers refuse to review, filing a blocking finding that their own context files were forbidden reading, and that refusal then travelled to `fixing` as if it were rework. Elsewhere the preamble is a soft backstop only — the sandbox + deny projection remain the enforcement where they exist (VF-7).
...
| `deep_research` | `deep_research` | a documentation PR (`docs/research/<id>/` by default — a flow's `report_dir` moves the base) | three disjoint analysis passes behind a `coverage_gate`, `external_research` (network-gated), a `citation` checker, a `command_profile` document gate, two non-blocking evaluators |
| `security_audit` | `security_audit` | a **private** report under `.worc/security-reports/<id>/` by default (`report_dir` may move it; a base outside `.worc/` is the operator's to gitignore) | a `dependency_scan` checker; `publishing: none` (no git at all) |
...
**A terminal `done` advertises no failure.** A loop guard that stops a fix loop writes `failure_report.json` / `stuck.md`, and the task can still finish — a flow whose evaluator does not gate publication carries on, or an operator continues it. At the transition to `done` the two artifacts are renamed `recovered-*`, the task's `failure_report_path` is cleared, `tasks.recovered_loop` names the loop, and the ledger record says `recovered_from_stuck` with that `recovered_loop` instead of pointing at a report — the evidence is kept, the claim that a finished task is failing on it is dropped. `worc list` shows such a task as `done (recovered: <loop>)`; a fresh `rerun` clears the mark.
...
repo:
  {
    ...
    governance_paths: [], # ADDS rule locations to the reported governance floor
  }
...
agents:
  ...
  providers: # node-based routing: a node declares `provider`, else the global primary below
    ...
    codex:
      # `permission_profile` is the provider-neutral access level for BOTH providers; the old
      # `sandbox` key is gone (a load error since v38) and full access is refused at any setting.
      {
        command: codex,
        model: "",
        reasoning: null,
...
logging: { level: warning, artifacts: standard, clean_runs_on_success: true }
...
Everything the orchestrator generates lives under two gitignored roots. The private/control home `<repo>/.worc/` holds `config.yaml`, the agent task-authoring `guide/`, operator `flows/` and `tools/`, `state.db` (+ `-wal`/`-shm`), `orchestrator.pid`, `logs/` (plan, diffs, stage logs, `summary.json`, validation reports, provider attempts, prompt audit, durable HITL), `memory/`, `security-reports/`, `workspace/`, the per-task `runs/` parent (below), and the `tasks/rejected` quarantine. `install` appends a `.worc/` and a `.worc-io/` line to the repo's tracked `.gitignore` — and, unless the operator asked to track task files (`Track task files in git?`, `--track-tasks`), an anchored `/tasks/` line for the lifecycle tree (`paths.tasks_dir`; `install --tasks-dir DIR` scaffolds, configures and ignores a different one from that single value). The anchoring matters: `tasks` is a common directory name, and an unanchored pattern would also ignore a `src/tasks/` package. The line is never seeded over a tree git already tracks something in, and the per-run `.git/info/exclude` repair covers only the two runtime roots, so deleting that one line is how the operator turns tracking on.
...
The second root is the **exchange** `<repo>/.worc-io/<task-id>/` (WRI-001) — the only provider-readable orchestration surface. Everything an agent reads as a context path (the task snapshot, `plan.md`, `current.diff`, the first failing checks log, evaluator `findings.json`, generic `<node>.out.md` / tool `stdout.txt`, subtask specs, handoff briefs, memory packets, a sanitized HITL answer packet) is published there through **one redaction + path-safety boundary** (`providers/exchange.py`): content is scrubbed, the destination is proven a contained single-link regular file (no symlink/junction/reparse/hard-link/NTFS-ADS escape), the write is atomic and LF-stable. Private writers keep writing under `.worc/logs/` unchanged — each routing point is additive (the private copy stays the audit record; only a redacted copy crosses). A pre-launch invariant enforces that `.worc-io/` holds at most the current task's directory, and each provider request is checked so no live/private/control path (only `working_directory`) reaches the agent. At every terminal status the orchestrator **seals** a checksum-verified copy into `<private_home>/runs/exchange-seals/<task-id>/seal-<NNNNNN>/` and then removes the active `.worc-io/<task-id>/` (WRI-007), so a terminal task never leaves an agent-readable exchange behind; `rerun --continue` restores the latest verified snapshot. A tree that mutation detection flags is **quarantined** to `runs/exchange-quarantine/<task-id>/<NNNNNN>/` instead — never sealed, never restored — with an `evidence.json` carrying the expected and observed manifests, `created_at`, the mutating `node_id` and the task `attempt`, and the bundle path appended to `tasks.quarantine_refs`; a bundle with nothing to record is not written. Automatic recovery refuses to resume over a contaminated exchange; an operator `rerun --continue` clears the flag (the refs outlive it) and continues.

Outside these two roots sit the `tasks/` lifecycle dirs (`preparing`/`pending`/`done`/`failed`, the four names fixed in `runtime_layout.py` beside the roots; the parent is `paths.tasks_dir`) at the repo root — gitignored by default, and git-tracked only if the operator asks: then the task file plus its `<id>.summary.md` (in `done/` or `failed/`) are the committed audit trail. The code commit excludes `.worc/` (gitignored), `.worc-io/` (gitignored), and the lifecycle tree (it rides the separate audit commit, when there is one). The one other place a flow writes outside them is its **report directory**: `<report_dir>/<task-id>/`, where `report_dir` is the flow's optional base — `docs/research` by default for `repository_document` (committed like any other change), `.worc/security-reports` for `private_control_workspace_report`. For the private policy the invariant "a private report never enters git" no longer rests on the default base sitting under `.worc/`; it is carried in two places at run time, together covering every terminal: the publish node refuses a report with any git-trackable file (`manual_action_required`), and the resolved private directory is excluded from the code commit's staging set for the whole run — declared per commit from the task's frozen flow, so a fresh-process `worc resume` and the operator merge path are covered, and excluded base-wide on the merge sweep. A private base outside `.worc/` is therefore the operator's to gitignore. Parallel tasks via `git worktree` remain on the roadmap (see [backlog/](backlog/)).
...
9.  publish: scoped code commit + task-scoped audit commit (only when the lifecycle tree is git-tracked), push, gh pr create (PR body = the summary, + the task's `references:` under `## References`) — idempotent
...
```text
install       set up <repo>/.worc/ (config + guide + flows + tools), gitignore .worc/, .worc-io/ and (by default;
              --track-tasks / --no-track-tasks) the task lifecycle tree (--tasks-dir DIR), run preflight
...
run           process exactly one task end to end — arg is a PATH to the task file, not a task id;
              refuses a terminal task's own leftover file (use rerun / finalize)
...
merge-task    go-ahead to merge a reviewed PR (update branch w/ base, resolve conflicts, merge);
              a conflicted path left without a decision aborts the merge (exit 2, PR left open)
...
stop / restart  manage the watch daemon via <repo>/.worc/orchestrator.pid (the stop ladder;
                a `run` records itself in orchestrator.run — seen by every liveness probe,
                deliberately out of the stop ladder's reach, since it has no stop wiring)
