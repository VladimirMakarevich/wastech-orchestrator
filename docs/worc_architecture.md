...
- **Security ceiling** — a node's `permission_profile` may not exceed the flow `permission_ceiling`; evaluators are forced `read-only`; `extra_args` pass the common forbidden-args screen and Codex arguments additionally pass its closed typed parser; `role_file` paths contain no traversal; unknown fields fail closed.
