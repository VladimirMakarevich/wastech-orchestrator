...
There is one canonical layout: the orchestrator's runtime home is the gitignored `<repo>/.worc/` (config, `logs/`, `state.db`, `workspace/`, ...), and the `tasks/` lifecycle dirs live at the repo root — gitignored by default, trackable on request. The _code_ commit uses scoped staging and excludes `.worc/` and the lifecycle tree; when that tree is tracked, a separate task-scoped **audit commit** records it — the task file moved to `done/`/`failed/` plus its `<id>.summary.md` (with the tree gitignored there is no audit commit at all). Everything under `.worc/` (plan, review, diffs, `summary.json`) stays local and is never committed.
