...
- At whole-task **close** (before `publish` in the `implementation` flow) it synthesizes the plain-language `summary.md` (the PR body) plus advisory caveats and records `supervisor_final`. The finalize turn runs on a **fresh** session seeded by a deterministic packet of the run's facts (`.worc-io/<task-id>/supervisor/packet.json`) built from the recorded node runs and each node's own output — never from the observations — so its input is a few kilobytes regardless of how long the run was, a resumed task's summary is as complete as a first run's, and `mode: none` still produces a full PR body. It is also handed every in-flow evaluator's recorded verdict and findings, so a gate that accepted **with** findings cannot be summarized as one that simply passed — and the packet repeats each verdict on the step that produced it. A `tool` step carries the `data` object its program reported and a `stdout_head` of its output, an `evaluator` step the `findings` of that pass, each under its own size cap (1 000 / 800 / 2 000 characters, cut with a trailing `…`), so a flow with two lenses and ten rework rounds is summarized from all ten. They are inline because it is the only form the turn can read: the packet's `diff_path` / `findings_path` name the private copies under `.worc/logs/<task-id>/`, which survive terminal cleanup but stay read-denied to every agent — they are for the person reading the post-mortem. The `publish` step, whose own finalize hook builds the packet, is shown `pending` with a `note` saying that is expected.
...
`follow_ups`, `supervisor_usage`, `cost_not_accounted` and `degraded` are present only when they apply — `cost_not_accounted` exactly when some attempt of the task carries no recorded cost. (The former four-field `What / How / Integration / Why` stub is gone, and with it the `how` / `integration` / `why` triad.)
...
It is a **reading aid, not the record**: the effective model and reasoning of every attempt are also written to `provider_attempts.model` / `.reasoning` and to each attempt's `result.json`, from the same source, so a per-model spend query (`SELECT model, SUM(usage_cost) FROM provider_attempts GROUP BY model`) works with this switch off.
