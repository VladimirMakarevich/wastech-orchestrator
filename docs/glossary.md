...
- **`shell`** - Interactive operator console (the `[shell]` extra): a prompt_toolkit REPL. Entry is **passive** — it attaches to a live `watch` daemon or opens idle (the queue is not served on entry; it never auto-spawns or auto-claims). `up`/`watch` starts serving by spawning a daemon and verifying it came up (surfacing the captured startup error on failure); `enqueue`/`promote`/`ps` (`jobs`)/`status`/`tasks`/`list`/`logs`/`prs`/`merge-task`/`finalize`/`rerun`/`down`/`restart`/`cancel`/`clear` dispatch onto the existing commands (`clear` wipes the console screen), and `help` (`?`) prints the verb list. `quit` (`exit`) **detaches** (the daemon and any in-flight task keep running; reopen to reattach) — warning and asking for confirmation first when the daemon is still serving the queue. It never hosts the engine itself.
...
- **`preflight`** - Runs readiness checks for the resolved env file, every allowed provider (version, auth probe, a `<provider>-binary:` resolution line, and a live no-model isolation capability smoke), the isolation policy and its host floor (`isolation-floor:`), the environment expansion and assigned paths (`allowed-environment:`, `extra-environment:`, `assigned-paths:`), the `gh` repository pin (`gh-repo-pin:`), `gh`, Telegram, and the configured check sets, announcing each active relaxation (`read-isolation: OFF`, `git-evidence: ON`, the advanced-mode line). Read-only in intent but **not filesystem-read-only**: `assigned-paths:` resolves real paths and may repair the clone-local `.git/info/exclude`. It does **not** validate flows — that is `validate-flow`.
- **`preflight --paid-isolation-probe`** - The one probe that cannot be free. Claude offers no way to run a command under its sandbox without the model, so this spends **one real (billed) model call** per supporting provider, letting an agent try to write into the two Git directories, the control home, and one allowed workspace path; the verdict is read from the **filesystem**, never from what the model said. Never implied — no run and no plain `preflight` spends it. Evidence lands at `.worc/preflight/claude-paid-isolation-probe.json`.
- **`telegram-test`** - Sends one correlated Telegram prompt and waits for a reply as a smoke test.
- **`upgrade-config`** - Updates an older `config.yaml` to the current schema shape without changing user values. It **confirms before writing** (`-y` / `--yes` skips; `--dry-run` never asks), because its add-missing-only merge writes back **every** key a deliberately small generated config left at its default, and re-emitting via YAML drops inline comments. Two removals it deliberately does not strip: `agents.providers.<id>.sandbox` and the whole `skills` block.
- **`upgrade-docs`** - Refreshes the installed packaged guide copy under `.worc/guide/`. Generated content with no operator edits to preserve, so it is a straight overwrite (no backup): differing files are rewritten and files no longer in the package are removed; `--dry-run` previews.
...
- **`validate-flow`** - Runs the full fatal flow validator (graph, ceiling, and the config-aware layer including the `.worc/tools/` tool check) on demand over the operator's own flows in `.worc/flows/`, read-only and without claiming a task, plus the non-fatal prompt-variable lint (a `WARN` per unknown `{name}`, which would otherwise render verbatim to the agent). Takes a flow NAME or `--all` (a bare invocation is a usage error). Exit `0` valid / `1` invalid / `2` not found or no config. **`preflight` no longer validates flows** — this is where flow correctness is checked before you queue work, and the engine enforces the same validator fatally at dispatch regardless.
...
- **`--log-level`** / **`--log-format`** / **`--log-file`** / **`--heartbeat-seconds`** - The global operator-output flags: verbosity (overriding `logging.level`), `logfmt` or `json`, an additional rotating log file (10 MB, 5 backups — a `watch` daemon's copy of it is what `top` and `shell` tail), and the long-operation heartbeat interval (default `30`, `0` disables).
...
- **Front matter** - The YAML block at the top of a Markdown task file. It carries the task metadata that the validation gate allows: a closed key set (`ALLOWED_TASK_KEYS`), with an unrecognized key rejected unconditionally and `id` plus a non-blank `title` and a non-empty `## Description` required. None of that is operator-configurable — the keys that once pretended it was are gone (config v35).
...
- **`branch_name`** - Full task-branch override (only in `new` branch mode). When omitted, the orchestrator derives `{repo.branch_prefix}/{epoch}-{id}-{slug}` — the Unix-epoch stamp is what makes a fresh attempt's branch unique, so a rerun never collides with the previous one.
...
- **`slug`** - Lowercase branch fragment derived from the task title, truncated to whatever the 50-character readability budget leaves after the prefix, epoch, and id (and dropped entirely when nothing is left). It is part of the default branch name, not a separate task field.
...
- **`depends_on`** - List of other task ids that must be merged before this task can start. Non-blocking: the scheduler skips a dependent while a dependency is unmerged and runs another eligible task instead. Eligibility is computed live from PR/merge state — nothing is persisted for it.
- **`priority`** - Scheduling hint for the eligibility queue: `high` / `mid` (default) / `low`, ties broken by filename. Fail-open — an unrecognized value folds to `mid`, so a typo never blocks a task. It re-orders eligible tasks only; `depends_on` is always stronger, and the single slot is unchanged.
- **`queue`** (task field) - Queue tag that partitions one git-distributed task pool across several worc instances: an instance picks the task only when its `orchestrator.queue` equals this value (plain string equality, no balancing). Defaults to `default`, and unlike `priority` it is fail-closed — a non-string or blank value rejects the task. Decomposition subtasks inherit it implicitly.
...
- **State machine** - The finite set of task statuses and allowed transitions between them: `new → validated → preparing → running → (done | failed | manual_action_required)`, with `pending → validated | preparing` ahead of the slot and a `→ failed` / `→ manual_action_required` bailout from every non-terminal status. Terminal statuses have no outgoing edge.
- **Park** - A resumable pause, not a status: the row stays `running` while `tasks.blocked_since` records the first park instant (the clock `agents.retry.max_blocked_s` measures) and `tasks.blocked_until` records when the provider says its window reopens. A `running` row with no live daemon reads as `parked (no daemon)`, and a soft `stop --force` parks at the next node boundary.
...
- **`schema_version`** - The config format version (currently `39`), bumped only when the format changes. The loader refuses a newer schema than it understands; an older or absent one loads, and `upgrade-config` brings it forward.
- **`orchestrator`** - The outer queue behavior block. It controls automatic continuation, the watch poll interval, and this instance's queue selector.
- **`orchestrator.auto_mode.enabled`** - When `true`, `watch` may pick the next pending task after terminal cleanup succeeds.
- **`orchestrator.auto_mode.confirm_next_task`** - Fail-closed Telegram approve/deny prompt before `watch` claims each pending task (default `false`; requires `telegram.enabled`). It gates new claims only — resuming an in-flight task after a daemon restart is never gated.
- **`orchestrator.poll_interval_seconds`** - Watch loop interval; each tick fetch/pulls `base_branch` before re-scanning. `0` makes `watch` a single pass.
- **`orchestrator.queue`** - This instance's queue selector (default `default`), matched by string equality against a task's `queue`. `worc watch --queue NAME` overrides it per launch.
...
- **`paths.tasks_dir`** - Repo-relative directory holding the `preparing`/`pending`/`done`/`failed` lifecycle subfolders (default `tasks`). Validated repo-relative (no `..`/absolute) and rejected if it lives under the gitignored `.worc/` home; a subpath such as `config/tasks` is allowed.
...
- **`agents.retry`** - The bounded same-provider transient-retry policy plus the park ceiling: `max_attempts` (retries after the first failure, default `2`, counted per provider in the route and separately from `max_stage_attempts`), deterministic exponential backoff between `base_delay_s` (`2.0`) and `max_delay_s` (`30.0`), and `max_blocked_s` (`21600.0` — 6h) as the total parked wall-clock after which a parked task fails. Only the transient classes (`provider_unavailable`, `network_unavailable`) are retried.
...
- **`agents.providers.<id>`** - Provider-specific config for `codex` or `claude`. It holds the executable command, model, reasoning level, timeout, permission profile, extra args, and the `primary` marker; Codex also uses `sandbox`, and Claude also uses `max_turns`, `max_turns_gate`, and `allow_native_memory`.
- **`agents.providers.claude.max_turns_gate`** - Claude-only: a run that exhausts `max_turns` pauses for a durable Telegram continue/stop prompt instead of failing (default `false`; requires `telegram.enabled`). Continue resumes the same agent session with a fresh turn grant, which is what makes a low `max_turns` safe.
...
- **`security.allow_git_evidence`** - The operator grant (default `false`) that makes a flow node's `git_evidence` declaration effective. `config.yaml`-only, never a task / `extra_args` / flow key.
...
- **`validation.max_task_bytes`**, **`validation.max_task_lines`**, **`validation.max_line_bytes`**, **`validation.max_control_ratio`** - Size and content ceilings for task input (defaults `262144`, `5000`, `8192`, `0.01`). These four plus `quarantine_folder` are the whole block — which fields are required and whether an unknown key is refused are hard-coded in the gate, not configurable (config v35).
- **`validation.quarantine_folder`** - Where invalid tasks are moved (default `./.worc/tasks/rejected`).
...
- **`checks.timeout_seconds`** - Default per-command timeout for the check runner (default `7200`); a set's own `timeout_seconds` overrides it.
...
- **`git.auto_merge_strategy`** - Merge strategy used when auto-merge runs, `merge`/`squash`/`rebase` (default `squash`), and the default for `merge-task --strategy`.
...
- **`supervisor_usage`** - The measured per-run cost block in `summary.json`: calls, input, cache reads and writes, output, cost, and provider wall time — as a `total` and a `by_function` split over the jobs that ran (`observe`/`finalize`/`handoff`), in run order. Absent entirely when the layer made no calls, because a report of zeros would read as "it ran and cost nothing".
- **`logging.level`** - Persisted operator trace verbosity (`debug`/`info`/`warning`/`error`, default `info`); the `--log-level` flag overrides it for one invocation.
- **`logging.artifacts`** - Which per-attempt provider files survive under `logs/<task-id>/stages/…/<attempt>-<provider>/`: `minimal` (only `result.json`), `standard` (default; adds `stdout.log`/`stderr.log`), `full` (everything). Prompt-audit is independent.
...
- **`memory`** - The repo-scoped persistent memory block: `enabled` (dataclass default `false`, but a fresh `install` writes `true`) plus bounded, runtime-clamped knobs — short-term TTL, per-node retrieval packet caps, promotion thresholds, and the background-cleanup budget. No knob is a fatal config error. `supervisor.enabled: false` forces `memory.enabled` to `false` for the run.
- **Memory tiers** - The storage families the store keeps and `worc memory clear --kind` targets: `short` (recent episodic), `long` (the promoted long-term lessons), `entity` (repo entities), and `quarantine` (the mixed-kind holding area).
...
- **`merge`** - The conflict-resolution flow `worc merge-task` runs (only) when pulling `base_branch` into a task branch conflicts: `conflict_resolution` (agent) → `testing` (checks) with a bounded `fixing` loop, terminating at a no-op publish node (`done`, `policy: none`). It performs no git itself — the orchestrator commits the merge and merges the PR after the flow returns a clean, green tree. Not dispatched for incoming tasks; selected by `git.merge_flow`.
...
- **Tool registry** - The `.worc/tools/` directory that turns a `tool` node's free-string `tool:` name into an executable, owning the path containment (a name resolves to a file inside that directory, never a path). `install` seeds the two shipped tools, `check_chapter` and `check_length` (each with a `.cmd` wrapper for Windows); a node's flat scalar `args` reach the tool on stdin.
...
- **`output_artifact`** - A named slot that stores an agent node's result and threads it downstream. The slot table is core-fixed and closed: `enriched_spec` (`task.enriched.md`, audit-only), `plan` (`plan.md`, the one slot that reaches a downstream provider, as a redacted exchange copy), `summary` (`summary.md`, an orchestrator publish input), and `report` (`report.md`, written privately into the flow's report directory).
...
- **`publishing`** - The terminal publication policy for a flow — a closed set of five: `pull_request`, `documentation_pull_request`, `local_artifact`, `private_control_workspace_report`, and `none`.
- **`network_policy`** - The flow-wide network grant, `advisories` (vulnerability/package metadata, used by `security_audit`) or `research` (broader external fetches, used by `deep_research`). Absence means no network by default.
- **`network_access`** (flow node field) - Per-node override of that flow-wide default: `true` grants network to this node even in a flow with no `network_policy`, `false` forces it offline even when the flow grants one, and absence inherits. It moves only the network dimension — never the filesystem ceiling.
...
- **`ProviderHealth`** - The preflight result for one provider: executable availability, version, secret-free diagnostics, the advisory `degraded_reasons` (fatal only when that provider has no fallback), and the `auth` probe — `None` there means _not probed_, never "authenticated".
...
- **`AgentRunRequest`** - The request object sent to a provider. It carries the task id, node id, working directory, prompt, permission profile, timeout, attempt and node-run id, model, reasoning, session id, extra args, the optional output schema, and artifact paths — task content reaches a provider as a path, never as argv.
...
- **Control-state drift** (WRI-009) - Git control state (index, HEAD, task ref, repo-local config, hooks, merge markers) changing across a provider attempt **that gets a shell**. It is **reported and never parks the task, on any node class** — a `WARNING` carrying the redacted aspect summary plus the ⚠️ trace label `TRACE_GIT_CONTROL_DRIFT`, and the run continues. The fingerprint reads "the state moved", never whose hand moved it, so parking cost a finished node's work over what is usually the operator's own neighbouring commit. Read the warning as a **stop-the-run signal**; what actually holds a commit made inside a run is the pre-publish dangerous-diff gate.
- **Shell reach** - The provider-neutral question "does _this attempt_ get a shell?", which is what every drift/write bracket keys on rather than a profile name: Codex's `read-only` sandbox permits commands, while Claude's depends on whether the resolved tool set keeps `Bash`. The seam dispatches by provider id and the adapters answer; it is **fail-closed toward bracketing** (an unknown provider yields `True`), because a wrong `True` costs one fingerprint and a wrong `False` leaves an attempt unwatched.
- **⚠️ synthetic trace labels** - The four Telegram/console trace outcomes that are not a plain node verdict: `TRACE_REWORK_EXHAUSTED` (a non-blocking evaluator accepted after spending its rework budget), `TRACE_UNEXPECTED_WRITE` — `done (node wrote to the workspace unexpectedly)` — for any node with a shell but no write access that changed the tree, `TRACE_GIT_CONTROL_DRIFT` — `done (node changed git control state)` — the sharper half of the same event, and `TRACE_ADOPTED_COMMITS` — `done (publish adopted commits it did not make)`. The middle two were renamed from `TRACE_READ_ONLY_WRITE` / `TRACE_READ_ONLY_GIT_DRIFT` when the park went away, because they no longer describe a `read-only`-only phenomenon.
...
- **Green-but-dirtying guard** - The core-owned property of a `command_profile` node: the working tree is snapshotted before and after the run, and a _passing_ check that rewrote commit-candidate files (an auto-formatter, say) fails closed to `manual_action_required` with the node run recorded as `dirtied_working_tree`. A flow cannot declare it away or disable it; the read-only `citation` / `dependency_scan` checkers are out of its scope.
...
- **`strict_isolation`** - The master posture switch, and the only one. The dataclass/loader default is `true` (the fail-closed sandbox, and what an omitted key means); **`install` writes `false`**, which _is_ the advanced mode. It is **not** a full-access gate — both provider full-access selectors are refused at every value of it. What it checks at either value is whether a provider's configured isolation is **legal**.
- **Advanced mode** - `strict_isolation: false`. One door, not a matrix: "full freedom for the agent under your responsibility, except the floor". It forwards the parent environment whole to every process run on the agent's behalf, hands every node an unscoped shell and the full tool surface, widens writes to the workspace volume rather than the clone, and puts every node online whatever its flow granted. `allow_git_evidence`, `permission_profile` and `network_policy` become inert in it. Recorded in the frozen control bundle's `manifest.json` (the mode the task started in) and the completed ledger record's `advanced_mode` (a task that finished) — deliberately never in the pull request.
- **Isolation floor** - The four levels that remain under advanced mode: (1) `.git`/`.worc` unwritable wherever the host can sandbox at all; (2) publication to your `origin` neither held nor detected — _recovered from_ instead; (3) publication anywhere else held by nothing, asked for only in the prompt every node receives; (4) publication _as_ the orchestrator **reported** by detection, never held. Level 1's absence on a host that cannot sandbox is the advisory `isolation-floor:` verdict.
- **`allowed_environment`** - The name gate for environment variables reaching child processes. **Replaces** its OS-aware default (9 names on Linux/macOS, 19 on Windows) rather than extending it. An entry is an exact name or a **prefix pattern** — a name plus one trailing `*`; `*` anywhere else is a load error, as is a secret-bearing prefix. `PATH` coverage is mandatory (a load error otherwise); on Windows a missing `SystemRoot` is a launch-critical FAIL at preflight and at `run`/`watch`/`rerun` start. Under advanced mode it gates only orchestrator-owned `git`/`gh`.
- **`extra_environment`** - Variables the orchestrator **assigns** (rather than forwards) to agent/check/tool children and to orchestrator-owned `git`/`gh`. Strings only; `PATH`, secret-looking names, non-identifier names and case-duplicates are rejected. For toolchain roots and caches, **never credentials** — values are plaintext in `config.yaml`. A value whose path overlaps `.git`, `.worc`, `.worc-io`, the tasks dir, the env-file or a `denied_read_paths` match is rejected lexically at load and canonically in preflight. For `git`/`gh` it passes one more gate: a **whitelist** over the `GIT_*`/`GH_*`/`GITHUB_*` namespace admitting only `GIT_CONFIG_GLOBAL`, `GH_TOKEN`, `GITHUB_TOKEN`.
- **Pinned launchers** - The executables the orchestrator launches _as itself_ — `git`, `gh`, `ps`, `bwrap`, the agent CLIs, and the `worc` console script the `watch` daemon re-invokes — resolved **once at startup** and used as those absolute paths for the rest of the process. It removes the substitution window _inside_ a run; it does not cover a stand-in already in place before the run, nor an edit to the installed package's own code.
- **Dangerous-diff gate** - The mid-task approval, shared by the **agent** node (which can send itself back with the denial) and the **publish** node (where the agent is gone, so a denial is a manual stop). It measures from **the last commit the orchestrator itself made** for the task, or the task's `base_ref` until there is one — never `HEAD`. It asks in three places: after a writing node's edit, at that node's `hitl` round-trip, and immediately before the publishing commit. The same approved change is never asked about twice in one task.
...
- **`Git Manager`** - The only component that commits, pushes, or opens a PR. Every push and PR call returns a **structured outcome** (`PushOutcome`, plus a `RemoteState` capture around the attempt) rather than a boolean, because "the branch already exists on `origin`" has four different right answers — see **Publishing recovers**.
- **Publishing recovers** - The rule that an existing branch, a moved branch, and an open pull request on the task head are **ordinary working state, never evidence of foreign ownership** (inferring that does not hold: a recreated `state.db` has no records at all while the branch and its PR are still yours). So provenance is deliberately not inferred and recovery is the mechanism: a remote that matches sends nothing, one behind takes an ordinary push, one that diverged into exactly the commit we recorded pushing takes a lease-guarded force-push, and one that diverged from something we never pushed has those commits **merged in locally**, the quality gate re-run over the combination, and only then is anything sent. An open PR on that head is adopted, retitled, and appended to.
- **Adopted commits** - Commits publishing had to merge in because they were on the remote and the orchestrator never put them there. The run records the **adopted** commit rather than reporting one it never performed, and says so in the pull-request body — or, with `publish: push`/`commit` where there is no body, in the run log and the ⚠️ `TRACE_ADOPTED_COMMITS` trace.
- **`publish_operations.pushed_sha`** - The commit a push actually left on the remote, so a branch someone else moved is distinguishable from the one we put there. Nullable, and NULL is its real meaning ("not pushed by us"), not a placeholder.
...
- **Frozen instruction bundle** (WRI-011) - A per-task, immutable snapshot of the agent inputs whose identity must stay stable — the validated task packet and the root repository instruction files (`AGENTS.md` / `AGENTS.override.md` / `CLAUDE.md`) — under one `instruction_manifest_digest`, at `<private_home>/runs/instruction-bundles/<task-id>/`. The agent only ever reads redacted exchange copies of these.
...
- **`state.db`** - The SQLite state store that keeps task progress and run bookkeeping. Its schema version is stamped in `PRAGMA user_version` (currently `25`): a newer database is refused, and so is an **older versioned** one (`1 ≤ v < 25`) — past bumps dropped and renamed tables, so an old shape cannot be reshaped in place; delete the local `state.db` and start fresh (greenfield). Only a `0` (brand-new or pre-versioning) database is adopted, migrated with the additive columns, and stamped.
- **`tasks`** - The one row per task: status, branch and slug, the fix-cycle and subtask counters, `current_node`, `flow_fingerprint`, the park timestamps, the cleanup bookkeeping, the frozen-bundle digests, and three nullable reference points below. The PR is not here — it lives in `publish_operations`.
- **`tasks.base_ref`** - The commit the working branch sat at when the task started. NULL means the config base branch is the start. It must survive the process: re-derived from `HEAD` after the run has committed it would walk forward, and the task's reported change would shrink to whatever is still uncommitted.
- **`tasks.gate_reference_sha`** - The commit the dangerous-diff gate measures the task's change from — the last commit the orchestrator itself made. NULL means the task's diff base. It advances per subtask commit in a decomposed run, which is why an approval given on one subtask is not requested again on the next.
- **`tasks.push_url_digest`** - The sha256 of where a push would go, read once at branch prep before any provider ran, so the unconditional pre-push re-read has a baseline. A digest rather than the URL because it can carry a token. NULL means no baseline was taken and is never read as a rewrite.
- **`artifacts`** - The per-task registry of produced files (the diff, reports, node outputs) as `kind` + `path` + checksum, so a resume or a report finds them without re-deriving paths.
- **`subtasks`** - The persisted decomposition units of a split task in linear `order`, each with its own status, `depends_on`, commit sha, and spec path.
...
- **`Notifier`** - The narrow interface for terminal notifications, the one-way node trace, and two-phase HITL interactions (`start_ask` then `wait_for_answer`, so a wait survives a restart).
- **Trace feed** - The one-way, best-effort per-node progress channel (`send_trace`, enabled by `telegram.trace`): one message per finished flow node carrying the node id and outcome only, never content.
...
- **`OutputContract`** - The per-node typed-output selector for agent nodes: `none`, `human_input`, or `planning`. Derived from the node's declared structure, never from its name — the flow's decomposition proposer gets `planning`, any other HITL-capable node gets `human_input`, a plain author node gets `none`.
...
- **`validation.required_fields`** and **`validation.reject_unknown_fields`** - Removed (config v35). Both were parsed and even written by `install`, but read by nothing: the task gate hard-codes the requirement (`id`, a non-blank `title`, a non-empty `## Description`) and rejects an unrecognized front-matter key unconditionally. Neither was the operator's to set — a config able to drop `id` or open the fail-closed unknown-key gate would weaken invariants that branch names, run directories, and the state store depend on. Tolerated on load (every installed config carries them); `upgrade-config` strips them.
- **`checks.discovery`** / **`checks.commands`** (v15), **`git.auto_merge_allow_per_task`** (v11), **`agents.providers.<id>.max_budget_usd`** (v14) - Other removed keys `upgrade-config` strips. They are tolerated on load, never read.
...
- **`agents.providers.codex.sandbox`** - The key no longer exists at all (removed in config **v38**). Its one remaining value selected a provider full-access mode, and that mode is now refused at every value of `security.strict_isolation`, so there is nothing left for it to take. It is **not** tolerated on load and **not** stripped by `upgrade-config`: a config carrying it is a load error with a named message telling you to delete the line. The access level is `permission_profile` plus `security.strict_isolation`.
- **`skills` block** (`skills.dynamic`, `skills.strict`) and **`skill_map.json`** - Repo-skill selection was removed outright: whole-repo `SKILL.md` discovery, operator pins on a flow node's `skills:` list, the once-per-task supervisor proposal, the `{skills_path}` prompt variable, the persisted per-node map, and the skill packages in the frozen instruction bundle are all gone. The block is **deliberately absent** from the strip list for the same reason as `sandbox` — a config still carrying it is a load error the operator resolves by deleting it, because stripping it would read as "the block still works, it just does nothing". `supervisor_usage.by_function` lost its `skill` job with it, leaving three.
- **`agents.providers.claude.allow_native_memory`** - Removed. It governed the write side of a deny on Claude's own config home; that deny no longer exists, because the agent CLIs' config homes (`~/.claude` / `$CLAUDE_CONFIG_DIR`, `$CODEX_HOME`) are outside every deny projection at any value of any key. Not tolerated on load.
- **`find_full_access_args`** / **"gated full access"** - The former model in which a provider's full-access selector was operator-selectable under `strict_isolation: false`. Both selectors are now **absolutely forbidden** at every value of that key, in both argv spellings, enforced by the config validator, the flow validator's config-independent ceiling, and each adapter's argv builder.
- **`TRACE_READ_ONLY_WRITE`** / **`TRACE_READ_ONLY_GIT_DRIFT`** - Renamed to `TRACE_UNEXPECTED_WRITE` / `TRACE_GIT_CONTROL_DRIFT` when git-control drift stopped parking a task. The old names described a `read-only`-only phenomenon; both events now apply to every node class that gets a shell.
