...
- **`state.db`** - The SQLite state store that keeps task progress and run bookkeeping. Its schema version is stamped in `PRAGMA user_version` (currently `21`): a newer database is refused, an older one is migrated forward and re-stamped.
