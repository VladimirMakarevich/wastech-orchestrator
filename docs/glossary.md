...
- **`content_chapter`** - A long-form book/journey chapter editor: a read-only scout → editor → the deterministic `check_journey` prose gate (mode `ru`) → a blocking story critic → a style pass → publish, all rework routed through `fixing`.
