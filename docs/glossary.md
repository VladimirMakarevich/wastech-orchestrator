...
- **`preflight`** - Runs read-only readiness checks for the resolved env file, every allowed provider (version, auth probe, and a live no-model isolation capability smoke), the isolation policy, `gh`, Telegram, and the configured check sets, and announces each active relaxation (`read-isolation: OFF`, `git-evidence: ON`). `native Claude memory ON` is announced per run by the orchestrator, not here. It does **not** validate flows — that is `validate-flow`.
...
- **`schema_version`** - The config format version (currently `35`), bumped only when the format changes. The loader refuses a newer schema than it understands; an older or absent one loads, and `upgrade-config` brings it forward.
...
- **`skills.dynamic`** - Whether the supervisor proposes a `node → skills` map once per task (skipped when the repo ships no skills). Off by default — a fresh `install` writes `false`, so no repo pays for a layer it may not need.
...
- **`supervisor_usage`** - The measured per-run cost block in `summary.json`: calls, input, cache reads and writes, output, cost, and provider wall time — as a `total` and a `by_function` split over the jobs that ran (`observe`/`finalize`/`handoff`/`skill`), in run order. Absent entirely when the layer made no calls, because a report of zeros would read as "it ran and cost nothing".
...
- **`state.db`** - The SQLite state store that keeps task progress and run bookkeeping. Its schema version is stamped in `PRAGMA user_version` (currently `21`): a newer database is refused, and so is an **older versioned** one (`1 ≤ v < 21`) — past bumps dropped and renamed tables, so an old shape cannot be reshaped in place; delete the local `state.db` and start fresh (greenfield). Only a `0` (brand-new or pre-versioning) database is adopted, migrated with the additive columns, and stamped.
- **`tasks`** - The one row per task: status, branch and slug, the fix-cycle and subtask counters, `current_node`, `flow_fingerprint`, the park timestamps, the cleanup bookkeeping, and the frozen-bundle digests. The PR is not here — it lives in `publish_operations`.
...
- **`checks.discovery`** / **`checks.commands`** (v15), **`skills.scan_root`** / **`skills.exclude`** (v19), **`git.auto_merge_allow_per_task`** (v11), **`agents.providers.<id>.max_budget_usd`** (v14) - Other removed keys `upgrade-config` strips. They are tolerated on load, never read.
