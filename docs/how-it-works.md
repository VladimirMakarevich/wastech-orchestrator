...
The task file follows the outcome: it moves into the `done/` or `failed/` folder, while a task stopped for a human stays where it is until the operator resolves it. Then the orchestrator tidies up before it will pick up another one — under the default branch mode that means putting the working copy back on the base branch; on the modes where the operator owns the branch, it is left alone. By default the folder that holds task files is gitignored, so none of this reaches your Git history; an operator who chose at install to track task files gets the move as a small separate audit commit instead. In that tracked setup git can put a finished task's file back into the queue folder when the working copy returns to the base branch — the orchestrator recognises its own finished task by the file's content and leaves it alone rather than running it again.
