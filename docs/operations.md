...
The current schema versions are `state.db` **v15** and `config.yaml` `schema_version` **28**, and the two are handled differently because the orchestrator is **greenfield** (no production data to preserve):
...
`--timeout SECONDS` (default 30) is the cooperative grace period. On **POSIX** an overrun escalates to `SIGKILL`; on **Windows** the graceful request rides the `orchestrator.stop` sentinel because `os.kill` cannot reach an unrelated watcher. The daemon polls that sentinel during idle sleep and the FlowEngine checks the same cancellation predicate before every node, so a normal soft stop no longer waits for the entire task graph. If the grace period still expires inside one long node, the CLI automatically invokes the existing `taskkill /F /T` tree-kill seam, reaping the daemon and its active agent; recovery uses the untouched checkpoint. Until shutdown is confirmed, the PID file is retained, so a concurrent or later `--force-full` never loses its target and `restart` cannot start a duplicate watcher. In an embedding that supplies no hard-kill seam, PID/stop/children handles all remain in place and the timeout is reported without starting a replacement. A legacy PID-less state with either retained stop or children handle is ambiguous too: `restart` fails closed instead of clearing the sentinel by starting another watcher. A stop-killed agent is classified as **cancellation** (not a crash), so the router never falls back to a fresh agent and the task is parked resumably rather than reported failed.

> **Every stop route reaps the whole agent subtree — nothing is orphaned.** The recorded agent handle plus one subtree-kill primitive is used by _every_ way a run ends: `--force-full`, a `run_process` timeout, and the daemon's own exit (a foreground `Ctrl-C` or a crash reaps the agent subtree in the daemon's `finally` before the PID file is removed). The agent now leads its **own** session/group (reversing the earlier shared-group topology), so a foreground `Ctrl-C` no longer relies on group inheritance to reach it — the daemon reaps it deterministically instead. POSIX is the validated acceptance gate; a live Windows smoke has confirmed the timeout path against a real temporary parent/child process tree and its runtime-file cleanup. The owner-level end-to-end smoke with a real watcher and coding-agent CLI remains pending (see [follow-ups](backlog/follow_ups.md)).
