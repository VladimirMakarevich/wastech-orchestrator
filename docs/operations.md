...
> **The agent's instruction inputs are frozen per task (WRI-011).** Alongside the control plane, at task start the orchestrator also snapshots the **agent inputs** whose identity must stay stable — the validated task packet, the selected skill **packages** (the whole `SKILL.md` directory, not just the file), and the root repository instruction files (`AGENTS.md`, `AGENTS.override.md`, `CLAUDE.md`, when tracked) — into a private immutable bundle under `<private_home>/instruction-bundles/<task-id>/` under one `instruction_manifest_digest`. The agent reads only redacted copies published to the exchange (immutability comes from the canonical digest, not the exchange copy), and each provider is launched with **live project-instruction discovery disabled** (Codex `AGENTS.md` reading off; Claude `CLAUDE.md`/project-settings discovery off) — the frozen repository instructions are injected through a controlled instruction surface instead. **What this means operationally:** (1) editing `AGENTS.md`/`CLAUDE.md`, a task file, or a `SKILL.md` while a task runs does **not** change that task's instructions — a later node/evaluator/supervisor/resume/fallback still sees the frozen snapshot; the edit is a normal proposed diff and is adopted by the next fresh task; (2) `rerun --continue` reuses the originally frozen **agent inputs** (task packet, skill packages, repository instructions) — editing `AGENTS.md`, a task file, or a `SKILL.md` between runs is **not** adopted by a continue (only a fresh rerun picks those up); an operator continue that adopts an edited control plane merely re-binds this manifest to the new control digest, it does not re-freeze the instruction content; (3) a task whose required task/skill/instruction input contains a **known secret value** (a resolved env / denied / memory secret), or a skill package that cannot be safely represented (binary/oversized/linked/duplicate-folder), fails **before** the model runs with an actionable error rather than shipping a copy where that secret was silently redacted away or a truncated instruction. Tasks that legitimately edit repository guidance/skills still work — only the running snapshot is immutable. The agent still receives the repository's conventions (via the frozen copy); the live files also remain readable as ordinary repo source.
