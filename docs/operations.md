...
Line order is fixed: `env`; then per allowed provider its health line plus any `WARN`/`FAIL` and its isolation smoke; then `isolation`, the optional `read-isolation` / `git-evidence` notices, the command sets, `gh` (only when `git.create_pull_request` is on), `telegram`, verdict.
