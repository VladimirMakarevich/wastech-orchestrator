...
The current schema versions are `state.db` **v13** and `config.yaml` `schema_version` **27**, and the two are handled differently because the orchestrator is **greenfield** (no production data to preserve):
