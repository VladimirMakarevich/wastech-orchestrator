...
The current schema versions are `state.db` **v12** and `config.yaml` `schema_version` **17**, and the two are handled differently because the orchestrator is **greenfield** (no production data to preserve):
