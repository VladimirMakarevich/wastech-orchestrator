...
- **`--reset-fix-budget`** (with `--continue`) — for a task that ran out of fix cycles (`manual_action_required`, `max_fix_cycles`), e.g. because a transient rate-limit turned `fixing` into a no-op: reset the consecutive fix-loop counters so `fixing` runs again. The global `max_total_fix_iterations` backstop is left intact, so the loop is still guaranteed to terminate — this never grants unbounded retries. (A task stuck on the global backstop itself is not unblocked by this; the dry-run warns when there is no headroom left.)
- **`--from <node>`** (with `--continue`) — re-enter at a chosen node of the checkpoint's flow instead of the recorded stage (e.g. `--from implementation` to re-implement). The node must exist in the flow the checkpoint was recorded against; if the flow file changed since then it refuses (rather than silently restarting from the top). Jumping _back_ is always safe; jumping _forward_ relies on the inputs that node reads already being on disk.
