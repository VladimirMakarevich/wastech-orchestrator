...
The wizard detects the Git root, `origin`, base branch, and cleanliness; finds `codex`/`claude`/`gh`; proposes checks from the repo's ecosystem (`pyproject.toml` / `package.json` / `Cargo.toml` / `go.mod`); writes a validated `config.yaml` into `<repo>/.worc/`; and copies the packaged `worc/` guide to `.worc/guide/`. That installed guide now includes the task docs under `.worc/guide/tasks/`, the compact config-helper section under `.worc/guide/config/`, the flow-authoring reference under `.worc/guide/flows/` (flow schema, role prompts, prompt variables), the top-level `best-practices.md` / `decision-guide.md` / `footprint.md` pages, and the copy-ready skills gathered under `.worc/guide/skills/` — task/config authoring (`worc-task` / `worc-deco-task` / `worc-config`) plus flow authoring (`worc-flow` / `worc-flow-role` / `worc-flow-tune`). It appends two lines — `.worc/` and the sibling `.worc-io/` exchange — to the repo's tracked `.gitignore` so both the runtime home and the agent-facing exchange are ignored (`tasks/` is intentionally left tracked). (`install --reconfigure` refreshes the `.worc/guide/` docs and the `.worc/config.example.yaml` reference to the packaged version. It snapshots what it replaces first, and **bounds its own backups**: only the newest three of each `config.yaml.bak-*` / `flows.bak-*` / `tools.bak-*` are kept, so repeated reconfigures no longer accumulate. Your own `state.db*.bak*` files are never touched.) To keep `.worc/flows/` itself git-tracked instead — so custom flows get history and go through a PR — see [How-To → Track your operator flows in git](how-to.md#4-track-your-operator-flows-worcflows-in-git). It never installs or authorizes the CLIs; it reports what is missing and auto-runs `preflight` at the end (a failed preflight keeps the config but exits non-zero with instructions).
...
> **Linux/WSL2 operators — new runtime dependency (WRI-002).** A **workspace-write Claude** node now runs its `Bash` tool inside Claude's OS Bash sandbox, which on Linux/WSL2 requires **`bubblewrap` (`bwrap`) + `socat`** on `PATH`. Under the default `strict_isolation: true`, a workspace-write Claude run on a host missing them now **fails closed** (`preflight` reports `isolation: FAIL`; a run ends `capability_unavailable` → fallback or `manual_action_required`) rather than running Bash unsandboxed. This is a change from earlier versions, which ran it unsandboxed. **Install both before upgrading** (`apt install bubblewrap socat` or your distro's equivalent), or set that provider `read-only`, or opt out with `strict_isolation: false` (accepting unsandboxed Bash). macOS (Seatbelt) and native Windows (restricted no-Bash mode) are unaffected.
...
The current schema versions are `state.db` **v21** and `config.yaml` `schema_version` **35**, and the two are handled differently because the orchestrator is **greenfield** (no production data to preserve):
...
- **`state.db`** does **not** migrate across versions. A brand-new (or pre-versioning) database is created at the current shape; a database stamped an **older** version (`1 ≤ v < 21`) is **refused fail-closed** — several past bumps dropped/renamed tables, and the store only ever adds columns, so an old shape cannot be reshaped in place. Recovery is to delete the local `state.db` and start a fresh workspace (greenfield: there is nothing to preserve). Do this **between tasks**, never with a task in flight. (v16 added additive normalized-token-usage columns: a per-run delta on `provider_attempts` and a running-cumulative snapshot on the lineage tables. v19 added `provider_attempts.task_id` — so a cost/usage roll-up sums by task without a `node_runs` join — and made `node_run_id` nullable, so the supervisor layer records its own provider calls with `node_run_id` NULL. v20 added `provider_attempts.supervisor_function` (`observe`/`finalize`/`handoff`/`skill`, NULL for a graph node), so that spend splits per phase in one `GROUP BY`. v21 added `tasks.blocked_until` — the provider-reported wake instant of a parked task, see [provider outage behavior](#provider-outage-behavior).)
...
Line order is fixed: `env`, one per allowed provider, `isolation`, the optional `read-isolation` / `git-evidence` notices, the command sets, `gh` (only when `git.create_pull_request` is on), `telegram`, verdict.
...
- `isolation: OK (enforced)` — every provider in `agents.allowed` passed the offline isolation check (Codex sandbox, Claude permission mode + tool/sandbox policy) **and** `security.strict_isolation: true` (the default) is active, meaning a failed check would abort the run rather than silently downgrade isolation.
- `isolation: OK (strict_isolation=false)` — the check still passed, but the operator has set `security.strict_isolation: false`, opting in to full-access provider modes (e.g. `danger-full-access` or `bypassPermissions`). The operator owns the risk; the gate will not abort a run if isolation cannot be enforced. On a host without a supported Claude Bash sandbox this is also where a workspace-write Claude node keeps **unsandboxed** Bash — read-isolation is disabled and the operator owns it.
- `git-evidence: ON` — `security.allow_git_evidence: true` is in effect, so a flow node declaring `git_evidence: true` gets the read-only git verbs (it stays `read-only` on disk). Off is a **grant** switch, not a kill switch: a Codex `read-only` node reads git history either way, so with it off the same flow has different reach per provider. See [configuration.md](configuration.md#allow_git_evidence-the-read-only-git-evidence-grant).
- `native Claude memory ON` — `agents.providers.claude.allow_native_memory: true` is in effect, so Claude's own auto-memory store under its config home is writable and persists across tasks **outside** the orchestrator's redaction net and audit. This is the only switch that lifts the write side of that deny; `disable_read_isolation` lifts only the read side.
- `read-isolation: OFF (<why>)` — `security.disable_read_isolation: true` is in effect, or it is forced by `strict_isolation: false`. **This line is on a fresh install:** `disable_read_isolation` defaults to `true`, so read-isolation is off out of the box and you turn it **on** by setting the key `false` (`strict_isolation` is still the master switch and always wins toward relaxation, so `strict_isolation: false` forces it off again). Providers run with **native project-instruction/config discovery** restored (Claude reloads `CLAUDE.md` + project settings/hooks/MCP/skills; Codex reloads the user `config.toml` + project `.codex` config/hooks/rules) and the private read-deny projection (`.worc`/env-file/provider homes/frozen bundles) lifted — deliberately accepting the reduced isolation (a re-enabled hook can run arbitrary commands). The **write** side (exchange/`.git`/`tasks/` write-deny, commit/staging gates, PR control) and the `denied_read_paths` blacklist stay in force. The run log emits the same warning at task start, so this is never a silent weakening.
- `isolation: FAIL` lists the offending provider/setting. With `security.strict_isolation: true` (the default) a run would **fail preflight** before any branch is created rather than silently downgrading isolation — fix the config (don't weaken the sandbox/permission profile) and re-run. The Claude check is **host-aware (WRI-002)**: a configured **workspace-write** Claude on a Linux/WSL2 host missing the Bash-sandbox dependencies (`bubblewrap`+`socat`) fails here — install both, or set that provider `read-only`. **Native Windows** does not fail: a workspace-write Claude node there runs in a documented restricted mode with **no Bash tool** (Edit/Write still work), so read-isolation holds via tool policy without an OS sandbox. If a Claude node genuinely needs a shell on native Windows, route it to Codex (which has a Windows sandbox) or opt out with `strict_isolation: false`. A required-but-unavailable sandbox that surfaces at run time is the deterministic pre-model `capability_unavailable` class: the router may fall over only to a provider that can itself isolate the node on that host, else the task ends in `manual_action_required`.
- **`<codex>: isolation smoke OK — …`** (WRI-006 / H7) — beyond the offline check above, `worc preflight` runs a **live, no-model** `codex sandbox` capability smoke of the generated `worc` profile on this host: it stands up a throwaway fixture and proves the private home is denied (direct + shell + through a workspace symlink alias), a repo read is allowed (the positive control), a repo write matches the profile, the exchange is read-only, and records the effective `codex mcp list` inventory. `OK` means the sandbox is genuinely OS-enforcing here. A **`WARN — isolation smoke: … sandbox could not run …`** means the host cannot demonstrate the sandbox (old Codex CLI, missing sandbox helper) — the `capability_unavailable` class; it fails preflight only when Codex has no fallback provider. A **`FAIL — isolation smoke: … not enforcing …`** means a denied path was actually readable/writable — a `configuration_error` security result that fails preflight unconditionally. The smoke runs only under `strict_isolation: true` and only for a healthy Codex; a Claude-only deployment never spawns it. (Claude's Bash sandbox has no credential-free runner, so its real OS-enforcement is proven by an operator-run authenticated smoke, not here — see the note below.)
...
> **The agent's task/skill inputs are frozen per task; repository instructions are read live and freely editable (WRI-011, amended by VF-5 and VF-20).** At task start the orchestrator snapshots the **agent inputs** whose identity must stay stable — the validated task packet and the selected skill **packages** (the whole `SKILL.md` directory, not just the file) — into a private immutable bundle under `<private_home>/runs/instruction-bundles/<task-id>/` under one `instruction_manifest_digest`; the agent reads only redacted exchange copies of those. **Repository instructions (`AGENTS.md`, `AGENTS.override.md`, `CLAUDE.md`) are handled differently after VF-5:** they are **not** injected and provider-native discovery is **not** disabled — each agent reads the repo's live root files itself (Codex via native `AGENTS.md` discovery; Claude via its Read tool, since its `--setting-sources ""` lockdown also turns off native `CLAUDE.md` auto-load). These files are **ordinary, editable repository content (VF-20)**: a workspace-write attempt may change `AGENTS.md`/`CLAUDE.md` or anything under `.agents/rules/` — the change lands as a normal proposed diff, never blocked. A per-source freeze of these files at task start still folds into the `instruction_manifest_digest` for audit (the record of what the agent read), and when a run's diff touches a governance path the orchestrator **loudly notifies the operator** — a `WARNING` in the run log/console, a `## Governance files changed` section in the PR/commit summary, a `governance_changed` field on the completed-ledger record, and a `governance=` mark on the Telegram completion message — so a run that edits its own rules is always visible, never silent. **What this means operationally:** (1) editing a task file or a `SKILL.md` while a task runs does **not** change that task's task/skill inputs — a later node/evaluator/supervisor/resume/fallback still sees the frozen snapshot; the edit is a normal proposed diff adopted by the next fresh task; (2) editing `AGENTS.md`/`CLAUDE.md` **between** runs is picked up automatically on the next run (the agent reads them live) — there is no instruction freeze to re-adopt; a task may also edit them (and `.agents/rules/**`) **during** a run as ordinary work — the orchestrator reports the change to the operator rather than blocking it (VF-20), and an operator who wants an always-ask step on these paths can add them to `security.protected_paths`; (3) a task whose required task/skill input contains a **known secret value** (a resolved env / denied / memory secret), or a skill package that cannot be safely represented (binary/oversized/linked/duplicate-folder), fails **before** the model runs with an actionable error. Tasks whose deliverable is editing repository guidance/rules/skills work end to end — the edit lands as an ordinary proposed diff, the operator is notified of any governance change, and the live files remain readable repo source.

> **Git control state is orchestrator-owned per task (WRI-009).** Every orchestrator `git` command runs with a private empty `core.hooksPath` and editor/pager/signing/fsmonitor/filter neutralization, so a **target-repository hook or an agent-set `.gitattributes`/config filter can never execute** inside an orchestrator Git subprocess (an untrusted repo-local clean/smudge/filter/diff driver — or `core.sshCommand`/`credential.helper` — stops the run in `manual_action_required`; operator **global** git config, e.g. a global `git lfs install`, stays trusted). Around every workspace-write attempt the orchestrator fingerprints Git control state (index, HEAD, the task ref, repo-local config, hooks, and merge/rebase/bisect markers) and re-checks it after the provider tree is proven quiescent — if the agent mutated the index/refs/config/hooks (e.g. `git add -f .worc-io/…`, a stray commit, an installed hook) the task ends **`manual_action_required`**, never a fallback. **One bounded exception:** on a `read-only` node holding the [git-evidence grant](configuration.md#allow_git_evidence-the-read-only-git-evidence-grant), drift **warns and continues** instead of parking — that node was handed the read-only verbs deliberately and cannot mutate the tree, so a benign side effect of `git log`/`status` must not kill an expensive run. Every other permission profile still parks. The event is visible: a `WARNING` in the run log carrying the redacted aspect summary, and a third synthetic ⚠️ Telegram trace label alongside "rework budget exhausted" and "read-only node wrote". Before **every** commit the complete staged set is proven to contain only that operation's files (a bare `git commit` commits the whole index, so the scoped `git add` is never trusted alone), and the audit commit refuses a `tasks/<id>.md` whose content no longer matches the task the run was authorized from. **What this means operationally:** (1) `.worc/`/`.worc-io/` and another task's lifecycle files can never ride a commit, even force-staged; (2) an operator who pre-stages files in the shared clone before a run is asked to unstage/commit them first (the run refuses rather than sweeping them in — unstaged edits are left untouched); (3) git **≥ 2.9** is required under strict isolation (older git ignores `core.hooksPath`), enforced at `run`/`watch`/`rerun` startup. The provider-side read-only-gitdir grant / write-deny projection lands with the Codex/Claude isolation tasks.
...
To bound the footprint going forward instead of cleaning periodically, lower the per-run artifact retention with [`logging.artifacts`](configuration.md#logging) (`minimal` / `standard` / `full`).
...
Separately from the logs, each task leaves private per-task runtime state under `.worc/runs/`: its frozen `control-bundles/` and `instruction-bundles/`, and its `exchange-seals/`. A **successful** task evicts its own subtree automatically ([`logging.clean_runs_on_success`](configuration.md#logging), default `true`); failed, parked, and `manual_action_required` tasks keep theirs so you can debug them, and `exchange-quarantine/` — tainted evidence — is never removed automatically at all.
...
There is one canonical layout. Everything the orchestrator generates lives under a single gitignored `<repo>/.worc/` home — `config.yaml` (plus the commented `config.example.yaml` reference), `.env` / `.env.example`, `guide/` (task docs + config helper + flow-authoring docs + copy-ready skills + `footprint.md`), `flows/` (editable flow + role-prompt copies), `tools/` (delivered `tool`-node executables, e.g. `check_chapter` / `check_length`), `state.db` (+ `-wal`/`-shm`), `orchestrator.pid` (plus the `orchestrator.stop` sentinel and `orchestrator.children` handles while a daemon runs), `logs/` (plan, diffs, stage logs, `summary.json`, validation reports, the ledger, the daemon logs), `memory/`, `security-reports/`, `workspace/`, `runs/` (the per-task frozen bundles, sealed exchanges, and quarantine — see [`runs clean`](#reclaiming-per-task-runtime-state-runs-clean)), and the `tasks/rejected` quarantine. The one sibling runtime root outside `.worc/` is the gitignored `<repo>/.worc-io/` exchange (the redacted, agent-facing current-task surface — WRI-001). The **only** things not gitignored are the `tasks/` lifecycle dirs (`preparing`/`pending`/`done`/`failed`), which live at the **repo root** and are **git-tracked**: the task file and its `<id>.summary.md` (in `done/` or `failed/`) are the audit trail. (`preparing/` is the staging area the watch scanner never reads — `promote` moves a finished file into `pending/`.) `install` appends two lines — `.worc/` and `.worc-io/` — to the repo's tracked `.gitignore`; `tasks/` is intentionally not ignored.
...
By default the pipeline (the packaged `implementation` flow) runs `refinement → planning → implementation → testing → review → fixing(loop) → documentation → publish`. The whole-task **summary** is not a graph node — the [supervisor layer](configuration.md#supervisor) writes it at task close (it becomes the PR body), so it cannot be disabled per task; removing that layer is the config switch `supervisor.enabled: false`, after which the deterministic report becomes the body. A **task** can disable a node that adds no value for it — convenient for debugging/testing and quick one-off runs without authoring a separate flow.
...
logs/
  ...
  <task-id>/
    ...
    prompt-audit/                 # who-ran-what + redacted prompt timeline (only when prompt_audit)
...
Tracked `tasks/` at the repo root is the **expected**, git-tracked audit trail — that is where live task files belong. The runtime home `.worc/` and the agent-facing exchange `.worc-io/` are gitignored by `install` (both lines appended to `.gitignore`), so their contents never ride a commit.
