...
`watch` respects `orchestrator.auto_mode.enabled`: off (default) it processes/resumes one task and returns the working copy to `repo.base_branch` (unless the branch mode / `repo.checkout_base_on_cleanup` keeps it on the working branch); on, it processes pending tasks sequentially, running terminal cleanup between them. A `manual_action_required` outcome always blocks automatic continuation. Exit code: `0` done, `1` failed, `2` manual_action_required, `3` paused (a task soft-parked on a provider outage — see [provider outage behavior](#provider-outage-behavior)).
