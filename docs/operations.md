...
- **Busy, `--force`** (or typed `YES`) → **soft** stop: the daemon finishes the current step, then exits between ticks (the graceful stop, now gated). Works on every platform. Soft never interrupts a running agent — to stop _now_, use `--force-full`.
...
`--timeout SECONDS` (default 30) bounds how long a soft stop waits for the daemon to confirm shutdown. On **POSIX** it then escalates to `SIGKILL`; on **Windows** a soft stop cannot signal the daemon at all, so it rides the `orchestrator.stop` sentinel file the daemon polls between ticks — and the daemon now re-checks that file about once a second during the poll sleep (not once per `poll_interval`), so a soft `stop`/`down` confirms well within the timeout even with a large `poll_interval` (default 300). If the timeout is still reached on Windows (the daemon wedged inside a long tick), `stop` drops the PID file but **leaves the stop-file in place** — a merely-busy daemon then stops itself the moment its current tick ends — and prints the concrete `taskkill /F /PID <pid>` command for a truly-stuck process rather than a vague "use Task Manager". A hard stop never destroys committed work — recovery is the same `resume()` that reconciles any abrupt exit. Because a stop-killed agent is flagged as a **cancellation** (not a crash), the router never falls back to a fresh agent and the task is parked resumable rather than reported failed.

> **Every stop route reaps the whole agent subtree — nothing is orphaned.** The recorded agent handle plus one subtree-kill primitive is used by _every_ way a run ends: `--force-full`, a `run_process` timeout, and the daemon's own exit (a foreground `Ctrl-C` or a crash reaps the agent subtree in the daemon's `finally` before the PID file is removed). The agent now leads its **own** session/group (reversing the earlier shared-group topology), so a foreground `Ctrl-C` no longer relies on group inheritance to reach it — the daemon reaps it deterministically instead. POSIX is the validated acceptance gate; the Windows hard stop is implemented but **validation-pending** (see [follow-ups](backlog/follow_ups.md)) until an owner runs the real Windows smoke.
