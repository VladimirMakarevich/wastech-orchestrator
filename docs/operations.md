...
> **The agent's task/skill inputs are frozen per task; repository instructions are read live but write-locked (WRI-011, amended by VF-5).** At task start the orchestrator snapshots the **agent inputs** whose identity must stay stable — the validated task packet and the selected skill **packages** (the whole `SKILL.md` directory, not just the file) — into a private immutable bundle under `<private_home>/instruction-bundles/<task-id>/` under one `instruction_manifest_digest`; the agent reads only redacted exchange copies of those. **Repository instructions (`AGENTS.md`, `AGENTS.override.md`, `CLAUDE.md`) are handled differently after VF-5:** they are **not** injected and provider-native discovery is **not** disabled — each agent reads the repo's live root files itself (Codex via native `AGENTS.md` discovery; Claude via its Read tool, since its `--setting-sources ""` lockdown also turns off native `CLAUDE.md` auto-load), and for a workspace-write attempt those tracked files are **write-denied for the run** (readable but immutable), so what the agent reads is reproducible on every node/resume/fallback. A per-source freeze of these files still folds into the `instruction_manifest_digest` for audit. **What this means operationally:** (1) editing a task file or a `SKILL.md` while a task runs does **not** change that task's task/skill inputs — a later node/evaluator/supervisor/resume/fallback still sees the frozen snapshot; the edit is a normal proposed diff adopted by the next fresh task; (2) editing `AGENTS.md`/`CLAUDE.md` **between** runs is picked up automatically on the next run (the agent reads them live) — there is no instruction freeze to re-adopt; **during** a run those files are write-locked, so an agent cannot rewrite its own instructions mid-task; (3) a task whose required task/skill input contains a **known secret value** (a resolved env / denied / memory secret), or a skill package that cannot be safely represented (binary/oversized/linked/duplicate-folder), fails **before** the model runs with an actionable error. Tasks that legitimately edit repository guidance/skills still work — the edit lands as an ordinary proposed diff, and the live files remain readable repo source.
