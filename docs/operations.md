...
The installed guide bundle also ships with the package (packaged source dir `worc/`, copied to `.worc/guide/`), so an upgrade brings newer docs than your already-installed copy. That bundle includes the task docs, the copy-ready skills (`worc-task`, `worc-deco-task`, `worc-config`) gathered under `.worc/guide/skills/`, and the config-helper subtree under `.worc/guide/config/`. Refresh the installed copy (under `.worc/guide/`) with **`upgrade-docs`**:
