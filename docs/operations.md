...
| **Terminal exchange cleanup blocked** — the snapshot sealed but the active `.worc-io/<task-id>/` could not be removed (a Windows lock/read-only), or the provider tree was not proven quiescent. Later launches are blocked (`exchange_active_unsafe`). | Clear the lock / remove the stale `.worc-io/<task-id>/` by hand (the verified `seal-*` snapshot is safe), then re-run. |
