...
- **`--from <node>`** (with `--continue`) — re-enter at a chosen node of the checkpoint's flow instead of the recorded stage (e.g. `--from implementation` to re-implement). The node only needs to exist in the flow **currently on disk** — `--from` is an explicit, operator-driven override, so it resumes through any drift since the checkpoint rather than refusing (a `note:` is printed when the flow did change; the checkpoint's fingerprint is rebaselined to the current flow's so the resume actually lands at the chosen node instead of restarting from the top). Jumping _back_ is always safe; jumping _forward_ relies on the inputs that node reads already being on disk.
