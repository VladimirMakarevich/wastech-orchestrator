# Исследования от сторонней системы по внедрению линтера для python проекта

> Не всё можно применить как есть ля аднного проекта, поэтому нужно всю дальнейшую информацию оценивать скептически.
