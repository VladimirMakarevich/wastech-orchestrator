...
- **Изменение (РЕШЕНО 2026-07-03 — вариант (b), НЕ ослаблять):** скан оставлен строгим и единообразным по **всем** front-matter полям (включая `title`/`contacts` — allowlist display-полей НЕ вводился). Правило «front-matter значения — только простой текст (без `` ` `` `;` `|` `$(` и ведущего `-`; сниппеты — в тело)» задокументировано во всех гайдах ([docs/task-authoring.md](../task-authoring.md#front-matter-values-are-plain-text) §«Front-matter values are plain text», `packaged/guide/README.md`, `packaged/guide/tasks/skills/worc-task/SKILL.md`, зеркала в mdlint `.worc/guide/`). Это и есть F5b (документируем, а не ослабляем). Плюс сохранена часть (b): `finding.detail` (поле+причина) выводится в консольной строке reject (`PipelineResult.validation_detail` → CLI), не только `reason`. **Security-ревью не требуется** — защита не ослаблена.
- **Тесты (реализовано):** unit — `title`/`contacts` с бэктиком/`;`/`|`/ведущим `-` по-прежнему reject (скан единообразен, без исключений); consumer-путь — `validation_detail` (поле+причина) прокинут в reject-строку. Осторожно: защита НЕ ослаблена.
- **Docs/config (реализовано):** во всех гайдах задокументировано правило «front-matter значения — только простой текст, сниппеты — в тело» (task-authoring §«Front-matter values are plain text» + packaged guide + mdlint зеркала).
- **Критерий готовности:** правило зафиксировано и описано во всех гайдах; при любом reject оператор видит поле+причину на консоли. **Security-ревью не требуется** (скан не ослаблен).
...
- **F5b (docstring/контракт) — ✅ РЕАЛИЗОВАНО 2026-07-03:** решено НЕ ослаблять argv-скан (F5a вариант b), поэтому правило «front-matter значения (`title` и др.) — только простой текст, без `` ` `` `;` `|` `$(` и ведущего `-`; сниппеты — в тело» добавлено в контракт авторинга во всех гайдах: mdlint `.worc/guide/{README.md, tasks/skills/worc-task/SKILL.md}` **и** worc-source `packaged/guide/{README.md, tasks/skills/worc-task/SKILL.md}` + `docs/task-authoring.md` §«Front-matter values are plain text».
...
- **security-ревью:** более НЕ требуется для F5a — решением 2026-07-03 скан оставлен строгим (правило задокументировано вместо ослабления), защита не изменена.
