...
| **F37 Claude native-memory deny** (`<config_dir>/projects/<repo>/memory`) | Denied — unless `allow_native_memory: true` | **Deny lifted** (read_isolation_off drops it) | **via read_isolation_off** (OR the per-provider `allow_native_memory` opt-in) | [providers/claude.py:659-660](../../../src/wastech_orchestrator/providers/claude.py) | claude |
