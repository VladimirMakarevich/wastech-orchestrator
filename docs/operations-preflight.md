...
Line order is fixed: `env`; then per allowed provider its health line, its `<provider>-binary:` line, any `WARN`/`FAIL`, and its isolation smoke; then `isolation`, the optional `read-isolation`, `isolation-floor` (printed only when a provider has no floor), the `advanced-mode` and `git-evidence` notices, the environment lines (`allowed-environment`, `extra-environment`, `assigned-paths`), `gh-repo-pin`, the command sets, `gh` (only when `git.create_pull_request` is on), `telegram`, verdict.
...
- `isolation: OK (strict_isolation=false)` — the check still passed, and `security.strict_isolation: false` is in effect; a separate `advanced-mode: ON (security.strict_isolation=false) — …` line follows the floor lines. **This is what a fresh `install` writes**, so expect these lines rather than the one above on a new workspace. It is one line by design and it points at [the advanced mode](configuration-agents.md#the-advanced-mode-strict_isolation-false) rather than reciting the whole floor into every report — read that section before you keep the key. It does **not** mean full access is available: the two provider full-access selectors are refused at every value of the key.
- `isolation-floor: NONE (<provider>: <cause>)` — printed once per allowed provider that has **no OS-enforced write floor** for this run; a host with a floor prints no such line at all. Advisory, never a refusal: the run continues. It is a status line in the shape of its neighbours — subject, verdict, cause — and the cause is all it says; what the missing floor costs (`.git` and `.worc` writable, so control-plane tamper detection, exchange quarantine and `state.db` integrity are unenforced) is stated once in the shipped `.worc/guide/config/security.md`. The run log prints the same text at task start (`isolation floor NONE (…)`). Two different things print it, and the cause says which. A **host** that cannot sandbox — native Windows (`claude: native Windows — no supported OS sandbox for the agent shell`), or Linux/WSL2 missing `bubblewrap` + `socat` (`claude: Linux/WSL2 — install bubblewrap+socat to get the floor back`) — at either value of `strict_isolation`, and there the line carries the remedy. And **`strict_isolation: false` itself, on every host** (`claude: strict_isolation=false — no OS sandbox on any host`), because the advanced mode raises no OS sandbox for Claude anywhere; that arm carries no remedy on purpose, since the remedy is `strict_isolation: true`, an operator decision rather than a missing dependency. So a macOS machine that reported `isolation-floor: OK` under `true` reports `NONE` under `false` — expect this line on a fresh `install`. **Codex is not symmetric**: it always gets a generated permission profile, so its floor does not move with the key; on native Windows its own line says only that the answer cannot be classified offline and points at the live smoke (``codex: native Windows — sandbox not classifiable offline, `worc preflight` probes it``). Under `strict_isolation: true` a refusal still happens, just later and narrower — the adapter raises `capability_unavailable` for the attempt that actually needs a sandboxed shell (native Windows drops the shell instead), which is a per-node verdict a fallback provider can cover.
...
- It runs the full fatal validator (graph + security ceiling + the config-aware layer, including the `.worc/tools/` tool-name check), so it catches a disallowed provider/model/reasoning, an over-ceiling node, or a `tool:` node naming a tool you have not delivered — the same failures a task would hit at dispatch. It also emits non-fatal `WARN` lines, each carrying its own consequence: a role prompt referencing an unknown `{token}` (it renders verbatim to the agent), and a `tool` node whose `fail`/`rework` edge declares a fix budget — that number holds as written only while the tool emits a top-level `findings` array; without one it is the count of identical failures that ends the loop (see [flow authoring → Custom tool nodes](flow-authoring.md#custom-tool-nodes)).
...
flow my_flow: WARN — node 'md-check': the declared fix budget of 6 holds as written only while the tool emits a top-level 'findings' array; without one it is the number of identical failures that ends the loop, not the fix rounds it plans
```
