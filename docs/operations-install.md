...
The current schema versions are `state.db` **v28** and `config.yaml` `schema_version` **41**, and the two are handled differently because the orchestrator is **greenfield** (no production data to preserve):
...
The installed guide bundle also ships with the package (packaged source dir `worc/`, copied to `.worc/guide/`), so an upgrade brings newer docs than your already-installed copy. That bundle includes the task docs, the copy-ready skills gathered under `.worc/guide/skills/` — task/config authoring (`worc-task`, `worc-deco-task`, `worc-config`), flow authoring (`worc-flow`, `worc-flow-role`, `worc-flow-tune`), and the unattended queue runner `worc-night-run` (see [Unattended night runs](operations-running.md#unattended-night-runs-the-worc-night-run-skill)) — and the `config/` and `flows/` subtrees. Refresh the installed copy (under `.worc/guide/`) with **`upgrade-docs`**:
