...
- **`config.yaml`** — `<artifacts_root>/config.yaml`; **schema v11**; discovered by walking up to the Git root; owner [B04](./blocks/B04-install-registry-and-config-discovery.md)/[B05](./blocks/B05-configuration.md).
...
- **Decorative configuration / flow fields.** Several parsed config and flow-YAML fields are never consumed at runtime (the flow `decomposition.gate`, `commit_each_subtask`/`commit_per_subtask`, `min_size_signal`, and the research/audit flows' `global_revision_iterations` budget). These are documented as found and recorded in [the audit](../backlog/2026-06-21-audit.md).
