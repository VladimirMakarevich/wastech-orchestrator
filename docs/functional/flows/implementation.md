...
(Node fields verified against [implementation.yaml:30-67](../../../src/wastech_orchestrator/core/flow/packaged/implementation.yaml#L30); the `testing` checks node is just `{id, kind, checker}` — the former `discovery` field was removed with the checks-monorepo change.)

## Loops and budgets

Two fix loops feed back into `fixing`, plus the global cap ([implementation.yaml:69-82](../../../src/wastech_orchestrator/core/flow/packaged/implementation.yaml#L69)):
