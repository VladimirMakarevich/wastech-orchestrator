...
3. **`INVALID_FIELD_TYPE`** — `title` must be a string; `branch_name`/`task_type` must be string-or-null; `auto_merge`/`prompt_audit`/`decomposition` must be boolean-or-null; `contacts`/`depends_on`/`subtasks` must be lists of strings (a bare string/bytes is rejected where a list is expected). `priority` is intentionally **not** type-checked here — it is fail-open and folds to `mid`.
