...
`ALLOWED_TASK_KEYS` is exactly `{id, title, task_type, branch_name, auto_merge, prompt_audit, decomposition, contacts, depends_on, priority, queue, subtasks, nodes}` ([model.py](../../../src/wastech_orchestrator/task/model.py)); `REQUIRED_TASK_FIELDS` is `{id, title}`. A "clean" task carries identity/dispatch only, plus bounded operational knobs: a full branch-name override, merge dependencies, scheduling priority, a multi-instance queue tag, operator-authored subtask references, per-node disable toggles, prompt audit, auto-merge, and the decomposition gate. Provider, model, reasoning, and the old `refined` flag are **not** task fields; they live on the flow node or are derived, so they fail-closed as `unknown_top_level_field`.

`task_type` is the dispatch key that selects the flow (`implementation` / `deep_research` / `security_audit` / an operator flow); `None` defers to the registry default. It never affects _how_ a node runs. `branch_name` is a full task-branch override; when absent, the Git Manager auto-generates `repo.branch_prefix/<epoch>-<task-id>-<slug>` (capped at 50 chars). An override over 50 characters is **not** a hard reject — the gate logs a warning and clears it so auto-generation takes over (a fixable mistake should not abort the run); the 255-byte Git ceiling stays a hard reject. `auto_merge`, `prompt_audit`, and `decomposition` are tri-state (`True`/`False`/`None`): the task value wins outright, and `None` defers to the global config. `decomposition` only flips the _gate_ (whether a split is permitted, default `agents.decomposition.enabled`); the flow's `decomposition:` block + the planning node's proposal still decide whether a split happens, and an operator `subtasks:` manifest ignores the gate. `priority` (`low`/`mid`/`high`, default `mid`) orders the scheduler's eligibility queue and is the one **fail-open** field: unlike every other constrained field, an unrecognised value or wrong type folds to `mid` via `normalize_priority` rather than rejecting — a scheduling hint must never block an otherwise-valid task. The scheduler reads it from the lightweight front-matter scan (`priority_rank`), so ordering stays a cheap read; full normalization onto `NormalizedTask` happens here in the gate. `queue` (default `"default"`) is the multi-instance partition tag — `watch` only picks a pending task when `task.queue` equals the instance's `orchestrator.queue` selector (plain string equality). Unlike `priority` it is **fail-closed**: a non-string value or an empty/whitespace string rejects the task (`INVALID_FIELD_TYPE`). The scheduler reads it from the same lightweight scan (folding a missing/malformed value to the default there) and applies the equality filter in `watch_once` before ranking, so out-of-queue tasks are invisible to the instance.
...
3. **`INVALID_FIELD_TYPE`** — `title` must be a string; `branch_name`/`task_type` must be string-or-null; `queue` (when present) must be a non-empty string; `auto_merge`/`prompt_audit`/`decomposition` must be boolean-or-null; `contacts`/`depends_on`/`subtasks` must be lists of strings (a bare string/bytes is rejected where a list is expected). `priority` is intentionally **not** type-checked here — it is fail-open and folds to `mid`.
...
`write_normalized` writes `task.normalized.json` under `logs/<task-id>/`, serializing `id`, `title`, `description`, `task_type`, `branch_name`, `auto_merge`, `prompt_audit`, `contacts`, `depends_on`, `priority`, `subtasks`, and `nodes`. `load_normalized` reads it back on resume; legacy manifests written before a field existed load with its default (`nodes` → `{}`, tri-state fields/`branch_name` → `None`, `priority` → `mid`). Persisting `branch_name` keeps a custom branch override from silently reverting to the default branch naming policy across a crash-resume before publish. `write_validation_report` writes `validation_report.json` (`task_id`, `passed`, `reason`, `detail`, `completeness`) for both accepted and rejected tasks.
