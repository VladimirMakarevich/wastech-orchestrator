...
`gate_on` is resolved by the orchestrator: `_decomposition_gate_on(task)` returns the per-task `decomposition` value when set (tri-state, task-wins, mirroring `_prompt_audit_on`/`_auto_merge_on`) and otherwise `config.agents.decomposition.enabled` ([orchestrator.py:1801](../../../src/wastech_orchestrator/core/orchestrator.py#L1801)). It only flips whether a split is permitted — `decide_decomposition` still owns the decision; an operator `subtasks:` manifest bypasses the gate (`gate_on=True`). `max_subtasks` is `config.agents.decomposition.max_subtasks` ([orchestrator.py:1161](../../../src/wastech_orchestrator/core/orchestrator.py#L1161), config field at [config/schema.py:121](../../../src/wastech_orchestrator/config/schema.py#L121)). The contract (`decompose`/`subtasks`) is read off the proposed_by node's `structured_output` by `read_decomposition` ([postprocess.py:90](../../../src/wastech_orchestrator/core/flow/postprocess.py#L90)) — the engine itself never inspects these fields.
