...
The Router takes a `before` snapshot via `snapshot.capture()` (if a hook is supplied) at the top of `run_stage` ([router.py:184](../../../src/wastech_orchestrator/routing/router.py#L184)). When it decides to fall back after an infra failure, and both a hook and a `before` exist, it calls `snapshot.partial_change_since(before)` and stores the result in `partial` ([router.py:322-323](../../../src/wastech_orchestrator/routing/router.py#L322)). The next (fallback) attempt's request then gets the partial's `diff_path` substituted via `_build_request` — replacing any prior cumulative diff — while `permission_profile`, prompt, context paths, output schema, permissions, and network flag are left untouched ([router.py:371-393](../../../src/wastech_orchestrator/routing/router.py#L371), `test_infra_failure_after_changes_hands_diff_to_fallback`). Because a fallback attempt is always **cross-provider** (the substitute is a different CLI), `_build_request` also resets provider-specific request fields: node-pinned `model`, most `reasoning` values, `extra_args`, and `session_id`. A model id is provider-specific (codex `gpt-5.5` is not a Claude model), CLI extra flags are provider-specific, and a durable session id belongs to one provider, so the substitute must re-resolve its own config/defaults (`test_cross_provider_fallback_drops_provider_specific_request_fields`). The one reasoning exception is Codex `minimal` → Claude `low`, preserving the "minimum effort" intent because Claude has no `minimal` level (`test_codex_minimal_reasoning_fallback_maps_to_claude_low`). There is deliberately **no** rollback/restore method on the `SnapshotHook` Protocol — its absence is the no-auto-rollback guarantee; files changed by the failed primary are never reverted ([snapshots.py:43-56](../../../src/wastech_orchestrator/routing/snapshots.py#L43)). The `PartialChange.note` is carried for the caller to weave into the fallback's prompt context; the Router itself only uses `diff_path` ([snapshots.py:29-40](../../../src/wastech_orchestrator/routing/snapshots.py#L29)).
