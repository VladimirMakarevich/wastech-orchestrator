...
2. **Rendered prompt — gated on the artifact wiring.** When `services.register_artifact` is `None` the function returns before writing any file ([observability.py:46-48](../../../src/wastech_orchestrator/core/flow/observability.py#L46)). Otherwise `write_rendered_prompt` writes the redacted prompt to `logs/<task-id>/stages/<node_id>/[sub-<NN>/]rendered-prompt.md` and registers it ([observability.py:101](../../../src/wastech_orchestrator/core/flow/observability.py#L101)).
