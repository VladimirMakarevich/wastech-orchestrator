...
- **Configuration model/validation** — [B05](./B05-configuration.md); **config discovery** — [B04](./B04-install-registry-and-config-discovery.md).
...
Load config, (if PR) `require_gh`, `build_orchestrator` (artifacts under `worc_home_for`), `run_task`, print status + PR ([cli.py:716-732](../../../src/wastech_orchestrator/cli.py#L716)).
...
`run_preflight`: `provider.preflight()` for resolved providers + `check_isolation` + check diagnostics + telegram-preflight → readiness + lines ([cli.py:924-980](../../../src/wastech_orchestrator/cli.py#L924)).

### `rerun` / `finalize`

Plan (`plan_rerun`/`plan_finalize`) → if `--dry-run` print plan; otherwise confirm and `rerun_task`/`continue_task` / `finalize_task` ([cli.py:771-921](../../../src/wastech_orchestrator/cli.py#L771)). Fails if the watch daemon is alive.

### `telegram-test`

`build_notifier` + `ask_human` — a real round-trip without task processing ([cli.py:983-1014](../../../src/wastech_orchestrator/cli.py#L983)).

## Checks and Constraints

- Subcommand is required; numeric flags (`--heartbeat-seconds`/`--poll-seconds`/`--timeout`) must be `>= 0` ([cli.py:1358-1363](../../../src/wastech_orchestrator/cli.py#L1358)).
- Config/DB version gates → clean exit 2 (no traceback) ([cli.py:1365-1396](../../../src/wastech_orchestrator/cli.py#L1365)).
- `rerun`/`finalize` fail when the watch daemon is alive (shared clone) ([cli.py:780-786,875-881](../../../src/wastech_orchestrator/cli.py#L780)).
...
- Configuration not found → hint about `install`/`--config` (exit 2) ([cli.py:594-603](../../../src/wastech_orchestrator/cli.py#L594)).
- Unknown command → `SystemExit` ([cli.py:1397](../../../src/wastech_orchestrator/cli.py#L1397)).
...
- [B05](./B05-configuration.md)/[B04](./B04-install-registry-and-config-discovery.md) — configuration loading and discovery.
