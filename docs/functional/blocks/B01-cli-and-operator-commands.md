...
`build_parser` adds exactly fourteen subcommands under a required subparser ([cli.py:122](../../../src/wastech_orchestrator/cli.py#L122)), plus the global `--version` action ([cli.py:92](../../../src/wastech_orchestrator/cli.py#L92)). Verified against the argparse construction (not the help strings):
...
- **run** ([cli.py:170](../../../src/wastech_orchestrator/cli.py#L170)) — process one task file end-to-end through the pipeline ([B06](B06-orchestrator-pipeline.md)); positional `task_file` (metavar `PATH`) is a filesystem path to the `.md`/`.json`, not a task id (it registers a new task, so no id exists yet — unlike `rerun`/`status`/`finalize`, which are keyed by id).
...
- **promote** ([cli.py](../../../src/wastech_orchestrator/cli.py)) — move a staged task file from `tasks/preparing/` into `tasks/pending/` (atomic rename); takes a task id/file or `--all`; a decomposition root pulls its subtask specs. The watch scanner never reads `tasks/preparing/`.
- **preflight** ([cli.py:210](../../../src/wastech_orchestrator/cli.py#L210)) — read-only run-surface health/isolation check, no arguments. It does **not** validate flows (that is `validate-flow`'s job).
- **validate-flow** ([cli.py:213](../../../src/wastech_orchestrator/cli.py#L213)) — read-only, on-demand, config-aware validation of the operator's own flows in `.worc/flows/`; positional `name` (a bare stem or `NAME.yaml` resolved within `.worc/flows/`) validates one, `--all` validates every `*.yaml`, and passing neither is a usage error. Packaged built-ins are out of scope. Exit `0` (all valid) / `1` (any invalid) / `2` (name not found, usage error, or config load error).
...
- **list** ([cli.py:274](../../../src/wastech_orchestrator/cli.py#L274)) — read-only enumeration of the active task, the `tasks/pending` queue, and recent terminal tasks; `--pending` / `--recent [N]` / `--all` (a mutually exclusive group) focus one section, `--format {table,ids,json}` picks the rendering, and `--scope {rerun,status,finalize}` filters the id list to what that command accepts (completion-facing; implies `ids`).
...
```mermaid
flowchart TB
    argv(["argv"]) --> parse["build_parser → parse_args<br/>(global flags + 14 subcommands)"]
    parse --> guard["numeric guards (>= 0)<br/>heartbeat / poll / timeout"]
    guard --> disp["main: if-ladder on args.command"]
    disp --> drivers["cmd_install / cmd_run / cmd_watch /<br/>cmd_stop / cmd_restart / cmd_preflight /<br/>cmd_validate_flow / cmd_telegram_test / cmd_status /<br/>cmd_list / cmd_completion / cmd_upgrade_config /<br/>cmd_upgrade_docs / cmd_rerun / cmd_finalize"]
...
`cmd_list` ([cli.py:1506](../../../src/wastech_orchestrator/cli.py#L1506)) is the second read-only command beside `status`. It loads config (else exit 2), then opens `state.db` with `StateStore.open_readonly` **iff the file exists** (a missing DB is not an error — the file-derived pending queue still renders). The active section comes from `find_active_tasks()`, the pending section from `select_pending(pending_dir(config))` + `_scan_pending_meta` (no DB row yet, so a file is keyed by its scanned `id` or, failing that, its filename), and the recent section from the new read query `recent_tasks(limit)` ([B07](B07-state-machine-and-store.md)). `--format ids` routes to `_list_ids`, which prints bare ids on stdout (DB-derived) and applies `--scope`: `rerun` keeps only `FAILED`/`MANUAL_ACTION_REQUIRED`, while `status` and `finalize` both yield every known id (`finalize` is status-agnostic in `plan_finalize`, so the two coincide today; they stay distinct, command-aligned scope values so the completion script passes `--scope <command>` verbatim). `--scope` without an explicit `--format` implies `ids`. Edge cases all exit 0: an empty selection in table mode prints `list: no tasks`; `--format ids` with no matches prints nothing.

`cmd_completion` ([cli.py:1656](../../../src/wastech_orchestrator/cli.py#L1656)) needs no config: it reads the live parser surface (subcommand names + each subcommand's flags) via `_parser_surface` ([cli.py:1549](../../../src/wastech_orchestrator/cli.py#L1549)) and renders a static `bash`/`zsh` template. The script completes subcommands and flags statically, `run` with task files, and the id positionals of `status`/`rerun`/`finalize` dynamically by shelling out to `worc list --format ids --scope <command>` — a single source of truth, no `argcomplete`, no new runtime dependency. Task ids match `^[a-z0-9][a-z0-9._-]{0,63}$` (no shell metacharacters), so feeding the output through `compgen`/`compadd` is injection-safe.

### preflight (run_preflight)

`run_preflight` ([cli.py:931](../../../src/wastech_orchestrator/cli.py#L931)) is the shared read-only verdict used by both `cmd_preflight` and the installer's post-write auto-preflight. It aggregates an `ok` flag and `lines` over: each allowed provider's `preflight()` (`<cli> --version`; a missing adapter is a FAIL, [B18](B18-agent-providers.md)); the deterministic `check_isolation` policy ([B25](B25-security-policy.md)); a read-only summary of the configured `checks.command_sets` (`_summarize_command_sets`, [cli.py:965](../../../src/wastech_orchestrator/cli.py#L965), [B23](B23-check-discovery.md)) — no resolution, probing, or running; and `check_telegram_preflight` ([B26](B26-notifications-telegram.md)). It deliberately does **not** validate flows — preflight is a run-surface health gate, and flow correctness is an on-demand, operator-scoped concern owned by `validate-flow` (and enforced fatally at task dispatch by `FlowRegistry.resolve`, [B29](B29-flow-definition-and-validation.md)). `cmd_preflight` ([cli.py:987](../../../src/wastech_orchestrator/cli.py#L987)) prints the lines and returns `0` iff ready, else `1`. All lines are secret-free by contract.

`cmd_validate_flow` ([cli.py:987](../../../src/wastech_orchestrator/cli.py#L987)) loads the config, builds a `FlowRegistry` over `<repo>/.worc/flows/`, and validates the operator's own flows **config-aware** via `FlowRegistry.check_flows` — the full fatal validator (graph + ceiling + config-aware layer, incl. the `.worc/tools/` tool check) so it catches exactly what the engine sees at dispatch, plus the non-fatal prompt-variable lint surfaced as WARN. Its scope is `operator_flow_names()` — packaged built-ins are excluded ([B29](B29-flow-definition-and-validation.md)). It prints `flow <name>: OK`/`FAIL`/`WARN` lines and returns `0`/`1`/`2` (see the command list above).
...
- `status` performs zero side effects: read-only DB open, no provider/check/git invocation ([cli.py:1052](../../../src/wastech_orchestrator/cli.py#L1052)). `list` is read-only the same way (it opens the DB only `open_readonly`, scans `tasks/pending` without mutation, and adds no new status or schema bump); `completion` touches neither the DB nor config.
...
- `tests/test_cli_preflight.py` — `cmd_preflight`/`run_preflight` readiness lines and exit code: provider health, isolation FAIL, telegram OK/SKIP/FAIL, and that preflight no longer touches flows (a rogue operator flow does not flip the verdict); plus `telegram-test` success/timeout/disabled.
- `tests/test_cli_validate_flow.py` — `cmd_validate_flow`: OK/FAIL per operator flow, `--all`, the `NAME`/`NAME.yaml` forms, operator-only scope (a packaged built-in is not found), the non-fatal prompt-var WARN, the neither/both usage errors, and the `0`/`1`/`2` exit codes.
...
- `tests/core/test_cli_pipeline.py`, `tests/core/test_cli_rerun.py`, `tests/core/test_cli_finalize.py` — the `run`/`rerun`/`finalize` drivers: dispatch, plan/dry-run/confirm flow, daemon-running refusal, and status→exit-code mapping. `test_cli_pipeline.py` also covers `list` (default three-section overview, `--format ids`, `--scope rerun`/`status`, `--format json`, file-derived pending, empty-DB notice) and `completion` (bash/zsh script shape).
