...
- **install** ([cli.py:124](../../../src/wastech_orchestrator/cli.py#L124)) — set up the orchestrator in a repo under `.worc/` and write `config.yaml`; positional `repo_path` defaults to `.`, with `--provider`, repeatable `--check`, `--create-pr`/`--auto-mode` (BooleanOptional), `--non-interactive`, `--reconfigure`, `--skip-preflight`, `--dry-run`. Driver delegates to the wizard ([B03](B03-installer-and-scaffolding.md)).
...
Defined on the top-level parser, so they precede the subcommand: `--config`, `--log-level` ({debug,info,warning,error}, default info), `--log-format` ({logfmt,json}), `--log-file`, and `--heartbeat-seconds` (float, default 30, `0` disables) ([cli.py:93-120](../../../src/wastech_orchestrator/cli.py#L93)). `--version` prints `%(prog)s {__version__}` and exits 0 ([cli.py:92](../../../src/wastech_orchestrator/cli.py#L92); confirmed by `test_cli_version.py`).

### Dispatch and exit codes

`main` parses argv, then performs three post-parse numeric guards via `parser.error` (which exits 2): `--heartbeat-seconds`, `--poll-seconds`, and `--timeout` must each be `>= 0` ([cli.py:1263-1268](../../../src/wastech_orchestrator/cli.py#L1263)). The two latter are read defensively with `getattr(..., None)` because they exist only on some subparsers. It then dispatches on `args.command` through an `if`-ladder ([cli.py:1273-1296](../../../src/wastech_orchestrator/cli.py#L1273)), wrapped in a single `try` that catches `ConfigError`, `IncompatibleStateError`, and `detect.GhNotAvailableError` and turns each into `error: <msg>` + **exit 2** ([cli.py:1297-1299](../../../src/wastech_orchestrator/cli.py#L1297)). A command value not in the ladder raises `SystemExit` ([cli.py:1300](../../../src/wastech_orchestrator/cli.py#L1300)).
...
`cmd_status` ([cli.py:1041](../../../src/wastech_orchestrator/cli.py#L1041)) is strictly read-only: it opens the DB with `StateStore.open_readonly` ([B07](B07-state-machine-and-store.md)), resolves either the named `task_id`, the active tasks, or the single latest task, and prints each task's id/title/status, the flow checkpoint `node=` (`get_flow_checkpoint`), branch, subtask progress, `fix_iterations`, `updated_at` + elapsed seconds, and any cleanup error — then the resolved check profile, without resolving, probing, or running anything. A missing DB prints a message and returns 0; a named-but-missing task returns 1, while an empty active/latest set returns 0 ([cli.py:1066-1069](../../../src/wastech_orchestrator/cli.py#L1066)).

### preflight (run_preflight)

`run_preflight` ([cli.py:821](../../../src/wastech_orchestrator/cli.py#L821)) is the shared read-only verdict used by both `cmd_preflight` and the installer's post-write auto-preflight. It aggregates an `ok` flag and `lines` over: each allowed provider's `preflight()` (`<cli> --version`; a missing adapter is a FAIL, [cli.py:832-844](../../../src/wastech_orchestrator/cli.py#L832), [B18](B18-agent-providers.md)); the deterministic `check_isolation` policy ([B25](B25-security-policy.md)); check diagnostics ([B23](B23-check-discovery.md)); the flow registry's `validate_all()` over **every** packaged built-in and operator flow in `.worc/flows/`, so a broken or unsafe operator flow fails preflight before any task runs ([cli.py:862-868](../../../src/wastech_orchestrator/cli.py#L862), [B29](B29-flow-definition-and-validation.md)); and `check_telegram_preflight` ([B26](B26-notifications-telegram.md)). `cmd_preflight` ([cli.py:879](../../../src/wastech_orchestrator/cli.py#L879)) prints the lines and returns `0` iff ready, else `1`. All lines are secret-free by contract.
