...
The glob matcher lives in the shared `wastech_orchestrator.globmatch` module (`compile_glob` / `path_matches_any`), reused by the dangerous-diff deletion allowlist ([B14](B14-dangerous-diff-guardrail.md)) so both surfaces share one dialect. It translates a repo-relative glob into an anchored regex, dependency-free so it works on the 3.12 floor: `**/` is zero-or-more leading directories (so `**/*.md` matches both `README.md` and `docs/a/b.md`), `**` crosses separators, a single `*` and `?` stay within one path segment ([globmatch.py](../../../src/wastech_orchestrator/globmatch.py)). `compile_glob` is `lru_cache`d.
