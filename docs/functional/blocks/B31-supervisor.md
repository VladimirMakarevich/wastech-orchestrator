...
Each step is best-effort: a missing/bad/traversing candidate falls through to the next, so a bad prompt file never breaks the run. The packaged `implementation` flow owns `implementation/supervisor.md` (observe) and `implementation/summary.md` (finalize, reviving the previously-dead `roles/summary.md`); flows with no `supervisor:` block (e.g. `deep_research`, `security_audit`) use the global config observe lens + the built-in finalize. Only the wording moves into files — the structured schemas stay in code. A third supervisor prompt, `handoff_role_file`, follows the same two-step fallback (flow → built-in) — see below.

### Subtask handoff brief

For a decomposed task, `handoff(subtask_order, floor_context)` ([supervisor.py](../../../src/wastech_orchestrator/core/supervisor.py)) produces the **interpretive** layer of the intra-task subtask handoff (subtask-context-handoff ADR). The orchestrator's decomposition fan-out ([B06](B06-orchestrator-pipeline.md)) assembles a **deterministic factual floor** for each successor subtask from its committed `depends_on` predecessors (changed files via `GitManager.files_in_commit`, commit sha, acceptance criteria, spec pointer) and — when the supervisor is available — calls `handoff` to add a three-section brief (**New surface area** / **Locked decisions** / **Open edges**) via a `{new_surface_area, locked_decisions, open_edges}` schema on the **warm durable session** (it already observed the predecessor — no new turn budget). The combined floor + brief is **redaction-scrubbed** and written to `logs/<task-id>/subtasks/NN-slug.handoff.md` (local, uncommitted, **never** a memory tier), then injected as `{?predecessor_context}` into the region's `implementation` node. `handoff` is **best-effort**: a failed/empty brief yields `None` and the successor runs on the deterministic floor alone (accepted degradation). The handoff wording is the flow's `handoff_role_file` → built-in; the schema is hardcoded.
