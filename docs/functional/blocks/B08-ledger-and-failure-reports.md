...
This is the single writer for both failure-report paths. The flow recorder's `StateStoreRunRecorder.write_failure_report` calls straight through to it for a **budget-exhaustion** stuck (`loop`/`limit_name` from the named loop, building the `DecomposedFailureInfo` from `node_runs`/subtasks) with `last_check_log=None`, `last_review_findings=None`, `final_diff=""`, then stamps `failure_report_path` on the task row ([recorder.py:48](../../../src/wastech_orchestrator/core/flow/recorder.py#L48)). B06's `_write_infra_failure_report` calls it for an **infra terminal** (`_fail`, `loop="infra"`, `limit_name` = the infra error) so a `failed` / `manual_action_required` reached via an infra error is never left without a machine-readable report.
