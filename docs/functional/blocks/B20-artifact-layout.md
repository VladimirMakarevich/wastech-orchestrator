...
The module is deliberately content-blind and provider-agnostic: it imports neither the redaction module nor any provider syntax — it imports only `AgentRunResult` and `Stage` from `providers/base` for result serialization ([artifacts.py:23](../../../src/wastech_orchestrator/providers/artifacts.py#L23)). Redaction (B21) is the caller's job; the request passed in is already redacted ([artifacts.py:112-113](../../../src/wastech_orchestrator/providers/artifacts.py#L112)). The stream files (`stdout.log`, `stderr.log`, `events.jsonl`) and the optional `output-schema.json` are written directly by the providers (B18) using the paths this module hands back, not by this module.
...
- `create_attempt_dir(artifacts_root, task_id, stage, attempt, provider, *, node_run_id, subtask=None)` ([artifacts.py:80](../../../src/wastech_orchestrator/providers/artifacts.py#L80)) — creates the attempt directory (`exist_ok=False`) and returns its `ArtifactPaths`.
...
```text
<artifacts_root>/logs/<task-id>/stages/<stage>/[sub-<NN>/]run-<node_run_id:06d>/<attempt>-<provider>/
```

- `logs` / `<task-id>` / `stages` are literal segments; `<stage>` is `stage.value` (the `Stage` enum value, e.g. `planning`, `implementation`, `fixing`, `review`) ([artifacts.py:97](../../../src/wastech_orchestrator/providers/artifacts.py#L97)).
...
- `run-<node_run_id:06d>` is zero-padded to six digits; `<attempt>-<provider>` is the leaf, e.g. `1-codex` ([artifacts.py:100](../../../src/wastech_orchestrator/providers/artifacts.py#L100)). The two tests pin this exactly: `logs/task-001/stages/planning/run-000042/1-codex` ([test_artifacts.py:26](../../../tests/providers/test_artifacts.py#L26)) and the subtask form `…/implementation/sub-02/run-000007/2-codex` ([test_artifacts.py:43-45](../../../tests/providers/test_artifacts.py#L43)).
