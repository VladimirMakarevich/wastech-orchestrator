...
- Codex appends `-c sandbox_workspace_write.network_access=true`; the `--sandbox` value and the `never` approval policy stay in force ([codex.py:207-211](../../../src/wastech_orchestrator/providers/codex.py#L207)). The flow validator rejects Codex `workspace-write` agent nodes with resolved network access before launch; read-only Codex nodes keep a read-only sandbox. Verified by `test_network_access_*` and `test_read_only_request_uses_read_only_sandbox` in `test_codex_command.py`.

### Permission/sandbox mapping; forbidden values raise `CONFIGURATION_ERROR`

Claude maps a profile to a `(permission_mode, allowed_tools)` pair: `read-only → ("plan", Read/Glob/Grep)`, `workspace-write → ("acceptEdits", Read/Glob/Grep/Edit/Write/Bash)` ([claude.py:82-85](../../../src/wastech_orchestrator/providers/claude.py#L82)). `map_permission` raises `ProviderError(CONFIGURATION_ERROR)` for the forbidden full-access profile (`danger-full-access`) and for any unknown profile ([claude.py:219-226](../../../src/wastech_orchestrator/providers/claude.py#L219)), and the adapter never selects `bypassPermissions` itself. Codex resolves an effective sandbox from provider config plus the request profile without relaxing a read-only node to workspace-write. Both builders first run `find_forbidden_args` over `config.extra_args + request.extra_args` and reject absolutely-forbidden `--dangerously*` / `--yolo` / `--ignore-rules` flags as `CONFIGURATION_ERROR`; structured full-access selectors (`--sandbox danger-full-access`, Claude `--permission-mode bypassPermissions`) are operator-selectable but gated by `security.strict_isolation` preflight and the flow config-aware validator. `CONFIGURATION_ERROR` is not in `FALLBACK_ELIGIBLE`, so the router never falls past it.
...
- ~~large-scale DRY duplication between the two adapters (`run()` skeleton, `_write_request`, `_request_representation`, `_finalize_failure`, `_extra_secrets`, `_secret_env_values`, `_scrub_raw_session`, footer helpers, `ParsedEvents`, and the byte-identical tail).~~ **Resolved (2026-06-22, audit #7):** extracted to `_adapter_base.py` `BaseCliProvider`; the adapters now carry only `_build_argv`/`_parse`/`_signatures`/`_executable_label` (+ Claude's `_representation_extras` for the `reasoning` key and `map_permission`). `_secret_env_values` (the one copy) imports `_MIN_DENIED_SECRET_LEN` from B21's redaction module (audit #27 — single source for the env-secret length threshold). A test asserts both adapters still emit their distinct argv.
- ~~Codex reasoning map was a local unguarded dict subscript.~~ Resolved: provider capabilities now expose Codex reasoning aliases, and `build_codex_argv` raises `ProviderError(CONFIGURATION_ERROR)` for an unsupported value before launch.
