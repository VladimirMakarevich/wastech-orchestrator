...
- `src/wastech_orchestrator/providers/base.py:25` — `Stage.PUBLISHING` is dead/vestigial: defined in the enum but referenced nowhere in `src/` or `tests/` (the supervisor reuses `Stage.SUMMARY` as its identity at `core/supervisor.py:42`). The `Stage` enum is a documented transitional identity, but this member specifically has zero consumers.
