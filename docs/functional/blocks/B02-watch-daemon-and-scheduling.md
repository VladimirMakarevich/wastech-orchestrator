...
1. Refuse a second watcher: if the recorded daemon is genuinely live, print "already running" and exit 1 ([cli.py:963](../../../src/wastech_orchestrator/cli.py#L963)). Liveness goes through `running_daemon_pid`, which matches the recorded start-time so a **recycled** PID (an unrelated process that reused the number) does not read as our daemon. A stale or recycled PID is not refused — it is overwritten on start.
...
`stop_process` is the escalation engine ([process_control.py:101](../../../src/wastech_orchestrator/process_control.py#L101)): read the PID **record** (absent → `found=False` no-op), probe liveness with the recorded start-time (dead **or recycled** → reap the file, `already_dead=True`, never signaling the innocent recycled process), send `SIGTERM`, then poll `is_running` until `timeout`; if still alive, escalate to `SIGKILL`. The PID file is unlinked in every terminal branch — the backstop for a `SIGKILL`ed daemon that could not remove its own file ([process_control.py:144](../../../src/wastech_orchestrator/process_control.py#L144)). A `ProcessLookupError` racing between the probe and either signal is swallowed and treated as already-dead.
...
- **One daemon per artifact root.** A genuinely-live recorded daemon makes `cmd_watch` refuse (exit 1); a stale **or recycled** PID is overwritten ([cli.py:963](../../../src/wastech_orchestrator/cli.py#L963)).
- **PID identity is recycling-proof.** The PID file is JSON `{pid, start_time}`; liveness (`is_running` / `running_daemon_pid` / `stop_process`) matches the recorded start-time, so a recycled PID never reads as our daemon. When the start-time can't be read it degrades to a bare-PID probe ([process_control.py](../../../src/wastech_orchestrator/process_control.py)).
...
- **`stop` is idempotent.** Absent, malformed (incl. a pre-JSON bare integer), stale, or recycled PID files signal nothing ([process_control.py:119](../../../src/wastech_orchestrator/process_control.py#L119)).
