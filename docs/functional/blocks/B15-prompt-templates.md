...
Because an unknown `{name}` renders verbatim, a typo (`{plna_path}`) or an invented variable silently ships to the agent as placeholder text. `lint_prompt_variables` ([validator.py](../../../src/wastech_orchestrator/core/flow/validator.py)) scans each node's `role_file` for `{name}` / `{?name}` tokens (via `referenced_variables`, [prompts.py](../../../src/wastech_orchestrator/core/prompts.py)) outside the flow's valid-set and returns a `PromptVarWarning` per file+token. It is **non-fatal** — a verbatim render is the safe-renderer fallback, so failing the run would break the "code/JSON braces pass through" contract; `preflight` prints the warnings (`FlowRegistry.lint_all`, surfaced in [cli.py](../../../src/wastech_orchestrator/cli.py)) without failing the gate.
