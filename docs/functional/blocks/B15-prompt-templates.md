...
`ALLOWED_PROMPT_VARS` is the exact, closed set of names a template may interpolate ([prompts.py:21-39](../../../src/wastech_orchestrator/core/prompts.py#L21)): `task_id`, `stage`, `repo_path`, `repo` (a flow-engine alias for `repo_path`; one shared allowlist), `task_path`, `plan_path`, `diff_path`, `checks_path`, `review_path`, `subtask_order`, `subtask_count`, `subtask_spec_path`, `skills_path`, `memory_path` (the per-node memory retrieval-packet path; empty unless memory is enabled and the node references it), and `predecessor_context` (the intra-task subtask handoff-brief path; empty outside a decompose subtask with `depends_on` predecessors). Every entry is either a scalar metadata value (`task_id`, `stage`, `subtask_order`, `subtask_count`) or an artifact _path_ — never a large body. The test pins this exact set ([test_prompts.py:25-40](../../../tests/core/test_prompts.py#L25)). `{skills_path}` is the skill-reference paths joined by newline (or `None` when there are none) ([agent.py:399](../../../src/wastech_orchestrator/core/flow/nodes/agent.py#L399)) — see [B13](B13-skill-selection.md). Beyond this static core, the effective allowlist is widened per flow with each agent node's `{<node_id>_path}` — see the anti-drift-lint section below.
...
### The anti-drift lint (flow-derived valid-set)

Because an unknown `{name}` renders verbatim, a typo (`{plna_path}`) or an invented variable silently ships to the agent as placeholder text. `lint_prompt_variables` ([validator.py](../../../src/wastech_orchestrator/core/flow/validator.py)) scans each node's `role_file` for `{name}` / `{?name}` tokens (via `referenced_variables`, [prompts.py](../../../src/wastech_orchestrator/core/prompts.py)) outside the flow's valid-set and returns a `PromptVarWarning` per file+token. The flow-local **supervisor** prompts (`supervisor.{role_file, finalize_role_file, handoff_role_file}`) are role files too, scanned against the tiny set the supervisor actually populates (`{task_id, repo, repo_path}`) — so a `{plan_path}` in a `supervisor.md` is flagged, since the supervisor never fills it. It is **non-fatal** — a verbatim render is the safe-renderer fallback, so failing the run would break the "code/JSON braces pass through" contract; `preflight` prints the warnings (`FlowRegistry.lint_all`, surfaced in [cli.py](../../../src/wastech_orchestrator/cli.py)) without failing the gate.

The valid-set is **flow-derived**, not the static `ALLOWED_PROMPT_VARS`: `valid_prompt_vars(snapshot)` ([prompt_vars.py](../../../src/wastech_orchestrator/core/flow/prompt_vars.py)) is `ALLOWED_PROMPT_VARS ∪ node_output_vars(snapshot)`. The same helper is the effective allowlist the agent runner hands the renderer, so the lint and the renderer always agree on which names are real. The delivered operator guide (`packaged/guide/flows/prompt-variables.md`, seeded to `.worc/guide/flows/`) is the author-facing reference for the same set.
...
- **Uses:** none in `core.prompts` (stdlib `re` only); `core.flow.prompt` uses `pathlib` and re-exports through `render_prompt`. The `role_file` is package data shipped inside the flow (e.g. `packaged/flows/<task_type>/`), seeded as editable copies under `.worc/flows/` by the installer ([B03](B03-installer-and-scaffolding.md)).
