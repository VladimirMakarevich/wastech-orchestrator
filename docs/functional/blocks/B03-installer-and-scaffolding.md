...
- `propose_default_commands(repo_root) -> list[str]` ([checks/detect.py](../../../src/wastech_orchestrator/checks/detect.py)) — seeds the installer's default `checks.commands` through the **same** lockfile-aware B23 detector the runtime resolver uses, rendered to shell strings via `shlex.join`. Replaces the deleted first-match `install/detect.detect_checks` (one ecosystem-detection source of truth — B23).
...
- **Checks** ([wizard.py:161-171](../../../src/wastech_orchestrator/install/wizard.py#L161)): an explicit `--check` list wins; otherwise `propose_default_commands` (the B23 detector) proposes lockfile-aware ecosystem defaults as shell strings — non-interactive takes them as-is, interactive offers them (default yes) and falls back to a line-by-line `ask_list`.
...
- ~~**duplicate ecosystem/check detection** independent of B23.~~ **Resolved (2026-06-22, audit #12):** the first-match `detect_checks`/`_node_checks` were deleted; the wizard now seeds defaults through `checks.detect.propose_default_commands`, which runs the same lockfile-aware `RepositoryInspector`→`CheckCandidateDetector` (B23) the runtime resolver uses and renders the highest-confidence candidate per logical check via `shlex.join`. One ecosystem-detection source of truth.
...
- `tests/install/test_detect.py` — `git_info` root/origin/branch/cleanliness (incl. nested subdir and no-origin), provider/`gh` discovery via `shutil.which`, `preflight.require_gh` raise/no-op, and `propose_default_commands` by marker (incl. lockfile-aware pnpm/uv cases).
