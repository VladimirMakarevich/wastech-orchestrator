...
`build_config_mapping` assembles a primitive dict mirroring the packaged `config.example.yaml`, stamped with the current `CONFIG_SCHEMA_VERSION` ([config_writer.py](../../../src/wastech_orchestrator/install/config_writer.py)). Only the _selected_ providers get an `agents.providers.<id>` block, emitted in canonical `ProviderId` order regardless of how the wizard collected them. Exactly one block is marked `primary: True` — Claude when selected, else the first provider — encoding the single-global-primary routing invariant (B17/B05). `providers.defaults.SHIPPED_PROVIDER_DEFAULTS` is the code source of truth: Claude ships `claude-sonnet-5`/`high`, while Codex ships `""`/`high` so model selection belongs to the CLI/account. A regression test loads both generated and packaged YAML and requires the Codex values to equal `CANONICAL_CODEX_MODEL_DEFAULT`. Provider blocks also carry `permission_profile: workspace-write`, Codex `sandbox: workspace-write`, Claude `max_turns: 400`, and empty `extra_args`.

The `security` block is hard-coded to immutable safe defaults — `strict_isolation: True`, an OS-aware `allowed_environment` allowlist (`default_allowed_environment()`: the cross-platform base plus the host OS's launch essentials, e.g. `SystemRoot` on Windows), `denied_read_paths` (`.env`, `secrets/**`), and `denied_commands` including `git commit` / `git push` / `gh pr create` / `gh pr merge` ([config_writer.py:103-114](../../../src/wastech_orchestrator/install/config_writer.py#L103)). The quarantine folder is placed under `.worc/tasks/rejected` so rejected tasks are never swept into the audit commit ([config_writer.py:78](../../../src/wastech_orchestrator/install/config_writer.py#L78), [config_writer.py:124](../../../src/wastech_orchestrator/install/config_writer.py#L124)); `git.footprint.audit_on_branch` is `"task"` ([config_writer.py:141-143](../../../src/wastech_orchestrator/install/config_writer.py#L141)). `auto_merge` is off by default ([config_writer.py:136](../../../src/wastech_orchestrator/install/config_writer.py#L136)).
...
flowchart TD
  ...
  GUIDE --> FLOWS["copy built-in flows + node prompts to .worc/flows/"]
  FLOWS --> GI[append .worc/ to .gitignore]
  GI --> ENV[write .env.example]
  ENV --> PRE[auto-preflight]
  PRE --> Z([exit 0 / 1])
```

1. **Resolve the `.worc` home and config path** ([cli.py:1400](../../../src/wastech_orchestrator/cli.py#L1400)): `<repo>/.worc/config.yaml`, resolved to an absolute path. `WORC_HOME = ".worc"` ([cli.py:75](../../../src/wastech_orchestrator/cli.py#L75)).
2. **`--dry-run`** prints the full plan and returns 0 without touching the filesystem ([cli.py:1403-1405](../../../src/wastech_orchestrator/cli.py#L1403)).
3. **Idempotency / reconfigure** ([cli.py:1407-1411](../../../src/wastech_orchestrator/cli.py#L1407)): if `config.yaml` already exists and `--reconfigure` was not passed, print "already configured" and just re-run preflight. With `--reconfigure`, copy the existing config to a timestamped `config.yaml.bak-<UTC>` sibling first (`_install_backup_config`, [cli.py:1302](../../../src/wastech_orchestrator/cli.py#L1302)).
4. **Generate + validate, then create dirs, then atomic write** ([cli.py:1413-1415](../../../src/wastech_orchestrator/cli.py#L1413)): `_install_create_dirs` makes the repo-root audit dirs and the `.worc/` runtime dirs (idempotent) ([cli.py:1310](../../../src/wastech_orchestrator/cli.py#L1310)); `_install_atomic_write` writes to a temp file in the same dir and `os.replace`s it into place, unlinking the temp on any failure ([cli.py:1285](../../../src/wastech_orchestrator/cli.py#L1285)).
5. **Copy the packaged guide and the built-in flows** ([cli.py:1419](../../../src/wastech_orchestrator/cli.py#L1419)): `_copy_worc_docs` copies the packaged `worc/` docs into `.worc/guide/`, and `_copy_packaged_flows` ([cli.py:380](../../../src/wastech_orchestrator/cli.py#L380)) copies the packaged built-in flows (`implementation`/`deep_research`/`security_audit`) plus their per-node role-prompt templates into `.worc/flows/` — both skip files that already exist unless `--reconfigure`. The flow copies are **active and the only ones that run**: the registry resolves flows solely from `.worc/flows/` — the packaged tree is delivery-only (B29) — so the operator edits these files to customize a node's prompt or graph, and a built-in that install has not delivered here is unresolvable. On `--reconfigure`, `_backup_flows_dir` ([cli.py:404](../../../src/wastech_orchestrator/cli.py#L404)) first snapshots the existing `.worc/flows/` to a timestamped `flows.bak-<UTC>` sibling so operator edits stay recoverable, then the copy refreshes to the packaged version. A `.env.example` is also written ([cli.py:1432](../../../src/wastech_orchestrator/cli.py#L1432)).
6. **Gitignore `.worc/`** ([cli.py:1435](../../../src/wastech_orchestrator/cli.py#L1435)): `append_runtime_excludes` idempotently appends the `.worc/` line to the repo's tracked `.gitignore` (B22).
7. **Auto-preflight, fail-closed** (`_install_run_preflight`, [cli.py:1360](../../../src/wastech_orchestrator/cli.py#L1360), called at [cli.py:1440](../../../src/wastech_orchestrator/cli.py#L1440)): unless `--skip-preflight`, run `run_preflight`; on a non-ready verdict, **keep the written config but return exit 1**. Preflight validates every provider and the isolation policy, prints the `checks.command_sets` summary, and checks Telegram readiness — it does **not** validate flows (that is the on-demand `worc validate-flow`; a dispatched flow is validated fatally at task time by `resolve`, B29). There is no check-profile seeding — the gate is whatever the operator later authors in `command_sets`.

### The `.worc/` layout it scaffolds

Two distinct trees ([cli.py:68-84](../../../src/wastech_orchestrator/cli.py#L68)):

- **Tracked, repo-root** `tasks/{pending,done,failed}` (`REPO_TASK_DIRS`) — the committed audit trail (the task file + its `<id>.summary.md` in `done`/`failed`). Created empty, so they do not show in `git status` until a task writes into them ([cli.py:1129-1130](../../../src/wastech_orchestrator/cli.py#L1129)). `install` always scaffolds the default `tasks/` layout; the runtime honors `config.paths.tasks_dir` (an operator who renames it creates the lifecycle subfolders themselves).
- **Gitignored, under `.worc/`** `{logs, workspace, checks, tasks/rejected}` (`WORC_RUNTIME_DIRS`) plus `config.yaml`, the runtime `state.db` / `orchestrator.pid`, the `guide/`, and the editable `flows/` (the built-in flows + their `roles/` prompt templates, plus any `flows.bak-<UTC>` snapshot left by `--reconfigure`) ([cli.py:83-84](../../../src/wastech_orchestrator/cli.py#L83)). The whole `.worc/` home is gitignored, so the operator's `git status` stays clean; `tasks/rejected` (the quarantine) lives here so rejected tasks escape the audit commit ([cli.py:73-84](../../../src/wastech_orchestrator/cli.py#L73)).
...
- **Uses:** B19 (safe subprocess runner for git probes), B05 (config loader + schema + semantic validator the generator round-trips through; the generated `checks.command_sets` is empty), B22 (`.gitignore` append; `append_runtime_excludes`), B18/B17 (`ProviderId`, per-provider blocks, single primary), B26 (Telegram preflight line). It no longer touches B23 — checks are not seeded at install — and preflight no longer touches B29 (flows are validated on demand by `worc validate-flow`, not at install).
