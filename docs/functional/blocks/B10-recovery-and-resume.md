...
### B-lite soft pause and the resume ceiling

An infra exhaustion (every allowed provider returned `provider_unavailable` / `network_unavailable` after retries + fallback, **or** `rate_limited` after fallback — the `PARK_ELIGIBLE` classes, `TRANSIENT_RETRYABLE ∪ {rate_limited}`) does **not** go terminal `failed`. The orchestrator wrapper parks the task: it stays `RUNNING` (active), stamps `tasks.blocked_since` (first park only), and ends the run with no state transition (`_park` in [B06](B06-orchestrator-pipeline.md)). A rate-limit park waits out the provider's usage-window reset; because the parked task holds the single slot, the `watch` queue also stops picking up new tasks until it clears. Because the task is still active, the very next `reconcile` returns `RESUME` for it — recovery needs no special case. The checkpoint preserves `current_node`, so the resumed run re-enters where it stopped; publish idempotency (B22) keeps it double-commit-safe.

The pause is bounded: before re-running, `_resume_via_engine` checks `blocked_since` against `agents.retry.max_blocked_s` (total parked wall-clock, using the injected clock) and, if exceeded, goes terminal `failed` instead of resuming — so a sustained outage cannot park a task forever. `blocked_since` is cleared at any terminal transition (`_go_terminal`).
...
- **A B-lite soft pause is resumable, not terminal, and time-bounded.** An infra exhaustion in `PARK_ELIGIBLE` (transient outage **or** rate-limit) parks the task as active (`tasks.blocked_since`) so the next `reconcile` resumes it; it is failed only once it stays parked past `agents.retry.max_blocked_s` (checked in `_resume_via_engine`, B06). Cleared at terminal.
- **A corrupt/missing normalized manifest fails closed.** `_resume_task` guards `load_normalized` in a `try/except (json.JSONDecodeError, OSError, KeyError, ValueError)`; an unreadable `task.normalized.json` degrades to terminal `manual_action_required` via a `_degraded_pipeline` built from the persisted `TaskRow` (counters/decomposition come from the controlled SQLite row, not the corrupt artifact) instead of crashing out of `resume()` ([orchestrator.py:727](../../../src/wastech_orchestrator/core/orchestrator.py#L727)).
