...
A transient-infra exhaustion (every allowed provider returned `provider_unavailable` / `network_unavailable` after retries + fallback) does **not** go terminal `failed`. The orchestrator wrapper parks the task: it stays `RUNNING` (active), stamps `tasks.blocked_since` (first park only), and ends the run with no state transition (`_park` in [B06](B06-orchestrator-pipeline.md)). Because the task is still active, the very next `reconcile` returns `RESUME` for it — recovery needs no special case. The checkpoint preserves `current_node`, so the resumed run re-enters where it stopped; publish idempotency (B22) keeps it double-commit-safe.
...
- **A B-lite soft pause is resumable, not terminal, and time-bounded.** A transient-infra exhaustion parks the task as active (`tasks.blocked_since`) so the next `reconcile` resumes it; it is failed only once it stays parked past `agents.retry.max_blocked_s` (checked in `_resume_via_engine`, B06). Cleared at terminal.
