...
2. **Security ceiling** (`_check_ceiling`) — an evaluator is always `read-only` and never `editing_lineage`; every agent `permission_profile` ≤ `permission_ceiling`; `extra_args` pass the common forbidden-flag check ([B25](B25-security-policy.md)); `role_file` contains no path traversal.
3. **Config-aware** (`validate_flow_against_config` → `_check_config_consistency`) — every explicitly-pinned node `provider` ∈ `agents.allowed`; node `reasoning` is valid for the provider that will run it; a Codex node's `extra_args` pass the closed typed parser regardless of `strict_isolation`; Codex is not routed to a `workspace-write` agent node with `network_access: true`; `permission_ceiling` is reachable by an allowed provider; Claude full-access selectors retain the existing `strict_isolation` gate.
