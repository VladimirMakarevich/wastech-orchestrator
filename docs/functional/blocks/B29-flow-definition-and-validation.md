...
3. **Config-aware** (`validate_flow_against_config` → `_check_config_consistency`, [validator.py:318](../../../src/wastech_orchestrator/core/flow/validator.py#L318)) — every explicitly-pinned node `provider` ∈ `agents.allowed`; node `reasoning` ∈ `{low, medium, high, xhigh, max}` ([validator.py:57](../../../src/wastech_orchestrator/core/flow/validator.py#L57)); `permission_ceiling` ≤ the capability of at least one configured allowed provider.
