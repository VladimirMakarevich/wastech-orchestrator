...
- **post-node hook** (`_engine_post_node`, [orchestrator.py:1392](../../../src/wastech_orchestrator/core/orchestrator.py#L1392)) — after each executed node: the supervisor observes the step (read-only, except the terminal publish node); an `output_artifact` slot is persisted; for the decomposition `proposed_by` node the decomposition is decided and materialized. (Skill selection no longer runs here — it is resolved once at task start in `_resolve_skill_layers`, above.)
