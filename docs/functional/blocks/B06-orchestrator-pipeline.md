...
- `build_orchestrator(config, ...)` and `build_providers` — the dependency-injection assembly, in the composition root [`composition.py`](../../../src/wastech_orchestrator/composition.py) (moved out of `core` so the `import-linter` `core-not-concrete-adapters` contract holds; `core` never imports a concrete adapter).
...
- **post-node hook** (`_engine_post_node`, [orchestrator.py:1563](../../../src/wastech_orchestrator/core/orchestrator.py#L1563)) — after each executed node: the supervisor observes the step (read-only, except the terminal publish node); a non-blocking evaluator that accepted only because its `max_rework_per_stage` budget ran out (`NodeOutcome.rework_exhausted`, [B30](B30-flow-node-runners.md)) is surfaced as a console **warning** (always, independent of Telegram) so an operator knows the stage moved on with a finding still open; when `telegram.trace` is on, a best-effort live step-trace (`<emoji> <node-id> → <outcome>`, node id + outcome only) is pushed via the notifier ([B26](B26-notifications-telegram.md)) — using the ⚠️ `TRACE_REWORK_EXHAUSTED` label for that budget-exhausted accept, else `NodeOutcome.kind` — fire-and-forget, never raised; an `output_artifact` slot is persisted; for the decomposition `proposed_by` node the decomposition is decided and materialized. (Skill selection no longer runs here — it is resolved once at task start in `_resolve_skill_layers`, above.)
...
`_engine_run` catches `NodeManualRequired` → `manual_action_required`, `EvaluatorInfraError` (an evaluator that could not run) → `manual_action_required` (branch + green diff preserved for the operator, not discarded), and `FlowCancelled` → the existing resumable `ErrorClass.CANCELLED` parking path. The same watch-daemon cancellation callback is wired into both FlowEngine and Router: a soft stop is noticed before the next node, while a hard-killed provider error suppresses retry/fallback. The plain `NodeInfraError` maps to either a **B-lite soft pause** when its class is transient/cancelled (`_park` stamps `tasks.blocked_since` and returns non-terminal `RUNNING`) or terminal `failed` for any other class. It syncs the engine's authoritative loop counters back into the operator-facing `tasks` columns before any terminal transition (`_sync_counters_from_run_state`). The B-lite ceiling (`agents.retry.max_blocked_s`) is enforced on resume ([B10](B10-recovery-and-resume.md)).
...
- `rerun_task` ([orchestrator.py:457](../../../src/wastech_orchestrator/core/orchestrator.py#L457)) archives prior artifacts, resets the branch to base, clears per-attempt state, and re-runs from scratch; `continue_task` ([orchestrator.py:479](../../../src/wastech_orchestrator/core/orchestrator.py#L479)) revives the terminal task and resumes at the flow checkpoint. Two optional operator controls patch the checkpoint before resume (`_apply_continue_controls`, the one-shot seam — never in the crash-recovery hydrate path): `--reset-fix-budget` grants a fresh consecutive fix budget by resetting the per-loop counters via `FlowRunState.reset_consecutive_fix_budget()` while preserving the global `fix_iterations` / `total_fix:*` so the `max_total_fix_iterations` backstop is never weakened; `--from <node>` re-enters at a chosen node by overriding the persisted `current_node` (validated against the checkpoint's resolved flow — an unknown node or a drifted `flow_fingerprint` refuses, since resume ignores the override on a mismatch). `plan_rerun` / `plan_finalize` are read-only fact-gatherers for the dry-run views. `plan_rerun` also broadens the dirty-tree gate: on `--continue`, once the task has reached a code-operating stage (`_worktree_is_task_output` — an evaluator/checks/publish node has run) the task's own uncommitted work is tolerated as the legitimate input to a review/fixing/publish re-entry (with a warning that it will be committed), while a fresh rerun and a pre-edit re-entry still refuse it. `plan_rerun` resolves the task source file tolerant of lifecycle-folder desync: if the stored `source_path` is missing it searches `tasks/{pending,done,failed}/` for the task by id (then slug) via `_resolve_task_source`, so a file manually moved between lifecycle folders still reruns; more than one match refuses with an ambiguity message rather than guessing.
