...
- **Scoped staging:** stage only the agent's intended code paths via an explicit pathspec; **never `git add .`/`-A`**; `tasks/`/`logs/`/`workspace/` (and, under in-repo, the root runtime files `state.db`/`config.yaml`) are always excluded from code commits (spec §21.1).
