...
Status: **superseded** (2026-06-28) — this exploratory note is now consolidated into the structured task at [memory/index.md](memory/index.md) (problem / requirements / design / acceptance / plan), whose design distills the research-backed [architecture blueprint](memory/research/memory-architecture-blueprint.md). Start there to build the implementation plan and ADR(s). Date: 2026-06-26 Owner: Vladimir Makarevich

> This document is kept as the historical exploratory predecessor. The blueprint keeps its spine and refines it where the [internal](memory/research/worc-report/worc-deeep-research-memory-report.md) and [external](memory/research/3rd-party-report/00-3rd-party-deep-research-memory-report.md) deep-research efforts and the [supervisor role-split note](memory/research/supervisor-role-split.md) pointed (see the blueprint's "How this refines the predecessor draft").
