...
- [ ] The selected permission modes are accepted by the supported Claude CLI and preserve the provider-neutral read-only/workspace-write ceiling; no test or shipped doc relies on the undocumented legacy `default` alias.
