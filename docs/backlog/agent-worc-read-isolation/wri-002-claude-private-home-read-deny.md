...
- Compute the private-home deny internally and append `Read(<home>)` + `Read(<home>/**)` to Claude's `--disallowedTools`, alongside the existing secret and native-memory denies.
...
- [ ] A Claude run has `Read(.worc)` and `Read(.worc/**)` in `--disallowedTools`; `.worc-io/**` is absent from the deny.
