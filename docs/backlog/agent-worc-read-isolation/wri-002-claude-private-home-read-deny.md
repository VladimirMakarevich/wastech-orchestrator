...
**Status:** implemented; **instruction-injection half rolled back by [VF-5](runtime-validation-findings.md) (2026-07-24)** **Milestone:** 1 **Source:** [decision record](README.md) **Dependencies:** WRI-009, WRI-011, WRI-012

> **Amended by VF-5 (2026-07-24).** `--setting-sources ""` is **kept** as the security lockdown (it disables project/user/local settings ⇒ no hooks/MCP/skills/plugins). What changed: the frozen repository instructions are **no longer injected** via `--append-system-prompt-file` (that instruction-injection role is removed with the WRI-011 rollback). Because `--setting-sources ""` also turns off native `CLAUDE.md` auto-load (the CLI gates memory on the same switch, with no memory-only path), the agent instead **reads the repo's root instruction files itself** (role-prompt-directed), and those files are write-denied for the run so what it reads is immutable. See [runtime-validation-findings.md](runtime-validation-findings.md) VF-5.
