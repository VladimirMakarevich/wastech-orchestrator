...
**Status:** implemented; **repository-instruction half rolled back by [VF-5](runtime-validation-findings.md) (2026-07-24)** **Milestone:** 0 (security prerequisite) **Source:** [decision record](README.md) **Dependencies:** WRI-001, WRI-010

> **Amended by VF-5 (2026-07-24) — the repository-instruction discovery-disable + freeze-and-inject half is rolled back.** Everything below about disabling provider-native repository-instruction discovery and injecting a frozen `AGENTS.md`/`CLAUDE.md` set (the In-scope items on Codex `project_doc_max_bytes`/Claude `--setting-sources ""`-for-instructions + adapter injection; the ACs about discovery-disable + controlled injection; the instruction-precedence-via-injection bullet) is **reversed**. Repository instructions are now read **natively by each agent**: Codex uses native `AGENTS.md` discovery; Claude keeps `--setting-sources ""` only as the security lockdown (no hooks/MCP/skills — verified: there is no memory-only load path) and the agent reads the root files via its Read tool. Reproducibility comes from **write-denying the tracked root instruction files for the run** (immutable) plus the kept manifest **digest**, not from injection. The **task-packet** and **skill-package** freeze in this spec are **unchanged**. See [runtime-validation-findings.md](runtime-validation-findings.md) VF-5 and [follow_ups.md](../follow_ups.md).
...
- Before the supervisor skill-map proposal, snapshot the task packet and a bounded, validated inventory of every candidate skill's metadata/package identity. The proposal receives exchange paths and small allowlisted metadata, never inline task title/description bodies or live skill paths. Bound the inlined inventory metadata explicitly — identifier-sized name/path fields with hard caps; skill descriptions come from repository `SKILL.md` content and are untrusted, so they are truncated to a recorded bound or replaced by path references. The supervisor's observe-step `final_message` excerpts remain an accepted, bounded inline surface; record that bound too. After Core accepts the proposal, expose only the selected packages to each node.
...
- [ ] The earliest supervisor skill proposal receives the frozen task/inventory without inline task title/description or live skill paths; its adapter-isolation half is a cluster exit criterion verified when WRI-002/003 land, while the frozen-input half gates this task.
