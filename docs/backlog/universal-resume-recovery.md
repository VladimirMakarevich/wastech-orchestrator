...
Status: **partially implemented** (Phase 0 + Phase 1 `--from`, 2026-07-11; proposed 2026-07-10) Date: 2026-07-10 Owner: Vladimir Makarevich

> **Implemented 2026-07-11 — Phase 0 (full) + Phase 1 (`--from` only).** Phase 0: `rerun --continue --reset-fix-budget` resets the consecutive fix-loop counters (reusing `FlowRunState.reset_consecutive_fix_budget()`) while preserving the global `fix_iterations`/`total_fix:*`, so the `max_total_fix_iterations` backstop is never weakened (bounded termination intact); and `--continue` now tolerates the task's own uncommitted work once it has reached a code-operating stage (`_worktree_is_task_output` — an evaluator/checks/publish node has run), warning that the WIP will be committed. Phase 1: `--from <node>` re-enters at a chosen node by overriding the persisted `current_node` (validated against the checkpoint's resolved flow; unknown node or fingerprint drift refuses). Applied one-shot in `continue_task` via `_apply_continue_controls` (not the shared hydrate path). No config `agents.resume` block and no schema bump were needed (the `--reset-fix-budget` boolean spelling avoids a per-attempt global raise). **Deferred** (tracked in [follow_ups.md](follow_ups.md)): capability #5 per-leg provider/model overlay, #6 bounded force-forward, #7 guided reconciliation / `--commit-wip` / own-vs-foreign discriminator, #8 resume-then-finish, `--from` under decomposition, and `--re-resolve` for flow drift.
