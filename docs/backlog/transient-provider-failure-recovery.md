...
- **No packaged flow declares a per-node `provider:`** (verified: zero `provider:` keys across `core/flow/packaged/*.yaml`). So every default-flow node resolves its primary to the global primary → `fallback is None` → the sequence is a single attempt.
...
- **[config.example.yaml](../../config.example.yaml)**, **[templates/config.example.yaml](../../src/wastech_orchestrator/templates/config.example.yaml)**, **[install/config_writer.py](../../src/wastech_orchestrator/install/config_writer.py)** — добавить дефолтный блок `agents.retry` (синхронно с дефолтами загрузчика).
