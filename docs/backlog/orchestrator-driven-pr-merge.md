...
Status: **implemented 2026-06-27** (operator-triggered merge + agent-assisted conflict resolution + safety-only gate + flat CLI). Date: 2026-06-23 Owner: Vladimir Makarevich

> **Implementation note (2026-06-27).** Built with one deliberate change from the locked decision below: conflict resolution is hosted as an **operator-editable `merge` flow** (the rejected "Option B"), not the "Option A" orchestrator routine this doc locked. In discussion the owner chose the flow so resolution reuses the engine (per-node provider/model selection, the bounded fix-loop, prompt-audit, operator-tunable YAML) instead of a second non-flow agent path. The flow name is configured by `git.merge_flow` (default `merge`, seeded at `.worc/flows/merge.yaml`); it runs **only on a conflict** (a clean base-merge stays mechanical), transactionally (ephemeral engine run, no checkpoint), and the orchestrator still owns the merge commit + `gh pr merge`. One scope cut vs. the owner's full idea: a single configured flow ships now; a path/area-based **collection** (`git.merge_flows`, selected like `checks.command_sets`) is the deferred seam. Everything else — `worc prs` (`--check`/`--sync`), `worc merge-task`, the added `worc tasks`, safety-only gating, no new status / no schema bump — matches the locked decision. See the [follow_ups](follow_ups.md) row for the file map.
...
worc prs --sync          # record PRs already merged externally; dry-run by default, --yes to write
...
6. **It unblocks the dependency graph.** `depends_on` scheduling already treats a task as ready only once its PR is **MERGED** (`pr_merge_state` → `MERGED`, see task-dependencies.md). An operator-driven merge is the human-in-the-loop event that flips a dependency to ready — today only `auto_merge` or a manual GitHub merge can.
7. **External merges are the inverse scenario — `--sync` is their counterpart.** When the operator merges a PR directly on GitHub (not through `merge-task`), the orchestrator has no record of it: the task stays with an open `pr` op, `depends_on` dependents remain blocked, and `worc prs` keeps showing the PR as pending. `--sync` is the reconciliation step: it probes every open-PR task against GitHub (`verify_pr_state`), records a `pr_merge` publish-op for MERGED ones (unblocking `depends_on`), and if the task was `manual_action_required` finalizes it to `DONE` via the same `finalize` path `merge-task` uses. CLOSED (not merged) PRs are only reported — no status change. The probe and publish-op are pure reuse of existing primitives.
...
- **`worc prs`** — list every task whose orchestrator PR is **open and un-merged** (a completed `pr` publish op with no completed `pr_merge`). DB-only and read-only by default, exactly like `status` (`open_readonly`, no providers/git/network — [cli.py:1226-1236](../../src/wastech_orchestrator/cli.py#L1226-L1236)). Columns: `task_id`, `title`, `status`, `branch`, `pr_url`. Optional `--check` enriches each row with **live** GitHub state (`verify_pr_state` / `pr_merge_state`: open? mergeable? conflicting?) — the only mode that touches the network. Optional `--sync` reconciles externally-merged PRs: for each open-PR task calls `verify_pr_state`; MERGED → writes `pr_merge` publish-op + if task was `manual_action_required` finalizes to `DONE` via the `finalize` path; CLOSED (not merged) → reports only, no status change. **Dry-run by default** (prints what would be written); `--yes` to actually write. Idempotent: re-running on an already-recorded PR is a no-op. This is the "catchup" counterpart to `merge-task`: instead of performing the merge, it acknowledges a merge that already happened on GitHub.
...
- **[cli.py](../../src/wastech_orchestrator/cli.py)** — сабпарсер `prs` (по образцу `status`, [cli.py:248-249](../../src/wastech_orchestrator/cli.py#L248-L249)) + `cmd_prs` (`open_readonly`, DB-only печать; флаг `--check` обогащает живым состоянием через `verify_pr_state`/`pr_merge_state`; флаг `--sync` — reconcile-путь, см. ниже). Диспетч в `main()`. Опционально: сабпарсер `tasks` + `cmd_tasks` (вся таблица, фильтр `--status`).
- **`--sync` reconcile-путь (в `cmd_prs`)**:
  - Открывает обычное соединение (не `open_readonly` — нужна запись), но только при `--yes`; без флага — dry-run (показывает что будет сделано, ничего не пишет).
  - Для каждой задачи из `find_open_pr_tasks()` вызывает `verify_pr_state`: `MERGED` → (dry-run: выводит строку «would record pr_merge for {id}»; `--yes`: пишет `pr_merge` publish-op через тот же путь, что `merge_pr` в `_auto_merge`, + если задача `manual_action_required` — финализирует в `DONE` через существующий `finalize`-путь); `CLOSED` (не merged) → только строка «PR closed without merge: {id}», статус не меняет; `OPEN` → пропускает (уже в очереди).
  - Идемпотентно: задача с уже существующим `pr_merge`-op не попадает в `find_open_pr_tasks()` → no-op.
- **Тесты:** `find_open_pr_tasks` (с `pr`/без `pr_merge` → в списке; с `pr_merge` → нет; без PR → нет); `cmd_prs` DB-only печать и ветка `--check` с фейковым `gh`-раннером; `--sync` dry-run (ничего не пишет, выводит план); `--sync --yes` MERGED-ветка (записывает `pr_merge`, финализирует `manual_action_required` → `DONE`); `--sync --yes` CLOSED-ветка (только выводит, статус не меняет); идемпотентность (повторный `--sync` на уже записанной задаче — no-op).
...
- **Питает `depends_on`**: успешный `merge-task` → `pr_merge_state` = `MERGED` → зависимые задачи становятся eligible (см. task-dependencies.md). Сегодня это умеет только `auto_merge` или ручной мерж на GitHub.
