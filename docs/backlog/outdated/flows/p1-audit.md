...
**Статус реализации (2026-06-19): ВСЕ ПУНКТЫ ИСПРАВЛЕНЫ.** 7 findings + 6 missing-coverage (тесты добавлены вместе с findings) + 2 secondary закрыты; каждый пункт ниже несёт блок **ИСПРАВЛЕНО** с описанием и тестами. Шов F3 закрыт в P1-объёме (`fresh_disposable`); durable lineage/`lineage_affinity`/`resume_own_lineage` остаются P2.2 по плану. Гейт зелёный: `ruff` + `mypy src` + полный `pytest`. Расхождения с предположениями аудита зафиксированы по месту (F1 → `MANUAL`-resumable вместо `failed/`; F4 → fail-closed вместо миграций; Secondary 2 → документирование вместо переименования).
...
**ИСПРАВЛЕНО (2026-06-19).** Выбран resumability-preserving вариант (origin-aligned: legacy имел resumable-publish через гранулярные статусы; plan.md §Риски опирается на идемпотентность `publish_operations` для resume) вместо git-хирургии с переносом `done→failed`. В `PublishNodeRunner.run` ([publish.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/src/wastech_orchestrator/core/flow/nodes/publish.py)) пост-finalize git-операции обёрнуты: `GitCommandError` → `complete_node_run(status="failed", error_class="publish_failed")` + `NodeManualRequired` → движок уводит в `MANUAL_ACTION_REQUIRED`. Файл консистентно остаётся в `done/` (код закоммичен, публикация не завершена), `rerun --continue` идемпотентно до-завершает push/PR. Это снимает и Additional #1 (не требуется перекоммичивать audit). Тесты: node-level `test_publish_git_failure_after_finalize_raises_manual` ([test_flow_node_runners.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/tests/core/test_flow_node_runners.py)) + e2e `test_publish_failure_after_finalize_is_manual_not_stranded_done` ([test_orchestrator.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/tests/core/test_orchestrator.py)) — закрывает MC2. Расхождение с предположением аудита (ожидался `failed/`): сознательно выбран `MANUAL`-in-`done/` как более точный и resumable исход; зафиксировано здесь.
...
**ИСПРАВЛЕНО (2026-06-19).** Добавлено поле `NodeInputs.route_override: Mapping[Stage, ProviderId]` ([base.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/src/wastech_orchestrator/core/flow/nodes/base.py)), заполняется из `p.task.agents` в `build_node_inputs` ([wiring.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/src/wastech_orchestrator/core/flow/wiring.py)); `RouterPort.resolve_route` расширен необязательным `override` (+обновлён docstring); `AgentNodeRunner`/`EvaluatorNodeRunner` зовут `resolve_route(stage, self._in.route_override)`. Тесты: e2e `test_task_agents_override_reaches_live_route` (override implementation→codex даёт `route_source=task_override`, `route_primary=codex` в `node_runs`) — закрывает MC1; +wiring-unit assertion `route_override == task.agents`. mypy/ruff/pytest зелёные.
...
**ИСПРАВЛЕНО частично — P1-объём (2026-06-19).** Граница соблюдена по плану ([p1-engine.md:204](/Users/a1234/Documents/GitHub/wastech-orchestrator/docs/backlog/flows/p1-engine.md:204), plan.md §Решения): durable lineage + `lineage_affinity`/`resume_own_lineage` остаются P2.2; в P1 честно исполнен `fresh_disposable`. Добавлены хелперы `editing_session_id` / `persists_session` в [base.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/src/wastech_orchestrator/core/flow/nodes/base.py): только `editing_lineage` читает и пишет provider-keyed in-memory сессию; `fresh_disposable` (и `resume_own_lineage` до P2.2) получает `session_id=None` и не пишет сессию обратно. `AgentNodeRunner._build_request`/`_update_session` и `EvaluatorNodeRunner._build_request` переведены на хелперы. Эффект: `review` (fresh_disposable) больше не наследует editing-сессию `implementation` И не затирает её — что заодно делает in-memory `lineage_affinity: fixing→implementation` устойчивее (fresh-узлы не загрязняют map). Тесты (MC6): `test_fresh_disposable_node_does_not_inherit_or_leak_session`, `test_editing_lineage_node_continues_and_persists_session` ([test_flow_node_runners.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/tests/core/test_flow_node_runners.py)). mypy/pytest зелёные, интеграционный сьют без регрессий. **Остаток (P2.2):** durable lineage-стор, истинная `lineage_affinity`-привязка к конкретному узлу, `resume_own_lineage` — вне P1.
...
**ИСПРАВЛЕНО (2026-06-19).** Выбран greenfield-aligned fail-closed (не миграции). `_enforce_schema_version` ([state_store.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/src/wastech_orchestrator/state_store.py)): при `0 < user_version < DB_SCHEMA_VERSION` — `IncompatibleStateError` с явным «delete state.db / fresh workspace» (на обоих путях open, до stamping). `current == 0` (новая/pre-versioning) по-прежнему адаптируется (`_SCHEMA` создаёт текущую форму). Переписаны вводящие в заблуждение комментарии v5/v6/v7 (теперь явно: «описывают форму текущей `_SCHEMA`, не действия `_migrate`; `_migrate` аддитивен; старые БД отвергаются, а не мигрируются»). Тест (MC5): `test_pre_v7_incompatible_shape_is_refused_not_stamped` — БД v6 со старой `provider_attempts(stage_run_id)` отвергается обоими open-путями, версия не штампуется (крах-на-записи невозможен). Существующие version-gate тесты (stamp/adopt-0/refuse-newer) зелёные.
...
**ИСПРАВЛЕНО (2026-06-19).** Добавлен публичный хелпер `subtask_spec_path()` в [decomposition.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/src/wastech_orchestrator/core/decomposition.py) (единый источник имени `NN-<slug>.md`, переиспользован и в `write_subtask_artifacts`). `_fan_out_subtasks` ([orchestrator.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/src/wastech_orchestrator/core/orchestrator.py)) перед регионом каждой подзадачи присваивает `inputs.subtask_spec_path` активного спека (и сбрасывает в None перед post-region phase). Роли `roles/implementation.md` / `roles/fixing.md` теперь ссылаются на `{subtask_spec_path}` + `{subtask_order}`/`{subtask_count}` («subtask N of M», условная фраза — пусто в single-unit). Тест (MC3): `test_decomposed_subtask_spec_path_reaches_implementation_prompt` — у 2 подзадач prompt узла implementation содержит `01-first.md`/«subtask 1 of 2» и `02-second.md`/«subtask 2 of 2». **Дополнительное расхождение, найденное по пути (Additional #6):** `inputs.subtask_count` вычислялся в `build_node_inputs` ОДИН раз до запуска flow (decomposition решается в planning ПОЗЖЕ) → всегда `None` на live-пути; «subtask N of M» молча терял `M`. Исправлено: `_fan_out_subtasks` выставляет `inputs.subtask_count = p.decomposition.n`. Отдельное поле в `AgentRunRequest` не добавлялось (канал через prompt достаточен — KISS).
...
**ИСПРАВЛЕНО (2026-06-19).** Выбран sync-вариант (engine остаётся источником истины; legacy-колонки зеркалятся), как менее инвазивный, чем переписывание всех read-путей. Два шва: (1) `save_flow_checkpoint` теперь принимает `fix_iterations` и пишет его в `tasks.fix_iterations` на каждом чекпоинте ([recorder.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/src/wastech_orchestrator/core/flow/recorder.py) передаёт `run_state.fix_iterations`) — live `status` во время прогона; (2) `_sync_counters_from_run_state` ([orchestrator.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/src/wastech_orchestrator/core/orchestrator.py)) перед каждым терминалом в `_engine_run` зеркалит `run_state` → `p.counters` (`fix_iterations` глобальный + `test_fix`/`review_fix`), так что терминальный `_transition`→`save_counters` и ledger пишут актуальные значения (нет clobber обратно в 0). Тест: `test_fix_iterations_synced_to_operator_surfaces` — после 1 test-fix цикла `tasks.fix_iterations == 1` и ledger-запись `fix_iterations == 1`. Обновлены 7 синтетических `save_flow_checkpoint`-вызовов в тестах (новый обязательный параметр; чекпоинт зеркалит счётчик задачи). Остаток корректности (bounded termination) не затронут — он и так работал на `loop_counters`.
...
**ИСПРАВЛЕНО (2026-06-19).** `_engine_apply_skills` теперь персистит выбор в `logs/<task>/selected_skills.json` (`_persist_selected_skills`, поля name/description/path, регистрируется как артефакт); `_restore_engine_inputs` на resume читает его обратно в `p.selected_skills` + `inputs.skill_paths` ([orchestrator.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/src/wastech_orchestrator/core/orchestrator.py)), симметрично восстановлению plan/diff/review. Обновлён doc-комментарий `_Pipeline.selected_skills`. Тест (MC4): `test_resume_restores_planning_selected_skills` ([test_recovery.py](/Users/a1234/Documents/GitHub/wastech-orchestrator/tests/core/test_recovery.py)) — прерывание на `current_node=implementation` с персистнутым skill, `resume()` → request узла implementation несёт путь `safe-change/SKILL.md`. (Additional #5: частичная митигация через `plan.md` была и раньше; теперь восстановлен и отдельный канал `skill_reference_paths`.)

### Missing Coverage — вердикты

Все шесть **подтверждены** и **ЗАКРЫТЫ (2026-06-19)** добавленными вместе с соответствующими findings тестами: MC1 → `test_task_agents_override_reaches_live_route`; MC2 → `test_publish_failure_after_finalize_is_manual_not_stranded_done` (+ node-level); MC3 → `test_decomposed_subtask_spec_path_reaches_implementation_prompt`; MC4 → `test_resume_restores_planning_selected_skills`; MC5 → `test_pre_v7_incompatible_shape_is_refused_not_stamped`; MC6 → `test_fresh_disposable_node_does_not_inherit_or_leak_session` + `test_editing_lineage_node_continues_and_persists_session`. Исходные пробелы (на момент аудита):
...
6. **Найдено при фиксе F5: `inputs.subtask_count` всегда `None` на live-пути.** `build_node_inputs` вычисляет `subtask_count` из `p.decomposition` единожды до запуска flow, а decomposition решается планировщиком позже → `M` в «subtask N of M» терялся даже после wiring пути. **Исправлено** (`_fan_out_subtasks` выставляет `inputs.subtask_count = p.decomposition.n`).
7. **Найдено при фиксе F6: skip-fixing не уходит «сразу в manual», а гоняет bounded test-fix-петлю до cap.** Тест `test_skip_fixing_routes_to_manual_on_failure` ассертил `fix_iterations == 0` — но это был **stale-счётчик** (тот самый баг F6), маскировавший реальное поведение движка. Движок доменно-нейтрален и НЕ спец-кейсит «нет fixing → сразу manual» (это было доменное знание legacy): при skip-fixing он берёт объявленное ребро `testing fail→fixing` (заряжая `test_fix`), скипает `fixing` (no-op), возвращается в `testing`, и так до cap `max_fix_cycles` → manual. **Терминальный исход (MANUAL) сохранён** — capability не потеряна; меняется лишь число (теперь честное) и факт повторного прогона checks N раз. Тест обновлён на честное `fix_iterations == max_fix_cycles` + комментарий. **Не чинилось** (вне scope аудита и осознанно по принципу «ноль доменного знания в движке»): если нужен именно мгновенный manual при выключенном fixing — это выражается ДАННЫМИ во flow (условное ребро `testing fail→manual` при `fixing` disabled), а не доменным спец-кейсом в движке. Кандидат в [follow_ups.md](/Users/a1234/Documents/GitHub/wastech-orchestrator/docs/backlog/follow_ups.md).
