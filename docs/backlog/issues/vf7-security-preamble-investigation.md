...
Status: **shipped 2026-07-24** (implemented per §7; originally an investigation-only analysis) Date: 2026-07-24 Owner: Vladimir Makarevich Related: [VF-5](runtime-validation-findings.md) (repo-instruction injection rollback), [VF-6](runtime-validation-findings.md) (`disable_read_isolation` escape hatch), [WRI-011](wri-011-freeze-agent-inputs.md)

**Implementation note (shipped 2026-07-24).** Built as designed in §2.1/§7: content in a Core module `core/flow/security_preamble.py` (`build_orchestrator_security_preamble`), a neutral `AgentRunRequest.security_preamble` field prepended at `build_effective_prompt` (`preamble → role → footer`), threaded via `NodeServices` (agent/evaluator) and the supervisor's own wiring, resolved once in `orchestrator._security_preamble()`. **The operator toggle (§4/§7) was intentionally NOT shipped** — per operator decision the block is unconditional Core behavior (always-on baseline + read-isolation-conditional reinforcement), justified because it is advisory text that restricts nothing and so is outside the security.md § MANDATORY escape-hatch rule (which targets functionality-limiting isolation). Everything else in §7's acceptance criteria holds. The §4/§7 discussion of the optional toggle is retained below as the original analysis.
