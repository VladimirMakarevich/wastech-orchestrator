...
- Making the orchestrator async. The fix is about _where_ synchronous work runs, not about converting the core.
- The single-bot-token long-poll constraint (409 Conflict) and the daemon-owned poll broker — a separate, larger item already noted under the [`worc listen`](worc-listen-remote-control.md) entry.
- Telegram formatting, redaction, and the ask protocol itself; all unchanged here.

## Implemented outcome

The interactive path awaits a worker-thread dispatch while `patch_stdout()` and the log-tailer task remain active. The synchronous headless dispatcher and its injected `run_cli` seam are unchanged, including command-local `SystemExit` handling and `--non-interactive` forwarding.

The Telegram adapter routes every async entry point through one coroutine-factory bridge. Calls without a running loop keep the direct `asyncio.run` path; calls made from a running loop construct and execute the coroutine on a dedicated worker thread. This includes messages, prompts, replies, identity/chat/webhook preflight calls, and the polling-health check.
