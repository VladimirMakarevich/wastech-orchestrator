...
| F30 — a finished decomposition strands its subtask specs in `pending/` | minor as observed, major on rerun | **fixed** | `core/orchestrator.py`, `task/parser.py`, `cli.py`, `worc-deco-task` guide |
...
## F27 — the provider CLI that could not read its own cache

The one finding in the trial whose lever is outside the repository, and the reason it is recorded here anyway is that reproducing it moved the finding two ways at once: the defect is worse than filed, and the repository was pointing operators straight at it.

**Reproduced from the artifact, not from a rerun.** `~/.codex/models_cache.json` on this host is fetched from the server, and today's copy contains no `base_instructions` field anywhere (`grep -c` returns 0) while closing its first model object at `line 97 column 5` — byte-for-byte the coordinates in the stderr the trial captured. So the probe is the cache file itself, and it says something the trial could not see from two failures: the field is **gone from the payload**, not intermittently absent.

**Which corrects the finding's headline.** "~33% of runs" reads as flakiness, and the entry itself notes the rate rose through the trial (0/3, then 1/2, then 1/1) without drawing the conclusion: that is not a random rate converging, it is a **one-way transition** with the server payload changing under a running trial. Past the transition a stale `codex` fails every run. Filed as `minor here`, and for the orchestrator it stays minor — the classification and fallback are the positive the entry credits them as — but the residual risk for an operator is not "one review in three costs a fallback", it is "codex does not run", and a route with codex as its only provider has nothing to fall over to.

**The fix is forward, and a pin would have been the wrong answer.** The entry's own lever line offers "pin or upgrade". Pinning is unavailable in the direction it implies: the other two releases on this host (0.139.0, 0.142.5) are further behind the same payload, so a pin can only pin the defect. Upgraded the standalone install `0.144.4 -> 0.152.1` (`codex update`; the older releases stay in `~/.codex/packages/standalone/releases/` and `current` is a symlink, so it is reversible). Nothing in the orchestrator needed a config change: the adapter launches `self._config.command`, the target repo sets no `command:` override, and the PATH entry `~/.local/bin/codex` is a symlink through `current`, so moving `current` is the whole deployment.

**Verified rather than assumed, because an upgrade across eight minor versions is exactly where an adapter breaks silently.** On 0.152.1: the cache re-fetches and parses with no `codex_models_manager` line on stderr; `codex exec` exits 0; both preflight probes still pass (`codex exec --help` exposes `-c/--config`, `codex exec resume --help` exposes `-m/--model` and `-c/--config`) and the resume grammar is unchanged (`codex exec resume [OPTIONS] [SESSION_ID] [PROMPT]`); `codex login status` exits 0; every flag the adapter's argv builder emits still exists, and the adapter's own argv shape (`codex --ask-for-approval never exec …` with the prompt on stdin) runs clean. Two of those are worth naming individually:

- **`--ask-for-approval` is still a global flag, not an `exec` one.** It is absent from `codex exec --help` on 0.152.1 — which looks like a break until you read [`codex.py`](../../../src/wastech_orchestrator/providers/codex.py), where the adapter deliberately places it _before_ `exec` because "current releases reject it when it is placed after the `exec` subcommand". It is present on `codex --help`. Recorded because the first probe of it produced a false alarm, and the next person to check a codex upgrade will grep the same wrong help page.
- **`default_permissions` is still live and still validated.** This is the security-relevant one: the adapter's read-isolation _is_ that key plus a generated profile, and a key that a new release silently ignored would take the `.worc`/`.env`/`state.db` read denials with it. On 0.152.1 `-c default_permissions=…` errors with `default_permissions requires a [permissions] table` (the mechanism is live), and `--strict-config` — which the adapter always passes — rejects an unknown key outright (`unknown configuration field …`), so a rename fails loudly instead of degrading. The pre-launch canary re-proves enforcement before every launch regardless.

**What landed in the repository.** [`README.md`](../../../README.md)'s "Officially supported CLI versions" line said `codex` **≥ 0.144.4** — the floor was the broken version, so an operator following the requirements section installed the defect and met an `exit 1` with no final message and no way to attribute it. It now names the verified version and, in one sentence, the signature and who is exposed: the stderr line to grep for, that the orchestrator turns it into `process_crashed` and falls over, and that a codex-only route has nothing to fall over to. That sentence is the "docs line so it is recognised" the finding's lever asked for; there is no troubleshooting page in the operator corpus to put it on, and the requirements section is where someone asking "which codex do I need" already is.
...
## F7 — the build the agent was told to run, and could not

The trial's last open `major`, and the only finding whose entry pointed at a lever that turned out to be the wrong one. Taking it up moved three things: what the defect is, what it costs, and which document owns the part that is left.

**Nothing about the observation is in doubt.** `npm run build` aborts under Claude's OS Bash sandbox with an esbuild deadlock (`exit 134`, `runService` waiting on a channel), and the Check Runner outside the sandbox runs the same command on the same tree green — twice per task, ~11.5s, with 200+ files written into `www/`. Same command, two environments, two outcomes; `grep -rn "sandbox|seatbelt|bwrap" checks/` returns nothing, because the Check Runner does not sandbox at all. The finding reproduced across two tasks, and the second time the agent's own diagnosis was more elaborate than the first (five runs, an 8 GB heap, a macOS crash report, `tsc --noEmit` as a control) — which is the property that makes this expensive rather than merely wrong: **every observation available inside the sandbox is consistent with the false answer**, so effort spent investigating buys more confidence in it.

**The entry's lever is where it went wrong.** "A direct, evidenced argument for step 4 (`unsandboxed_commands`)" is true of the evidence and false of the remedy. `npm run build` is not a command, it is a name that resolves through `package.json` — and `package.json` is ordinary agent-writable content: the write-deny set is only `.worc-io/`, `.git`, the git common dir, hooks and `tasks/` ([`runtime_layout.py:199-206`](../../../src/wastech_orchestrator/runtime_layout.py)). So `unsandboxed_commands: ['npm run build']` does not grant a build; it grants **whatever the agent later writes into that script, executed outside the sandbox**, where the `.git`/`.worc` denies that mechanically hold the publication invariant do not apply. That is a worse trade than the one the ADR already refuses: `gh` is banned from the key for handing over a publication path, and at least `gh`'s binary is not authored by the agent. The ADR's step-4 rule ("only proven incompatibility enters the key") does not catch this, because the incompatibility here is proven harder than most — so [the ADR now carries a second rule](../full-tool-access/full-tool-access-for-agents.md): a command whose behavior is defined by a file the agent can edit never enters the key, which excludes `npm`/`yarn`/`make`/`just` and every other manifest target **by construction** rather than case by case.

**What actually produced every cost in the entry was two role prompts.** The harm list is a wasted fix round, a false claim in the durable record, one such claim reaching the product, and — the escalation — an agent deleting `www/` inside `repo.local_path`, the operator's real checkout. None of it requires the agent to be able to build. All of it follows from the contract it was given:

- `implementation.md`'s **Verify** section told it to run the project's checks "and confirm they pass". For this repository half of that is impossible, so the instruction could only be discharged by explaining why — which is a diagnosis, and the only evidence available pointed at the host.
- `fixing.md`'s scope rule named exactly one category for a check that cannot pass: "a failure caused by a missing or incompatible **host toolchain**". A sandbox failure files itself there, and the rule then asks for it to be described plainly for a human — so the agent wrote "a human needs to look at this host's Node/build toolchain" in compliance. The rule that saved the code from being "repaired" is the same rule that shaped the false record.
- `review.md` had **already been fixed to say the right thing** — "your sandbox is not the gate's environment, so a failure there is evidence about the sandbox" — as part of the check-verdict repair. The sentence existed, was accepted, and was sitting in one of the three files that needed it.

So the repair is that sentence's rule, carried into the two edit nodes, plus the two things `review.md` had no reason to say:

- Both prompts now state that the gate is the Check Runner's and that a command failing under the agent is evidence about the sandbox, not about the machine — and ask for it in one sentence as a command that could not be run, explicitly **not** as a diagnosis.
- `fixing.md` gains the sandbox as a cause distinct from the host toolchain, with the Check Runner's recorded exit codes as the authority over the agent's own run, and says outright that investigating harder does not protect it here.
- `fixing.md` also gains the line the escalation earns: while isolating a failure, do not delete or regenerate build output, caches or directories — you are in a real checkout, not a scratch copy. The existing "revert any experiment" clause did not reach this, because the deletion happened during isolation, before anything was classified as an experiment.
- The gate loop stopped demanding the impossible: "re-run that same command to confirm it passes" now excepts a command that cannot run under the sandbox, and asks for the fix to be made from the recorded output instead.

No test pins the new wording, and that is the repository's own precedent rather than an omission: the sentence this one extends is not pinned either, nor are the scope block or the citation rule that landed in the same file. A role prompt is not a claim about a parser — the shape the help-text repair argued a text test is worth guarding — so there is nothing mechanical for an assertion to fail against.

**A fourth surface the entry does not list, found while fixing it — and it is a near-miss rather than a hit.** The false conclusion also reached a durable _advisory_ store: the Claude Code session that supervised the trial wrote it into its own project memory as a fact ("`npm run build` … dies with exit 134 … it is environmental, not something a change introduced", stamped `2026-09-02T01:48Z`, the same night `fixing` wrote the same sentence into the run record), together with a "how to apply" telling every later session to substitute `ngc --noEmit` and report that bundling could not be observed. Falsifying it took one command from an unsandboxed shell: `Application bundle generation complete. [10.396 seconds]`, exit 0 — the Check Runner's own 11.4s/11.5s, reproduced on demand. The memory is corrected. What makes this worth recording here rather than filing as a fifth cost is where it did **not** land: the orchestrator's own memory subsystem is off at the target (no `.worc/memory/`, no `memory:` block — an absent block is off), so nothing of this reached a store the orchestrator serves into prompts. Had it been on, this is exactly the shape that would have: a confident environmental claim, written by a node that investigated carefully, into a store whose known failure mode is serving proven-stale lessons into later prompts. It is the strongest argument in this record for fixing the contract rather than the sandbox — a wrong conclusion propagates through every advisory channel the product has, and only one of them was switched off by luck.

**What is deliberately not fixed, and now has an owner.** Whether the OS sandbox should be on at all under `strict_isolation: false` is a real question and this was not the place to answer it. It is now [an investigation in the backlog](../advanced-mode-sandbox-scope.md), opened because the operator's expectation ("no restrictions in advanced mode") turned out to disagree with both the code and, more interestingly, with the campaign's own record: `needs_sandbox` is computed from the tool plan and the host capability and never from the mode, while the campaign records the owner's variant B — advanced mode absorbs steps 2, 3 and 4 — beside a status line saying step 4 is out of the queue. That document also carries the observation worth more than either: write and network are **already fully open** in advanced mode (the whole volume, all domains), so the esbuild deadlock cannot be any of the four walls the campaign maps, and looks instead like a process/IPC axis of Seatbelt that no key addresses and no document names. If that holds, this class is curable by a step-2-shaped grant and step 4 was never the lever — which is the second time this finding's stated remedy would have been the wrong one.

## F30 — the half of the task lifecycle that only runs inbound

Found by the operator after the campaign closed, on the trial's own leftovers, and it is the one finding here whose evidence was sitting in the repository the whole time: five subtask specs in `tasks/pending/subtasks/` belonging to a task that finished weeks ago.

**Reproduced from the artifacts, then from the code.** `tasks/done/001-edge-to-edge-bottom-insets.md` is the root; `state.db` agrees the task is over (`status=done`, `subtask_count=5`, `subtasks_completed=5`, `source_path` already rewritten to the `done/` path); the five specs it names are still in the queue folder, and all five are git-tracked, so the repository permanently advertises pending work that is finished.

**The defect is an asymmetry, which is why it survived review.** Both halves of the lifecycle move task files, and only one of them knows a decomposition is a set. `promote_tasks` carries a root's specs in with it and states the invariant in its own docstring — "subtask specs move _before_ their root, so a root never appears in `pending/` without its specs" — with the ordering chosen to preserve it. `_relocate_task_file`, the terminal move, takes `(task_file, task_id, final)` and moves exactly one path. Nothing in it is wrong on its own; it simply never learned that a root can be a set, so the product declares the pair inseparable on the way in and separates it on the way out.

**And the cost is not tidiness, because a `subtasks:` ref is relative to the root file's own directory** (`root_dir = Path(task_file).resolve().parent`). Resolving the real manifest against the real repository, before and after the move it already made:

| root_dir                              | refs that resolve |
| ------------------------------------- | ----------------- |
| `tasks/pending` (where the specs are) | **5 / 5**         |
| `tasks/done` (where the root is)      | **0 / 5**         |

From there the chain is deterministic and already in the code: a missing ref returns `subtask_file_missing`, a reject runs `_quarantine`, and quarantine moves the task file into `.worc/tasks/rejected/`. `rerun_task` reaches that validation only after archiving the prior artifacts, clearing the exchange, **resetting the branch to base** and clearing the row — so a rerun of a finished decomposition destroys the attempt's git state and then files the finished root into quarantine, over a manifest that only broke because the orchestrator moved the root away from it. The rerun was not executed to confirm this: the proof is the resolution table plus three call sites, and running it would have reset a real branch to prove something the code already states.

**What is not affected, checked rather than assumed.** The stranded specs are inert for the scheduler: `select_pending` is `folder.iterdir()` filtered by suffix, so a `subtasks/` subfolder is never descended into. That is exactly the property the subfolder rule was introduced for, and it holds — which is also why the strandedness could sit unnoticed.

**The fix is the promote, run backwards.** `_relocate_task_file` now reads the manifest **before** the move (afterwards the refs resolve from nowhere, which is the defect itself) and re-anchors each spec on the root's new directory. Three details are deliberate:

- **The destination comes from the root's move, never from a per-file `lifecycle_destination`.** That function derives the tasks root from a file's parent, and a spec's parent is `subtasks`, which is not a lifecycle folder — so asking it where `tasks/pending/subtasks/01-a.md` belongs answers `tasks/pending/subtasks/done/01-a.md`, a lifecycle folder nested inside the queue. A test pins that this does not happen.
- **The root moves first, then the specs**, the reverse of promote's order and for the same reason: promote protects "never a root in `pending/` without its specs" because something will run it, while at the terminal end the task is already over and nothing resolves the manifest — so the ordering that matters is the one where an interrupted move leaves today's state rather than a new one.
- **Best-effort and idempotent, like the root's own move**, which "never lets a file-move failure mask the terminal outcome": a vanished spec, one already at its destination, and an occupied destination are skipped rather than forced.

`read_subtask_refs` moved out of `cli.py` into `task/parser.py` so both halves read the manifest through one function — the asymmetry began with only one of them having a reader.

**Quarantine is deliberately not included.** A rejected root goes to `.worc/tasks/rejected/`, inside the gitignored runtime home; dragging tracked spec files in there would remove them from the repository's view to tidy a folder. A rejected root is also not going to run, so the manifest it can no longer resolve costs nothing.

**Both `finalize` surfaces name the specs** (`+3 subtask specs`), for the reason the root's own move is named: these are tracked files the command dirties and deliberately does not commit, and several appearing in `git status` unannounced is worse than one. The dry run predicts the same count the real run moves, and an ordinary task prints no suffix at all.

**What the falsifying test is, since not all four are.** Neutering only the relocation call — leaving the announcement in place — fails `test_finalize_carries_the_subtask_specs_with_the_root` and passes the other three, which is the honest split: the nesting test guards the trap in the obvious implementation rather than the old behavior, and the two output tests guard the new surface. The operator's own five files were left where they are: they are the live reproduction, and moving them by hand would be a commit in a repository whose rules reserve that decision for them.
