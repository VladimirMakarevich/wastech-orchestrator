...
| **F7** — the agent cannot run `npm run build`; the Check Runner can | major | **not taken** | Three nodes wrote confident false conclusions about a broken host into the durable record, and one reached the product. Owned as step 4 of the `full-tool-access` ADR, deliberately out of this campaign. | `docs/backlog/full-tool-access/` |
...
| **F22** — a task spec contradicted by the repo it describes | minor | **not taken** | The target's own `docs/tasks/002-…md` claimed nothing documented the back-button ladder while two files already did. Task authoring in the target repository, not orchestrator behavior. | Target repo |
