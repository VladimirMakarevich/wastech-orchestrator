...
Status: **historical / closed** — all 14 steps sequenced and implemented (steps 1–13 landed on `main` 2026-06-26/27; step 14, orchestrator-memory, is implemented on `feat/memory-subsystem` and not yet merged to `main`). Archived 2026-07-02. Date: 2026-06-26 Owner: Vladimir Makarevich

> **Closure note (2026-07-02).** This roadmap is archived. Every step is implemented; the two residual threads are tracked in [follow_ups.md](../../follow_ups.md), not here: (1) **step 14 (orchestrator-memory)** is complete on `feat/memory-subsystem` but not merged to `main` (V1 disabled-by-default; V2/V3/V4 gated by a measured lift); (2) **step 13 (console)** shipped, and its post-review remediation — the `--force-full` topology fix (R1, done) plus the R2/R3 remainders — is captured in [cli-upgrade-remediation.md](cli-upgrade-remediation.md) and follow_ups. The perennial **Windows CI runner matrix** deferral also remains in follow_ups.

This document is a cross-ADR **implementation order**, not a new design. It captures the 14 backlog ADRs that were sequenced for build order so that shared seams (config schema, the CLI, the task-scan path, the watch loop, provider error handling, the Telegram channel, the supervisor/prompt-variable layer) were each touched in one deliberate pass instead of being re-edited by every feature. All steps are now implemented (step 14 on `feat/memory-subsystem`); their detail files live in [archive/done/](README.md). The goal is to minimize rework and merge conflicts, and to let each feature land on foundations the previous ones already established.
...
Step 1 (windows-cross-platform-support) is **implemented** (2026-06-26 — green dev loop on Windows + a cross-platform `worc watch`/`stop`/`restart` daemon; the Windows CI matrix is deferred, see its [outcome section](windows-cross-platform-support.md#implementation-outcome)). Steps 2–13 are implemented on `main` and archived in [archive/done/](README.md); step 14 (memory) is implemented on `feat/memory-subsystem`, pending merge.
...
| 14 | Orchestrator memory | [archive/done/orchestrator-memory.md](orchestrator-memory.md) |
