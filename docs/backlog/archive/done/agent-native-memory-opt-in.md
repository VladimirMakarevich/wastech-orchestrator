...
Status: **implemented** (2026-07-12) Date: 2026-07-11 Owner: Vladimir Makarevich

> **Implemented 2026-07-12** on `feat/agent-native-memory-opt-in`. The flag landed as `agents.providers.claude.allow_native_memory` (a **`ProviderConfig`** field, not `SecurityConfig`) — it is Claude-specific like `max_turns_gate`/`sandbox`, and `build_claude_argv` already receives the `ProviderConfig`, so no signature change was needed (the implementation note below anticipated a `SecurityConfig` kwarg). `build_claude_argv` now appends `_native_memory_deny_tools()` only when `not config.allow_native_memory`. Config schema bumped to **v29**. Deliberately **not** written by `config_writer` on a fresh install (so `upgrade-config` doesn't add it either) — it relaxes a security control, so it stays a conscious, hand-added opt-in documented in `config.example.yaml`, the operator guide, and `docs/configuration.md`. The two open questions below (Codex symmetric deny; hardened `.worc/` relocation) are recorded in [follow_ups.md](../follow_ups.md).
...
1. **Both-on semantics**: if the orchestrator's memory and native memory are both enabled, the agent receives two memory channels (the deterministic injected packet + the opaque native store). Confirm this is merely redundant/advisory and document it;
2. **Codex**: what native memory does Codex actually have, and does it need a symmetric deny (it is unmanaged today)? Separate investigation before any Codex knob.
