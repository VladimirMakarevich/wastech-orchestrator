...
- **`prs`/`merge-task`/`tasks`** ([orchestrator-driven-pr-merge.md](orchestrator-driven-pr-merge.md)) — **уже реализованы** (шаг 11); консоль подхватывает их как обычные `cmd_*`. Нюанс: `merge-task`/`prs --sync` отказывают, пока жив PID демона.
- **Жёсткий стоп на Windows (`taskkill`)** — `--force-full` сейчас POSIX-only; Windows-эквивалент (`creationflags=CREATE_NEW_PROCESS_GROUP` + `taskkill /F /T`) — отложенный follow-up (унаследован из [windows-cross-platform-support.md](windows-cross-platform-support.md)). До него на Windows жёсткий стоп деградирует в мягкий. Записать в [follow_ups.md](../../follow_ups.md).
