...
Status: **implemented** (2026-07-06) Date: 2026-07-04 Owner: Vladimir Makarevich

> **Implemented 2026-07-06** (branch `feat/worc-shell-reliable-control-surface`, stacked on `feat/reliable-stop-no-orphaned-agents`; gate green, real spawn smoke verified end-to-end on POSIX). **Root cause pinned:** the old auto-spawn appended the parent-parser flag `--log-file` _after_ the `watch` subcommand, so every spawned daemon died on an argparse "unrecognized arguments" error to a `DEVNULL`'d stderr — the auto-spawn had never worked. Delivered: passive entry (`attach_watch`, no auto-spawn/auto-claim); reliable `start_watch` (resolved `worc` console-script launcher, parent flags before the subcommand, child stdout/stderr captured to `.worc/logs/daemon-startup.log`, PID-file liveness verification with the captured error surfaced on failure); `up`/`watch` on-demand serving; **detach-on-`quit`** (daemon + task keep running); verb split `enqueue`/`up`/`watch` (dropped the `run`→enqueue alias); H1 (`--non-interactive` threaded into the forwarded `down`/`restart`); M1 (raw-remainder path parsing); M2 (default daemon log path); M3 (quit informs + is recoverable). **Resolved open questions:** (1) `quit` = detach always, no prompt (owner decision, consistent with H1); (2) one-shot `run <id>` **deferred** (owner decision — ship `up`/`watch` serving only; no second claim path / slot-guard); (3) `run` alias dropped (clean break, greenfield) so the verb is free for a future one-shot; (4) liveness signal of record = the PID file (poll to a 10s timeout, fast-fail on child early-exit). **Deferred:** the startup-capture log has no rotation (truncated per spawn); L1–L5 remain candidates; full Windows real-smoke stays pending (owner). See [follow_ups.md](../../follow_ups.md).
