...
- ✅ **Done: Phase 1 (High — data-loss & security; 2026-06-22).** #1 — `_resume_task` now guards `load_normalized` and degrades a corrupt/missing `task.normalized.json` to `manual_action_required` via a `_degraded_pipeline` built from the persisted `TaskRow` (new `test_resume_corrupt_normalized_artifact_marks_manual`). #2 — both `config.example.yaml` copies now list `gh pr merge`; loader default extracted to `_DEFAULT_DENIED_COMMANDS` and a `test_example_denied_commands_match_loader_default` drift guard added; `test_config_writer` asserts the merge denial. #3 — `pr_title` now round-trips through the normalized manifest (`test_pr_title_round_trips`). ruff + mypy + full pytest green; B05/B10/B16 docs synced.
...
- ✅ **Done: Phase 6 #B (`§NN` spec anchors; 2026-06-22).** AST-aware sweep — edits only docstring (`ast.Constant`) nodes and `tokenize.COMMENT` tokens, never code or non-docstring literals (so documented commands like `git add .` survive, code is untouched). Span-local substitutions, no global whitespace tidy. **0 `§` left in `src/` or `tests/`** (453 src + 132 test occurrences stripped). The **4** runtime/operator-facing literals (`recovery.py` `manual_reason`, `orchestrator.py` SlotBusy message, two `git_manager.py` messages), 13 author-shorthand tags (`§rerun`/`§finalize`/`§ auto-merge bypass`), and 3 vacuous "see spec" were hand-fixed. **Correction to the audit framing:** only **4** (not ~238) src `§` were in runtime literals — the earlier estimate wrongly counted docstrings. Live doc references (`.agents/rules/*`, real `.md` files) deliberately kept. ruff + ruff format + mypy + full pytest green. #23 (`_WORC_HOME`) remains deferred in [follow_ups.md](follow_ups.md).
...
| B | `§NN` spec anchors (481 across 73 files) | Confirmed | 6 | M | ✅ done (AST-aware sweep; 0 § left in src+tests) |
...
## Phase 6 — Docs & test hardening — ✅ DONE (2026-06-22)
...
**#B — `§NN` spec anchors.** ✅ DONE (2026-06-22) · Confirmed · 481 occurrences across 73 files (densest: `orchestrator.py` 55, `git_manager.py` 32, `cli.py` 23) · **M** · **Q-B**

- **Done as implemented (broader than Q-Bb, but safer):** an **AST-aware** transform edits only docstring (`ast.Constant`) nodes and `tokenize.COMMENT` tokens — never code, never non-docstring string literals — so it cannot mangle code (the blanket-regex risk: `tuple[X, ...]`, indented `) -> None:`, documented `git add .`) and cannot touch the 4 runtime literals blindly. Span-local substitutions (delete `(spec §N)`/`(§N)` parentheticals; `the §N <word>` → `the <word>`; `spec §N` → `spec`; `<file>.md §N` → `<file>.md` keeping the stable file pointer); no global whitespace tidy. Result: **0 `§` in `src/` or `tests/`** (453 + 132 stripped). Hand-fixed (judgment cases): the **4** runtime/operator-facing literals (dead-spec suffix dropped from the message — an improvement), 13 author-shorthand tags (`§rerun`/`§finalize`/`§ auto-merge bypass`), 3 vacuous "see spec". The one live `.md` cross-ref (`validator.py` → `security-ceiling.md §4`) kept its file pointer (section number dropped). **Correction to the audit:** only **4** (not the ~238 first estimated) src `§` were in runtime literals — the rest are docstrings (342) + comments (107). ruff + ruff format + mypy + full pytest green.
