...
Status: **implemented** Date: 2026-06-26 (implemented 2026-06-27) Owner: Vladimir Makarevich

> Implemented in config v22. Both gates ship off by default, fail-closed, and require `telegram.enabled` when on (preflight). Next-task gate (idea 27): `orchestrator.auto_mode.confirm_next_task`, gated in `cli.watch_once`. Max-turns gate (idea 29): `agents.providers.<id>.max_turns_gate`, in the agent node runner via `_invoke_with_turn_gate` reusing the durable `HumanGate`. See [configuration.md](../configuration.md), [telegram.md](../telegram.md), and blocks [B02](../functional/blocks/B02-watch-daemon-and-scheduling.md) / [B30](../functional/blocks/B30-flow-node-runners.md) / [B05](../functional/blocks/B05-configuration.md). Resolutions to the open questions below: **feasibility crux** — confirmed from code: the Claude adapter appends `--max-turns` on every invocation independent of `--resume`, so a resumed session gets a fresh budget (empirical CLI confirmation against a real account remains a recommended pre-production check). **Runaway continue** — no hard resume cap in v1 (operator decision): each continue needs a fresh approval and timeout→STOP bounds an unattended loop; a configurable cap is a follow-up. **Config placement** — provider-level `max_turns_gate` (next to `max_turns`), resolved orchestrator-side. **Schema version** — bumped to v22.
