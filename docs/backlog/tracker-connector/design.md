...
**Why.** `promote` is the atomic, refuse-to-overwrite step worc built for exactly this race. The audit commit stages only the task's own lifecycle file, so a staged file is never swept into a commit (`GitManager.commit_audit`). A file in `preparing/` is untracked and survives worc's branch checkouts. The connector therefore coexists with the single processing slot without knowing it exists.
...
**Decision.** State (SQLite), the connector log and its config live under `<repo>/.worc-connect/`; `worc-connect init` appends that directory to the tracked `.gitignore` the way `worc install` appends `.worc/` and `.worc-io/` (`RUNTIME_GITIGNORE_LINES`).
...
**Decision.** Task id `<prefix>-<item-number>` with a sequence suffix on re-trigger: `gh-142`, then `gh-142.2`, `gh-142.3` (worc's id grammar `^[a-z0-9][a-z0-9._-]{0,63}$` admits `.`). Branch `worc/<task-id>-<slug>` where the slug is the sanitized, lower-cased title truncated so the whole ref stays ≤ 50 characters — the soft cap above which worc silently falls back to its own generated name (`validation_gate.py`, `BRANCH_NAME_MAX_LEN`). The connector must know the branch to find the PR later, so it names it.
...
**Decision (worc side).** A new optional front-matter key `references:` — a list of 1..16 strings, each 1..200 characters, single-line, non-blank, scanned by the same `scan_frontmatter` as every other value; any violation is a Phase-A hard reject with a new `ValidationReason.INVALID_REFERENCES`. At publish, when a PR is about to be opened, worc appends to the body a section `## References` with one line per string, verbatim. The mechanism reuses the notice channel: `PublishNodeRunner._publish` passes the block to `GitManager.create_pr` next to `notice`, and `_body_with_notice` (or a sibling that appends rather than prepends) writes the annotated copy under the task's artifacts — the committed `summary.md` is not rewritten. `NormalizedTask` gains `references: tuple[str, ...] = ()`.
...
**Decision (worc side).** `_task_entry` adds `"pr_url": <url or None>` for rows that have one. Phase 02 verifies where the run records the URL (the ledger's `finalized` record carries `pr_url`; `TaskRow` may or may not) and reads it from wherever it already lives — no new column unless the row lacks it. The operator guide names the JSON entry shape as the scripting contract from that release on.
...
| `cli.py` | `_task_entry` gains `pr_url`. |
| `packaged/guide/` | task-fields page: `references:`; operations page: `worc list --format json` as the scripting contract. |
| `tests/` | gate tests for `references:`; publish test asserting the PR body ends with the block and the committed summary is untouched; `cmd_list` JSON test for `pr_url`. |

No change to `providers/`, `routing/`, `config/`, `security/`, the flow schema, or `state.db`.
...
2. `queued` / `running`: read `worc list --format json`; map `running` → `in-progress`; `done` → look for the PR; `failed` / `manual_action_required` → `failed`. A status of `pending` with no DB row is still `queued`.
...
4. A rejected task (`.worc/tasks/rejected/<id>.md` present, or `worc list` shows `failed` before any branch) → `failed`, comment carries the `reason` string from `validation_report.json` — the string only.
...
| `worc promote` exits non-zero | "already in pending" → `queued`; any other error → log, keep `staged`, retry next tick. |
| Task rejected by worc's gate | Row → `failed`; comment carries the `reason` string; label `worc:failed`. The connector never edits and re-queues a rejected file on its own. |
...
**worc (this repository).** Gate: `references:` valid, each rejection reason, injection-scan rejection, the key absent. Publish node + git manager: with `references`, the PR body passed to `gh pr create` ends with the `## References` block and the committed `summary.md` is byte-identical to before; with a `push`/`commit` cap the block is not built. `cmd_list --format json`: `pr_url` present and `null` when absent.

## Doc impact

- **worc, on this branch:** the packaged guide's task-fields page (`references:`) and operations page (`worc list --format json` as the scripting surface); `README.md` only if the task example gains the field (it need not); `docs/backlog/README.md` rows. Derived pages on the documentation branch likely affected: the task-authoring and operations pages, and the configuration reference for the JSON shape — breadcrumb for that separate task.
