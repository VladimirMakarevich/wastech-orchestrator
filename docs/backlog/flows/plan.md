...
Цель: движок исполняет граф; капабилити ядра обёрнуты как узлы **без смены механики**; движок воспроизводит текущее поведение implementation (golden-harness против существующих тестов).
...
- **P1.4** Паритетный flow-фикстура + golden-harness (двойной CI-прогон `_drive` ↔ движок).
- **P1.5** Удаление старого драйвера (`_drive`, реентри-диспетчер, `Stage`-как-конвейер) — только после зелёного P1.4.
