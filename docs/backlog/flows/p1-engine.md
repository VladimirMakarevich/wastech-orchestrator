...
- **Dangerous-diff guard (классификация)** в `AgentNodeRunner`: после `workspace-write`-узла ядро пишет diff (`{diff_path}`) и прогоняет `classify_dangerous_diff` (deletion/dependency) — core-owned, flow не отключает. Опасный diff пока **fail-closed → manual** (`NodeManualRequired`); полный durable approval round-trip — следующий кусок. `GitPublishPort`→`GitPort` (+`write_current_diff`/`changed_code_entries`).
...
**Остаётся в Step B (durable round-trip достоверно проверяется только end-to-end golden-harness'ом):** полный durable HITL/approval round-trip — обёртка `hitl` (Telegram round-trip), встроенный HITL refinement/planning (`node.hitl`, парс `parse_typed_stage_output`, persist/resume `interaction_path`) и **approval-ветка dangerous-diff guard** (`start_ask`/persist/resume/reconsider-on-denial); prompt_audit/heartbeat-наблюдаемость; реальная конструкция `NodeServices`/`NodeInputs` из `_Pipeline` + карта `node_id→Stage`; дремлющий `_drive_via_engine`-шов в `run_task`; и сам golden-harness dual-run (`_drive` ↔ `FlowEngine` на fake-CLI сценариях) + recovery-диспетчеризация.
