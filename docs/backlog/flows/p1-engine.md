...
**Остаётся в Step B (durable round-trip достоверно проверяется только end-to-end golden-harness'ом):** одна общая машинерия durable HITL/approval round-trip — обёртка `hitl` (Telegram round-trip), встроенный HITL refinement/planning (`node.hitl`, парс `parse_typed_stage_output`, persist/resume `interaction_path`) и dangerous-diff guard после `workspace-write` (`write_current_diff`+`classify_dangerous_diff`+approval); prompt_audit/heartbeat-наблюдаемость; реальная конструкция `NodeServices`/`NodeInputs` из `_Pipeline` + карта `node_id→Stage`; и дремлющий `_drive_via_engine`-шов в `run_task` (не вызывается, пока golden-harness Step B не задействует его).
