...
**This is an open call, not a settled one** — it is in the follow-ups under "Needs a decision", with the one-line change if the operator wants the downgrade and the paragraph of `architecture_design.md` that has to go with it.

**Acceptance, honestly.** Criterion 1 is met except for its `read-only` clause (above). Criteria 2 and 3 are met — 10g's gate is in the graph, and no engine path filters a commit by name (nothing on the publish path was touched). Criterion 4 (the citation checker's declared manifest field) was already closed by [P1.6](p1-6-citation-checker-strictness.md). The end-to-end "commits its report and no intermediate" cannot be observed from a unit suite: it is the campaign's pending checkpoint re-run.
