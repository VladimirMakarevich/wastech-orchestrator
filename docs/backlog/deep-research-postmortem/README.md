...
Status: **in progress — steps 1-7 implemented (P0.1, P0.3, P0.2, P1.6, P1.5, P1.4, P1.7); P2.8 piece 2 shipped early with P1.4; P1.4a accepted and next in line, then P2.9, P3.10** Date: 2026-07-25 Owner: Vladimir Makarevich
...
| P1.4a | [Read-only git evidence for an audit node](p1-4a-read-only-git-evidence.md) | Spun out of P1.4 change 3, **accepted, runs next (ahead of P2.8)**: a `read-only` node that may run the read-only git verbs. Not a flag — Claude prevents writes by tool absence and three guards ride on that | new capability | both providers + schema/validator/preflight |
...
| 8 | P1.4a | — | Spun out of P1.4 when its change 3 turned out to redefine what `read-only` means rather than add a flag. Accepted 2026-07-26 and moved ahead of P2.8: the missing history evidence is judged to have contributed to the two release-blocking false negatives. Independent of every other item. |
| 9 | P2.8 | P0.3 | Do not inline content through a redactor that mangles benign identifiers. |
| 10 | P2.9 | P2.8 | If the node stops writing the blueprint, the blueprint must still reach `synthesis` some other way. |
| 11 | P3.10 | — | Independent throughout; 10d resolves itself once P0.1 ships. |
