...
- The operator escape hatch [security.md](../../../.agents/rules/security.md) mandates needs a declared surface (a node field or a config key), which the flow schema, the validator, the ceiling check and the preflight all have to learn.

That is an ADR of its own, and bundling it with a graph split and four new prompts would put a security-envelope change behind a review of prompt wording. Deferred with the analysis above; nothing in it is blocked by this item.
