...
- **§3 read-only-ness.** `build_sandbox_settings` grew `deny_write_root`; the adapter passes the clone root for a read-only attempt, so the sandbox — not the allowlist — is what holds the node to reading. The git control-state fingerprint now also covers a granted read-only attempt rather than workspace-write alone — a shell is what makes `.git` reachable, so the profile alone was the wrong condition. `_apply_post_edit_guard` stays off.
...
- **Neither a stray write nor `.git` drift parks — decision 2 is read literally.** Both events on a granted read-only node warn and let the run continue: `NodeOutcome.read_only_write` drives a console warning plus a ⚠️ `TRACE_READ_ONLY_WRITE` trace for a working-tree write, and `NodeOutcome.read_only_git_drift` does the same with `TRACE_READ_ONLY_GIT_DRIFT` for control-state drift. The outcome stays `done`, and nothing downstream is handed a change either way. Implementation first shipped the write half only and left drift on `NodeManualRequired`, reasoning that the two are different events — a stray file is inert, while a planted hook or `insteadOf` is executed by the orchestrator's own next git command. **The operator confirmed the literal reading on 2026-07-27**, so the drift consequence was downgraded to a warning. Three things keep that from being a silent loss: the WRI-009 fingerprint is untouched (detection was never the negotiable part), the signal carries the drift's redacted aspect-level summary rather than a bare flag — the warning is the whole mitigation here, so "hooks: hook 'post-commit' added" has to reach the operator, not "something changed" — and the warning says to stop the run and discard the clone rather than merely to inspect it. A `workspace-write` node doing the same still parks; the exception is scoped to the class the grant applies to.
- **The read-only fingerprint moved to the reporting bracket.** Because a granted read-only node now reports rather than raises, its capture/compare pair lives in `_granted_shell_before` / `_result` — the same bracket as the working-tree check — instead of at `_invoke`'s compare site, which keeps only the workspace-write path that parks. That is what removed `_can_run_commands`: one bracket per consequence, and both read-only signals are computed in one place from one snapshot. The bracket also spans every attempt of a node, so a HITL re-run needs no accumulation.
...
- **The three `deep_research` analysis nodes declare it.** They are the nodes this document was written about, and leaving the capability reachable-but-unused would mean hand-editing a packaged flow to get any benefit. Inert by default. §4 asked only for the declaration _surface_, so this was implementation initiative and was flagged as such; **the operator confirmed it on 2026-07-27** — the declaration alone grants nothing, `security.allow_git_evidence` is an operator-config key nobody flips by accident, and the packaged flow doubles as the worked example of the field. Re-weighed against the same day's decision that control-state drift on such a node warns instead of parking: it does not change the answer (the sandbox below is untouched and enabling is still one deliberate act), but it does mean that if the switch is ever turned on, these three nodes get a shell whose `.git` drift no longer stops the run — the push-redirection residue in the campaign's `follow_ups.md` watch items. Their prompts' capability-conditional "delivery evidence" wording is deliberately **left as it is** — it stays correct after the grant, since the same flow still runs without a shell whenever the switch is off, the provider is Claude on native Windows, or the node is routed somewhere without a sandbox.
