...
4. Reconsider the model for the finalize turn specifically. The per-step layer is cheap advisory work and Sonnet is right for it; the finalize turn is the operator's only artifact and is worth the stronger model. This is separable from VF-16's global model discussion because `SupervisorConfig` currently has one model for both roles — splitting observe/finalize models is already proposed in supervisor-observation-cadence-p1.
