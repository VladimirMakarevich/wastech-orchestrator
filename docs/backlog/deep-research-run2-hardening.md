...
Orchestrator default (`packaged/`), with the target copy refreshed via the [`upgrade-flows`](upgrade-flows.md) / `install --reconfigure` path. On run 2's numbers this returns **~$21** and ~34 minutes, and it makes the P1.4 contract true rather than aspirational.
...
- **Target re-sync.** Everything here lands in `packaged/`; getting it into `wastech-mdlint` is the existing [upgrade-flows.md](upgrade-flows.md) work, and this document adds to that queue rather than duplicating it.
