...
**Answered by [P3](supervisor-disable-switch-p3.md) (implemented 2026-08-03):** `supervisor.enabled: false` is the single global switch, and the narrowing check is skipped when the layer is off — so `implementation.yaml`'s declared `events` no longer fails the task, and the after-claim sting no longer applies to "turn the layer off everywhere". The narrowing rule itself is unchanged for anyone who keeps the layer on, and the question this entry predicted ("why is a global `none` refused?") is now answerable with "because `none` is the narrowest cadence, not the absence of a layer — use `enabled: false`". Delete this entry once the derived `main` docs record it.
...
The warm-resume branch was deleted outright rather than put behind a key. A toggle would have forced a config-schema bump inside a phase that changed no schema, and a second code path in the tests forever — for a greenfield project that owes no backward compatibility. A warm auto-fallback "if the packet fails to build" was refused for a sharper reason: it would restore non-determinism in the one path P0 makes reproducible, and mask the build failure. The rollback is `git revert`; the only fallback is the orchestrator's deterministic minimal summary, and a failed build is recorded as `packet_built: false` on the `supervisor_final` row. Still true as written — [P3](supervisor-disable-switch-p3.md) adds no toggle for _how_ finalize runs; it adds one for _whether the layer exists at all_, and it upgraded the deterministic fallback this entry names from a stub into a real report (shipped 2026-08-03: `write_minimal_summary` is gone and one renderer writes the body on every terminal). [P1](supervisor-observation-cadence-p1.md) adds no `finalize.session` key either (P1-D8) — a single-valued key is dead config.

### No `finalize.enabled` / `handoff.enabled` keys (P1) — still true per phase; the case for a whole-layer switch arrived (P3)
...
**Half of that arrived on 2026-08-03** and is now [P3](supervisor-disable-switch-p3.md). Two things keep this entry alive rather than deleted. The **per-phase** keys stay refused — P3 adds exactly one `supervisor.enabled` for the whole layer, so `finalize.enabled` / `handoff.enabled` / `observe.enabled` remain non-goals and P3 lists them as out of scope. And the objection itself is not waived but **answered**: P3's prerequisite is a real deterministic report (rendered from P0's `PacketFacts`), so a run with the layer off still produces a pull-request body — the reason the toggle was refused no longer holds, which is different from the reason having been wrong. P3 shipped on 2026-08-03; delete this entry only when someone also files the per-phase case.
