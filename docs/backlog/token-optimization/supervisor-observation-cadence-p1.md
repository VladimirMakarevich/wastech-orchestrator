...
- **`worc upgrade-config`** — плоские ключи заносятся в `_REMOVED_KEYS` (`config/upgrade.py:127`), поэтому команда их стрипает и печатает `- supervisor.model (removed in this schema version)`, а новые ключи доливает из шаблона.
