...
| X2 | Doc-sync scope on `dev`: P0 §In scope 5, P1 §In scope 7 and P2 §Areas all name `docs/worc_architecture.md` / `docs/configuration.md`, which are `main`-only. | [AGENTS.md](../../../AGENTS.md) forbids creating them on `dev`; the in-scope list must instead name what lives here (`.agents/rules/`, `README.md`, `packaged/config.example.yaml`, `packaged/guide/`, packaged flows) plus a PR doc-impact note. |
