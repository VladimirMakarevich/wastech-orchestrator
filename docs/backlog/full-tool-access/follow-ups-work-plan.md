...
| Ам4-1 | fixed (решение Ам4-В1) | `allow_native_memory` открывает только per-project memory store: `_native_memory_optin_deny_tools` денаит config home по глубине (`//home/*`, `//home/*/*`), потому что язык деналов — globs и «кроме» в нём нет; неопределимый home — ошибка preflight (`native_memory_optin_error`). Четыре носителя (ось `write`, комментарий поля и changelog схемы, `config.example.yaml`, `guide/config/security.md`) говорят одно: открывается подкаталог, слои кредов и настроек над ним остаются write-denied. **Superseded 2026-08-24 ([Ам-5, Блок Б](phase-am-5-ordinary-state-and-codex-home.md)):** и опт-ин, и деналь на config home удалены целиком (`allow_native_memory` — config v39); описанная здесь механика больше не существует. |
...
### Пересмотр трёх Пре3-находок фазой Ам-5 — 2026-08-24

Фаза [Ам-5](phase-am-5-ordinary-state-and-codex-home.md) (Блок А) вернулась к трём уже закрытым находкам, потому что снятие парковок меняет их основание. Записано здесь, а не в [audit-follow-ups.md](audit-follow-ups.md): тот файл — реестр находок и стоит на бюджете `SIZE-001`, а исходы живут в этом плане.

| id | Что изменилось |
| --- | --- |
| `Пре3-8` | **Исправлено кодом.** Первая починка передала `mode` на возобновлении, и этого было мало: `_prepare_current`/`_prepare_existing` перечитывают базу из `HEAD`, что верно ровно один раз — на resume, после того как прогон уже коммитил в ветку, база уезжает вперёд, и отчётное изменение задачи схлопывается до незакоммиченного остатка. Носитель — аддитивная nullable `tasks.base_ref` (`DB_SCHEMA_VERSION` 25, по образцу v24 `push_url_digest`): ставится при первой подготовке ветки, дальше только восстанавливается. Взято в Ам-5 как цена Т5.1 — после снятия парковок сдвинутый `HEAD` и resume становятся штатным путём, а не редкостью. |
| `Пре3-9` | **Основание снято.** Дефект был в том, что перечни классов узлов «выглядят полными». После Т5.1 вердикт один для всех классов, и перечень в `architecture.md` перестал быть перечнем — вводить в заблуждение больше нечем. |
| `Пре3-10` | **В силе, и стало опорой.** `_log_lost_drift` остаётся целевой формой для любого пути, уходящего через `raise` без outcome. Именно поэтому Т5.1 вынесла дрейф пишущего узла наружу значением (`_invoke` отдаёт сводку, `_result` кладёт её на `NodeOutcome`), а не выбросила его вместе с парковкой. |

Отдельно: `Пре3-1` в части «вызвать гейт от точки отсчёта перед `commit_code`» на момент разведки Ам-5 **уже был закрыт** — `_require_dangerous_diff_approval` стоит непосредственно перед `commit_code` в узле публикации, — поэтому Т5.2 его не дублировала.
