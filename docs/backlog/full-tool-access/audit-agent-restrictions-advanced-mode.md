...
- `tool`-узел запускается через тот же `run_process` (argv-список, обязательный таймаут), имя резолвится fail-closed в реестре `<repo>/.worc/tools/`: путь должен лежать внутри каталога (симлинк наружу отклоняется), быть обычным файлом и быть исполняемым. Программа получает на stdin **только** allowlist путей и `args` флоу — никогда сырой session id. Core **записывает** её `findings`/`data`, но никогда не применяет: нет пути, где возвращённое значение приводит к git- или state-записи. Про скиллы важно держать в голове, что механизмов **два**, и ограничен из них только первый.

**Оркестраторский слой скиллов** (`core/skills.py`, модель «read-only reference paths»):

- **Обнаружение только по git** — `git ls-files` по всему дереву, из него берутся файлы с именем `SKILL.md`. Нетрекнутый `SKILL.md` в инвентарь не попадает, и `node_modules`/build/vendor не попадают бесплатно, потому что они нетрекнутые.
- **Никогда не исполняются.** В футер контекста уходит буквально `- skill (read-only reference; advisory, do not execute): <path>`. Claude-only инструмент `Skill` оркестратором не используется сознательно — чтобы оба провайдера вели себя одинаково.
- **Выбор provenance-closed.** Токен принимается, только если резолвится ровно в один **уже обнаруженный** скилл — по repo-относительному пути или по уникальному `name`. Бэйр-нейм, который носят два скилла, → `ambiguous` и отбрасывается. Ни оператор, ни супервизор не могут внести файл, которого скан не нашёл сам.
- **«Предлагает предлагающий, решает Core».** При `skills.dynamic: true` супервизор получает один ход на предложение `node → skills`: схема жёсткая (`additionalProperties: false`, токен ≤512 символов), парсер защитный (мусор → `{}`), и Core всё равно валидирует каждый токен по инвентарю — плохое предложение может максимум не добавить ничего. Установка пишет `dynamic: false`, то есть слоя по умолчанию нет.
- **`skills.strict`** — только про операторские пины: `false` (дефолт установки) — предупредить и пропустить нерезолвленный пин; `true` — остановить задачу в `manual_action_required`. Динамическое предложение с несуществующим именем всегда просто фильтруется.
- **Чтение при скане ограничено и denied-aware**: ≤262144 байт на файл, только frontmatter, и файл, попавший под `denied_read_paths` (`.env`, `secrets/**`), не читается вообще.
- **Описание скилла — untrusted репозиторный текст** и в промпт предложения попадает обрезанным до 200 символов. Тело скилла в промпт не инлайнится никогда.
- **Выбранный скилл фризится пакетом и публикуется редактированной копией.** Агент читает иммутабельную копию из обмена, а не живой файл: иначе `workspace-write` узел переписал бы себе правила посреди задачи, и следующий ревьюер/супервизор/resume получил бы другие инструкции, чем были проверены на старте.
- **Кэпы пакета fail-closed** (переполнение — ошибка, никогда тихое усечение): ≤64 файлов, ≤262144 байт на файл, ≤2097152 байт на пакет. Отказ также на: `SKILL.md` в корне репозитория (замыкание было бы всем деревом), симлинк/reparse point/hard link/спец-файл/NTFS ADS, коллизию ключей по case-fold/NFC, и две выбранных скилла с одинаковым именем папки.
- **Секрет-гейт на каждый файл пакета** перед публикацией в обмен; не-UTF-8 файл — ошибка. `skill_map.json` персистится, поэтому resume не перепредлагает, а дайджест манифеста сверяется перед переиспользованием сессии провайдера.

**Родной механизм скиллов CLI в этой конфигурации не ограничен** — см. пункт 9 честного списка ниже.
...
1. **Config home провайдеров не защищён ничем.** `~/.claude` / `$CLAUDE_CONFIG_DIR`, `$CODEX_HOME` — ни write-деналя, ни read-деналя, ни при каком значении любого ключа, и настройки, которая вернула бы деналь, нет (решение владельца 2026-08-24). Там лежат учётные данные (`$CODEX_HOME/auth.json` — файл на диске) и конфигурация, которую CLI загрузит сам при следующем старте (хуки в `~/.claude/settings.json`, MCP-команды в `$CODEX_HOME/config.toml`, а также `skills/`, `plugins/` и `AGENTS.md`/`CLAUDE.md` внутри config home). Причина решения: деналь ломал собственную механику провайдера — standalone-пакет Codex держит бинарь **внутри** `$CODEX_HOME`, и `apply_patch` не мог исполнить свой sandbox-helper, из-за чего не проходил ни один патч.
...
9. **Родной механизм скиллов провайдера открыт, и оркестратор его не ограничивает.** `--setting-sources project` — это ровно тот переключатель, который открывает project settings: хуки, MCP, плагины и **скиллы репозитория**; под `--setting-sources ""` (strict + read-isolation ON) их нет. Плюс без `--tools` существуют все встроенные инструменты, включая `Skill`, и ни в одном деналь-списке его нет. Значит `.claude/skills/**` целевого репозитория — обычные инструкции, которые CLI подхватывает сам: они не проходят инвентарь, фриз, редакцию и кэпы из 5.11, а `~/.claude/skills/` ещё и доступен на запись (пункт 1), то есть узел может положить туда скилл, который CLI загрузит на следующем старте. Одна оговорка про пределы этого аудита: `Skill` не попадает и в `--allowedTools`, а как `acceptEdits` в headless обходится с существующим, но не авто-одобренным инструментом, из кода не выводится — это живая проба, а не утверждение. У Codex симметрично: `trust_level="trusted"` плюс пользовательский `config.toml`, и в режиме сняты все `--disable` (пункт 10).
10. **Все feature-поверхности Codex включены** (2.3), включая `memories` — персистентное хранилище вне нашей редакции и аудита.
11. **Неизвестный будущий инструмент Claude авто-одобряется**, потому что гейта существования нет и перечислить набор нельзя.
12. **На хосте без OS-песочницы пола нет вообще.** Нативная Windows; Linux/WSL2 без `bubblewrap`+`socat`. У Claude там `needs_sandbox` = false, файл `--settings` **не пишется** — остаются только инструментальные деняли, а любая команда шелла достаёт и `.git`, и `.worc`. `preflight` и лог прогона печатают это громкой строкой, и прогон продолжается.
13. **Дрейф не паркует** (5.9) — сообщается и продолжается.
...
| Скиллы оркестратора | пути, не исполнение | пути, не исполнение | инвентарь + фриз пакета + обмен | токен вне инвентаря отбрасывается; живой `SKILL.md` не читается |
