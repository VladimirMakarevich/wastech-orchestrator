...
Status: **Шаг 0 — implemented (контракт приёмки и актуальная семантика: [requirements-step-0.md](requirements-step-0.md); закрытие follow-ups Пакета 1: [follow-ups-work-plan.md](follow-ups-work-plan.md)); Шаги 1, 1b, 2, 3, 4 — proposed для конфигураций без advanced mode; advanced mode вынесен в [requirements-advanced-mode.md](requirements-advanced-mode.md) и переопределяет части этого ADR** — условия, при которых очередной шаг берётся в работу, лежат в README этой папки. Date: 2026-08-05 Owner: Vladimir Makarevich Revised: 2026-08-20 (Пакет 1: исправлены launch-critical gate, policy-built environment, prefix/env-file граница и cache-path diagnostics; текст Шага 0 ниже хранит мотивацию, а нормой служат требования и шипнутый гайд) Revised: 2026-08-17 (Codex advanced-mode audit: сужены формулировки про публикацию, full-access режимы, fallback equivalence и `strict_isolation`) Revised: 2026-08-13 (сверка с кодом при формировании требований: `schema_version` уже 35, call-site'ов `build_child_env` шесть, а не семь, союз из 22 имён лежит в шаблоне, а не в сгенерированном конфиге; исправлены все относительные ссылки, сломанные переездом документа в подпапку) Revised: 2026-08-05 (ревью: исправлены четыре ошибки в описании механизмов, добавлены платформенные ветки, закрыты два открытых решения, добавлен реестр принятых ослаблений) Аудит: 2026-08-05 — сплошная перепроверка по коду и живым пробам `codex-cli 0.144.4` / `claude 2.1.222`; исправлены неверные грамматика `network.domains`, премиса Шага 4 про Go-CLI, механизм снятия `--tools`, все числовые оценки и две ссылки на строки; добавлены находки «unknown key парсится молча», «клон под глобальным `/tmp` снимает deny-вырезы» и «инвентарь инструментов CLI не перечислим»; по итогам аудита владелец закрыл все шесть решений (см. одноимённый раздел)
...
Штатно запрещена **публикация**: `git commit`, `git push`, `gh pr create`, `gh pr merge` остаются в `security.denied_commands`, а запись в служебные области (`.git`, `.worc`, `tasks/`, exchange, `.env`, `secrets/**`) остаётся write-denied там, где есть песочница/permission profile. Важная поправка advanced-mode ревью: `denied_commands` — не пол, а трение и телеметрия, особенно на Codex, где они не проецируются; продуктовый мандат «публикует оркестратор» не равен механической невозможности любого `push` на любом хосте. Точная новая формулировка пола — в [requirements-advanced-mode.md](requirements-advanced-mode.md) ТA.1.

**Решение владельца (2026-08-05): цель приоритетна.** Там, где механизм изоляции стоит на пути цели, он ослабляется осознанно — но не молча: каждое ослабление получает отдельный ключ конфига, громкую строку в `worc preflight` и запись в разделе [«Принятые ослабления»](#принятые-ослабления-реестр) с указанием, что именно перестаёт удерживать границу. Не меняется продуктовый мандат: штатные commit / push / PR делает только оркестратор, а не агент; механические гарантии зависят от конкретного хоста и провайдера и описываются отдельно.

Чего эта задача **не** делает: не включает `sandbox: danger-full-access` у Codex и не включает `--permission-mode bypassPermissions` у Claude. Advanced mode пошёл дальше: эти provider full-access селекторы удаляются из продукта, потому что утилит они не добавляют, а единственный их уникальный эффект — снять provider boundary и пропустить пробы пола.
...
Помимо агента команды в целевом репо запускает сам оркестратор — `checks.command_sets`, и запускает их **без песочницы, без write-guard и с полной сетью**, argv-списком и с policy-built окружением через `build_child_env(security)` ([`check_runner.py`](../../../src/wastech_orchestrator/check_runner.py)). Отсюда два практических вывода:
...
- `security.extra_environment` из Шага 0b попадает в общий `build_child_env`, поэтому agent/check/tool пути не расходятся. Orchestrator-owned `git`/`gh` строят окружение отдельной config-aware функцией: они получают allow-list и присваивания, после чего publication-retargeting имена вычищаются. В advanced mode полный parent env относится только к agent/check/tool путям, а `git`/`gh` остаются на allow-list.
...
- **Публикация штатно остаётся за оркестратором.** Ни один шаг не даёт агенту мандат публиковать и ни один механизм продукта не ожидает от агента `commit` / `push` / PR. Механически это держится локальной write-изоляцией `.git` там, где песочница существует, и детектом на нашем `origin`; `denied_commands` остаётся трением/телеметрией, но не полом, потому что на Codex не проецируется и в shell обходится.
...
- **Никаких секретов через конфигурационные environment-механизмы** — секретоподобные имена запрещены в allow-list и assignments. В strict mode agent/check/tool и orchestrator-owned `git`/`gh` остаются на allow-list; advanced mode отдельно передаёт agent/check/tool родительское окружение целиком, кроме имён из `.worc/.env`, а `git`/`gh` остаются на allow-list.

## Шаг 0 — окружение: три механизма

**Исходная проблема.** В strict mode дочерний agent/check/tool процесс получает только имена из `security.allowed_environment`, и этот список **заменяет** дефолт, а не дополняет его. Дефолт при этом OS-зависимый: кросс-платформенная база из 6 имён плюс OS-launch essentials — на POSIX это 9 имён, на Windows 19. `PATH` есть, поэтому `jq` и `fd` находятся; а `dotnet` без `DOTNET_ROOT`/`NUGET_PACKAGES`, `nvm`/`mise` без своих переменных и часть CLI без `LANG` работают не так или не работают. Шаг 0 реализовал exact/prefix forwarding и явные assignments; advanced mode использует отдельную семантику, описанную в требованиях.
...
Ключ **заменяет** дефолт целиком, а дефолт OS-зависимый. Числа проверены вызовом самого кода: `default_allowed_environment("Linux")` → 9, `("Windows")` → 19, их union → 22. «19» — это Windows-дефолт, а не размер союза; не перепутать при правке. **Поправка 2026-08-13:** союз из 22 имён лежит в шипнутом **шаблоне** ([`config.example.yaml:145`](../../../src/wastech_orchestrator/packaged/config.example.yaml), закреплено `test_example_allowed_environment_covers_every_os_default`), а **сгенерированный** `install`-ом `config.yaml` содержит только дефолт хост-ОС — `config_writer` пишет `list(default_allowed_environment())`, то есть 9 имён на POSIX ([`config_writer.py:143`](../../../src/wastech_orchestrator/install/config_writer.py), закреплено `test_os_launch_essentials_are_allowlisted_for_the_install_host`). Так что «в вашем `config.yaml` уже лежит кросс-платформенный союз» — неверно для свежей установки, и правило «редактируйте то, что лежит в файле» надо читать вместе с этим фактом. Ниже — правильная база; POSIX-only список из первой редакции на Windows выбрасывал `SystemRoot`, а без него Node-овый `claude.exe` падает с `0xC0000409` до вывода чего-либо, и preflight сообщает «CLI did not succeed».
...
Раз главный риск ключа — «оператор случайно снёс нужное имя», конфиг без `PATH` отклоняется при загрузке, а на Windows `worc preflight` и старт `run` / `watch` / `rerun` дают launch-critical FAIL без `SystemRoot` в обоих режимах. `PATH` и `SystemRoot` нужны always-on `git`/`gh`; в strict mode они дополнительно нужны agent CLI.
...
Что это даёт против 0a: значение видно в конфиге (**поправка 2026-08-13:** и **только** в конфиге — окружение сегодня не пишется ни в один артефакт прогона, в артефакте запроса лежат `argv`, промпт и пути, [`_adapter_base.py:710`](../../../src/wastech_orchestrator/providers/_adapter_base.py); решение владельца 2026-08-13 — в Шаге 0 это не менять), поведение одинаково на любой машине, забытый `export` больше ничего не ломает. Правило разрешения конфликтов: `extra_environment` применяется **после** `allowed_environment` и перекрывает одноимённое проброшенное значение; `PATH` из этого правила исключён (перезапись `PATH` целиком — это способ подменить любую утилиту, её должен делать только адаптер, как Codex делает для своего Windows-хелпера).

Реализованный шов принимает целиком `SecurityConfig`: agent/check/tool call-site'ы строят окружение общей функцией, а orchestrator-owned `git`/`gh` используют отдельный config-aware builder и финальный scrub перенацеливающих публикацию имён. `extra_environment` применяется после forwarding и перекрывает одноимённое значение, на Windows — без учёта регистра. Подстановка `{repo}` не вводилась: оператор пишет конкретное значение.
...
В strict mode это по-прежнему **allow-list по именам**: элемент списка может быть exact name или prefix pattern. Поддерживается только одна завершающая `*`; одинокий `*` и звёздочка в другом месте отклоняются, а lower-level expansion одинокого `*` fail-closed. Каждое раскрытое имя проходит через `is_sensitive_key`; отброшенные имена печатаются как WARN, чистое раскрытие — как INFO. На Windows matching и dedupe case-insensitive. Имя, загруженное из `.worc/.env`, prefix pattern в strict mode не выдаёт неявно: для этого нужны exact entry или assignment. В advanced mode паттерны описывают только окружение orchestrator-owned `git`/`gh`, что прямо печатается в preflight и run/resume log.
...
- in-clone cache должен быть проигнорирован. Оркестратор проверяет это через Git и, если нужно, добавляет split-only path-list элементы в clone-local `.git/info/exclude`; preflight может выполнить этот write-effect, а каждый task start повторяет repair. Само исходное path-list значение целиком в exclude не пишется;
- на узле, чья effective write policy не даёт запись в этот путь, кэш всё равно не заработает — наличие shell само по себе ничего не разрешает.
...
**Второе правило того же шага, и оно строже первого: в ключ не попадает команда, тело которой агент может переписать.** Первое правило («только доказанная несовместимость») этот класс пропускает, и трейл против `wastechlab-mobile-template` показал, как именно. `npm run build` там детерминированно умирает под песочницей Claude (дедлок esbuild, `exit 134`), а Check Runner вне песочницы ту же команду на том же дереве выполняет успешно — несовместимость доказана жёстче, чем была доказана совместимость `gh`: две задачи, пять под-прогонов, контроль через `tsc --noEmit`, опровержение прямо из `check_runs`. По первому правилу это имя проходит. Но тело `npm run build` лежит в `package.json`, а он агенту доступен на запись: deny-набор — только `.worc-io/`, `.git`, git-common-dir, hooks-dir и `tasks/` ([`runtime_layout.py:199-206`](../../../src/wastech_orchestrator/runtime_layout.py)). Агент перепишет `"build"` и получит произвольное исполнение **вне** песочницы — ровно то, за что из продукта удаляются `danger-full-access` и `bypassPermissions`, только теперь по опт-ину и под именем сборки. Это хуже случая `gh`: там наружу отдаётся готовый путь публикации, но сам бинарь не авторства агента.

Поэтому правило формулируется по построению, а не по случаю: **имя годится для ключа только если поведение команды не задаётся файлом, который агент может изменить.** Это исключает целиком класс script-runner'ов — `npm run *`, `yarn *`, `pnpm *`, `make *`, `just *`, `nx *`, любую цель из манифеста репозитория — и исключает его **до** того, как для очередного члена класса кто-то соберёт доказательство несовместимости. Допустимым остаётся имя настоящего исполняемого файла, чьё поведение не читается из клона (`docker`). Обход через wrapper или вложенный shell остаётся обходом и попадает в реестр, как и раньше.

Отдельно к сведению: сам этот случай не обязан кончиться Шагом 4. Стены 3 и 4 в advanced mode уже открыты (разрешён весь том, разрешены все домены), поэтому дедлок esbuild не объясняется ни одной из четырёх осей — он похож на процессно-IPC-ось seatbelt, которую ни один ключ не адресует и ни один документ этой кампании не называет. Если это так, лечится он входом уровня Шага 2, а не снятием изоляции. Проверка вынесена в отдельную задачу — `advanced-mode-sandbox-scope.md` в родительской папке бэклога (названа текстом: ссылка отсюда в проверяемую часть корпуса не нужна) — и она же обязана свести противоречие между вариантом В («режим вбирает Шаги 2, 3 и 4») и статусом «Шаг 4 выведен из очереди».
...
Не ослабляется продуктовый мандат: commit / push / PR штатно делает только оркестратор, а агент не получает такого задания. Механическая часть формулируется уже: `.git` остаётся write-denied там, где песочница существует, публикация в наш `origin` удерживается детектом, публикация наружу за пределы нашего `origin` не удерживается, когда сеть и креды доступны. Для Шагов 1–3 без advanced mode секреты по-прежнему не попадают в дочернее окружение; для advanced mode действует отдельная редакция и исключение `.worc/.env` из [requirements-advanced-mode.md](requirements-advanced-mode.md). Известные обходы на опт-инах выше остаются зафиксированной ценой, а не скрытыми stop-gate.
...
- **`schema_version`.** **Поправка 2026-08-13:** сейчас **35** ([`schema.py:198`](../../../src/wastech_orchestrator/config/schema.py)) — версию подняли под другую задачу, поэтому вся нумерация ниже сдвинута на единицу. Каждый шаг несёт свою версию: 0b → 36, 1 → 37, 2 → 38, 3 → 39, 4 → 40 (и 36 достаётся тому шагу, который поедет первым, а не обязательно 0b). Все — формат-добавки, fail-open, `upgrade-config` подсевает ключи из шаблона.
- **Производные документы `main`.** Затронуты `configuration.md` (новые ключи), `operations.md` (вывод preflight), `flow-authoring.md` (что теперь может `read-only` узел), `worc_architecture.md` (граница «песочница vs паттерны» описана там неверно так же, как была здесь). На `dev` эти файлы не существуют — doc-impact оставить строкой в описании PR (отдельного реестра-файла для этих заметок больше нет).

## Что осознанно не делаем

- **Codex `sandbox: danger-full-access`.** Один `return` ([`codex.py:371`](../../../src/wastech_orchestrator/providers/codex.py)) выбрасывает не только filesystem/network profile и канарейку, но и соседнюю config/tool-surface изоляцию, которую нормальная ветка добавляет вместе с ним: `default_permissions`, `--ignore-user-config`, project `trust_level="untrusted"`, отключение hooks/multi-agent/computer/browser/apps/plugins. Сохраняются `--ask-for-approval never`, policy-built child env, stdin/prompt isolation, redaction, process-quiescence и отдельный `web_search="disabled"` при `network_access: false`, поэтому это не «отключить вообще всю безопасность». Режим действительно может сделать работоспособными операции, которым мешали filesystem/network restrictions, но цена — снятие почти всей provider boundary, включая `.git`; advanced mode не использует его и [Ам-1](phase-am-1-drop-full-access.md) удаляет этот селектор из продукта.
- **Claude `--permission-mode bypassPermissions`.** Тоже не добавляет инструментов. Плюс отказывается стартовать под root/sudo на Linux/macOS — то есть оркестратор в контейнере под root просто не запустит узел. Отдельно к сведению: сегодня этот операторский override переживает `--tools` (инструментов всё равно нет), а после Шага 1 не переживает ничего — он обнуляет и запреты. Advanced mode не использует его и [Ам-1](phase-am-1-drop-full-access.md) удаляет этот селектор из продукта.
...
- **Инверсия окружения** («всё минус чёрный список»). Единственное место, где мы не идём за целью, и обоснование — в конце Шага 0: ни одной утилиты это не добавляет, а утечка происходит от самого наличия значения в окружении. **Поправка 2026-08-13:** это остаётся верным для Шага 0, но сам вопрос **переоткрыт** решением владельца — как отдельный открытый вопрос об «advanced mode» (полная свобода по окружению, `extra_environment` и инструментам под ответственность оператора), который рассматривается **после** фаз Шага 0: [open-question-advanced-mode.md](open-question-advanced-mode.md).
- **Ослабление `strict_isolation` целиком для старых Шагов 1–4.** Ни один из этих точечных шагов этого не требует: там, где нужен опт-ин, вводится узкий ключ (`allow_unsandboxed_shell`, `codex_allow_network_writes`), а не выключение изоляции на весь прогон. Advanced mode отдельно решил обратное: режимом стал сам `strict_isolation: false`, и его контракт живёт в [requirements-advanced-mode.md](requirements-advanced-mode.md), а не в этом историческом ADR.
...
| [`config/schema.py:198`](../../../src/wastech_orchestrator/config/schema.py) | `CONFIG_SCHEMA_VERSION = 35` (было 34 на момент первой редакции) |
