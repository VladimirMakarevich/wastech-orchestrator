...
Status: **Шаг 0 — accepted, в работе по фазам; Шаги 1, 1b, 2, 3, 4 — proposed; advanced mode — открытый вопрос** Date: 2026-08-13 Owner: Vladimir Makarevich
...
| [open-question-advanced-mode.md](open-question-advanced-mode.md) | **Открытый вопрос, не принят:** advanced mode — один переключатель «ничего не конфигурировать», полная свобода по окружению, `extra_environment` и инструментам, под ответственность оператора. Стоит после всех фаз Шага 0 намеренно. |
...
После фаз идёт [open-question-advanced-mode.md](open-question-advanced-mode.md) — он **не фаза и не принят**: это зафиксированная идея «переключатель полной свободы» с перечнем того, что перестанет действовать, и шестью вопросами, которые надо закрыть, прежде чем она станет фазой. Стоит последним потому, что Шаг 0 как раз и выясняет, сколько конфигурирования реально остаётся.
