...
Status: **Шаг 0 — accepted, в работе по фазам; предусловия пола — пять блокирующих дефектов, приняты в объём и не начаты; advanced mode — требования согласованы, все 12 вопросов закрыты 2026-08-17; Шаги 1, 1b, 2, 3 — остаются предложением для конфигураций без режима; Шаг 4 — требует отдельного решения, он несовместим с полом** Date: 2026-08-13 Owner: Vladimir Makarevich Revised: 2026-08-17 (очередь пересобрана после решений по advanced mode)
...
| [requirements-advanced-mode.md](requirements-advanced-mode.md) | **Заключительная фаза кампании:** решения владельца по всем 12 вопросам с отклонёнными вариантами, определение пола через свойства, требования ТA.1–ТA.9, отрицательные требования, **пять блокирующих предусловий**, тесты и живые пробы. |

## Порядок

```mermaid
flowchart TD
    S0["Шаг 0 — окружение<br/>фазы 0.1 … 0.4"] --> P["Предусловия пола<br/>5 блокирующих дефектов"]
...
**Предусловия пола идут вторыми и обязательны.** Это не «улучшения», а пять мест, где заявленная единственная гарантия сегодня не проверяется или не удерживается: пол не доказан ни одной пробой; `push` и PR агента не детектируются вообще; чужая публикация **усыновляется** и записывается как успех оркестратора; самокоммит обнуляет гейт опасного диффа; креды читаемы агентом прямо с диска. Три из пяти — дефекты и без всякого режима, поэтому часть работы окупается сразу. Перечень с цитатами по коду — в [requirements-advanced-mode.md](requirements-advanced-mode.md).
