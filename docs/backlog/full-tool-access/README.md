...
Status: **Шаг 0 — реализован полностью (фазы 0.1–0.4, 2026-08-18); **предусловия пола — этап реализован полностью**: Пре-1 и Пре-3 2026-08-18, Пре-2 2026-08-19 (`DB_SCHEMA_VERSION` 23); **Ам-1 и Ам-2 реализованы 2026-08-19** (`schema_version` 38; закрытие приватного `Read` сделано в Ам-2, как и планировалось) — с этого момента `strict_isolation: false` означает advanced mode; advanced mode — все 17 вопросов закрыты (1–12 разбором, 13–17 независимым ревью того же дня), после Codex-ревью 2026-08-17 открыты два точечных решения по реализации; Шаги 1, 1b, 2, 3 — остаются предложением для конфигураций без режима; Шаг 4 — требует отдельного решения, он несовместим с полом** Date: 2026-08-13 Owner: Vladimir Makarevich Revised: 2026-08-17 (очередь пересобрана после решений по advanced mode; затем — поправки независимого ревью, см. раздел поправок)
...
| [phase-pre-2-remote-side.md](phase-pre-2-remote-side.md) | Фаза Пре-2 — удалённая сторона: отпечаток `ls-remote` / push-URL / PR по каждой попытке и восстановление вместо усыновления (четыре случая). **Реализована 2026-08-19** (`DB_SCHEMA_VERSION` 23); конфликтный вариант случая 4 паркует — решение владельца. |
...
- Тот же fixture, профиль **без** выреза `.git` (`write_guard=None`): канарейка падает `CONFIGURATION_ERROR` на пробе `tasks/`, созданный пробой файл удалён. То есть подставленный профиль без выреза действительно ловится, а не проходит молча.
...
| Пре-3 | Прогон, где агент коммитит своё изменение: гейт всё равно спрашивает про удаление, а публикуется отревьюенное состояние | нужен целевой репозиторий и платный прогон живого CLI | механизм доказан интеграционными тестами на **реальном git**: самокоммит виден гейту, декомпозиция не спрашивает повторно, наборы путей отчёта и входа гейта совпадают внутри подзадачи, чужой коммит усыновляется с громкой строкой. Живой прогон добавил бы одно: что между работой агента и гейтом `HEAD` не двигает какой-то другой механизм |
