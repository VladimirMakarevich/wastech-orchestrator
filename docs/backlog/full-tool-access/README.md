...
Status: **Шаг 0 — accepted, в работе по фазам; предусловия пола — четыре блокирующих дефекта плюс закрытие `Read`, приняты в объём и не начаты; advanced mode — все 17 вопросов закрыты (1–12 разбором, 13–17 независимым ревью того же дня), открытых решений нет; Шаги 1, 1b, 2, 3 — остаются предложением для конфигураций без режима; Шаг 4 — требует отдельного решения, он несовместим с полом** Date: 2026-08-13 Owner: Vladimir Makarevich Revised: 2026-08-17 (очередь пересобрана после решений по advanced mode; затем — поправки независимого ревью, см. раздел поправок)
...
| Документ | Роль |
| --- | --- |
| [full-tool-access-for-agents.md](full-tool-access-for-agents.md) | ADR: цель, четыре стены, механизмы, границы, реестр принятых ослаблений, решения владельца и таблица живых проб. **Дизайн-обоснование, а не контракт приёмки.** Устарел в двух местах, и второе несущее: раздел «Что осознанно не делаем» — в части full-access режимов (их решено удалить, а не «не использовать»); и **сам инвариант публикации** — ADR четыре раза утверждает «коммит / пуш / PR делает только оркестратор» безусловно, в том числе «`denied_commands` остаётся полом под всем остальным», что прямо противоречит решению по вопросу 10 и всему ТA.1. Читать эти абзацы как действующие нельзя. |
