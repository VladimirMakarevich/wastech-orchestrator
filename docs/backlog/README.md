...
| [Task-chain context propagation](task-chain-context-propagation.md) | Supervisor writes a focused `chain_summary.md` at finalize; dependent tasks receive predecessor summaries in planning prompt via `{predecessor_context}`. Covers linear chains from `depends_on` and `decompose`. Prevents duplication, contract breakage, and redundant exploration. | V1: linear chains only. Diamond deps, parallel tasks, and intra-task node summaries are out of scope. |
