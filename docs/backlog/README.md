...
| Document | Purpose |
| --- | --- |
| [implementation-roadmap.md](implementation-roadmap.md) | Cross-ADR **build order** for the 12 open ADRs — sequences them by shared seam (config, CLI, task-scan, watch loop, provider errors, Telegram, supervisor) to minimize rework and conflicts. Ordering only, not a new design. |
...
| [Orchestrator memory (cross-task reuse, defrag, autodream)](orchestrator-memory.md) | Supervisor-owned persistent memory across tasks — short-term / long-term / entity tiers, an operator-run defrag/cleanup command (manual or scheduled nightly/weekly, checked against the real codebase), and a bounded "autodream" self-cleanup between task ticks. Superset of the lightweight-memory slice. **Proposed (exploratory).** | No secrets in memory; not an unbounded context dump; supervisor stays advisory (Core decides); autodream self-edits must be bounded + audited and never touch an active task. |
...
| [Log management: `worc logs clean` and `logging.*` config](log-management.md) | `worc logs clean [--keep N]` to reclaim disk space from task artifact directories; `logging.level` and `logging.artifacts` (minimal/standard/full) in `config.yaml` to persist operator-trace verbosity and control which per-node files are written. **Proposed.** | `completed.jsonl` never deleted by default; redaction invariant holds at all levels. |
...
| Orchestrator memory (cross-task reuse, defrag, autodream) | [orchestrator-memory.md](orchestrator-memory.md) |
