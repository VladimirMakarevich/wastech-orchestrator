...
| [Branch mode (existing / current branch)](branch-mode.md) | Per-task `branch_mode: new \| existing \| current` (+ `branch_ref`) with a `repo.branch_mode` default, so a task can continue an existing branch, accumulate a chain of tasks on one branch, or run in the current checkout — plus a downgrade-only per-task `publish` modulator (`commit`/`push`/`pull_request`). | Branch mode is orthogonal to publishing (graph shape stays flow-level; `publish` only modulates the existing node, never adds one). Destructive git ops (delete/reset) run in `new` mode only; `head == pr_base` skips the PR but still pushes. Subsumes the per-task slice of "Dry-run without push". |
