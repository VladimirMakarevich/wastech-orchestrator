...
| [documentation-node.md](documentation-node.md) | Detail file for the `documentation` agent node in the packaged `implementation` flow (**shipped 2026-06-23**): on `review --accept--> documentation --> publish`, after the code is accepted an agent updates the target project's docs under the flow's workspace-write ceiling, using the same role-prompt/durable-lineage/commit machinery as the other nodes; runs once per task (incl. decomposed). Pure graph-shape change + one role prompt; no schema/config bump. Also generalized the dangerous-diff guard so a prior in-task approval of the identical diff is not re-prompted by the new second workspace-write node. |
