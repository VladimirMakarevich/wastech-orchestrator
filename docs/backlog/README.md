...
| [Multiple named editing lineages in one flow](multiple-editing-lineages.md) | Allow more than one durable editing session per execution unit, each shared across a distinct subset of `editing_lineage` nodes, so a flow can run two isolated editing tracks (e.g. code + spec) without leaking session context. Lineage key = `lineage_affinity or node.id` (no new field), finally making `lineage_affinity` route sessions at runtime. | Backward-compatible (every packaged flow has ≤1 affinity-less editing node); `state.db` version bump, no migration (greenfield); cross-provider resume + no-secrets invariants hold per lineage. |
