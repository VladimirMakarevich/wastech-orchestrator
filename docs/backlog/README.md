...
| [worc shell reliable control surface](worc-shell-reliable-control-surface.md) | `worc shell` auto-spawns the `watch` daemon at startup and it dies silently (stdout/stderr → `DEVNULL`, no liveness check, fragile `sys.executable -m` spawn), so the queue stalls with no signal. Make the shell passive-on-entry and on-demand: attach-or-idle (no auto-claim), reliable console-script spawn with liveness verification + surfaced error, `quit`=detach so tasks survive, verb split `enqueue`/`up`/`run` with a slot-guard; folds in §6 R2/R3 gaps H1/M1/M2/M3. | Shell stays a thin client (no orchestration logic); single-active-task per-instance and "commit/push/PR only orchestrator" unchanged; argv-list spawn, cross-platform, no secrets in the new startup log. From `wastech-mdlint` P4 testing. |
