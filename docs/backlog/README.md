...
| --- | --- | --- |
| [Rerun flow-drift reconciliation and fix-budget confirmation](rerun-flow-drift-and-budget-confirm.md) | `rerun --continue --from NODE` refuses on any whole-file flow-fingerprint drift, even changes upstream of `NODE`; make `--from` check only the subgraph reachable forward from `NODE` and resume through compatible drift automatically (no new flag), plus an interactive confirmation gate before resuming into an already-exhausted fix-loop budget. | Closes the deferred flow-fingerprint-drift item named in [follow_ups.md](follow_ups.md) (from the now-removed `universal-resume-recovery.md`); `state.db` stays the sole authoritative state, no manual SQL edits. |
