...
| --- | --- | --- |
| [Codex-primary correctness](codex-primary-correctness.md) | **🔴 Blocker для codex-primary.** F38: адаптер собирает `codex exec resume <ID> --cd …`, но в codex 0.142.5 exec-level флаги должны идти ДО подкоманды `resume` → все resume-узлы (supervisor/documentation/fixing) падают exit 2 и держатся на claude-fallback (при single-provider=codex — hard-fail). F39: `supervisor` имеет `model` без `provider` → под codex-primary уводит claude-модель на codex. Решение: переупорядочить resume-argv + version-guard в codex-probe; дать `SupervisorConfig` явный `provider` + валидацию model↔provider. | Ядро не знает CLI (правка в `providers/codex.py`); fail-fast лишь без fallback; кросс-платформенно. From `wastech-mdlint` P5-прогон p5-01 ([TEST-FINDINGS.md](../../TEST-FINDINGS.md) F38/F39). |
