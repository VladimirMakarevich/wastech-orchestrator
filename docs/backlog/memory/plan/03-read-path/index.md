...
- **`memory_path` prompt variable** — add to `ALLOWED_PROMPT_VARS` in `core/prompts.py`; populate in the node prompt-variable builders; it points at the per-stage packet, never the memory root.
- **Role-prompt references** — reference `{memory_path}` in the packaged role prompts (seeded into `.worc/flows/roles/` at install).
