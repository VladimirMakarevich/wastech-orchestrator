...
Status: **review complete — findings open** Date: 2026-07-01 Owner: Vladimir Makarevich — [task hub](index.md)
...
What remains: **one genuine dropped thread** (F1 — a step planned _inside_ V1 that fell between phases, with stale tracker text), **one connected risk** the per-phase notes do not join up (F2), a **security default worth a deliberate decision** (F3), a **rollback-fidelity edge** (F4), a cluster of **assert-by-design-but-untested** criteria (F5), and minor nits (F6).
...
| NFR5 / C1 / AC-SF1 — redaction before every write | ✅\* | single `_redact` chokepoint + audit rows redacted; planted-secret drill = **0 leaks** (\* but structural-only at runtime — see F3) |
...
| FR8 / AC-SF3 / AC-SF4 — hash-chained audit + snapshot/restore | ✅\* | `verify_chain`; rollback drill byte-identical (\* one first-created-file edge — see F4) |
...
### F1 — 🟠 The write-time validation "tighten" step was planned for V1 but never closed (tracker now stale)
...
### F2 — 🟠 In V1, long-term _lessons_ have no rot-protection at all (existence **and** contradiction both off)
...
### F3 — 🟡 Runtime redaction is structural-pattern-only (`extra_secrets=()`)
...
### F4 — 🟡 `restore` is not byte-identical when cleanup _first-creates_ a tier file
...
### F5 — 🟡 Several acceptance criteria are asserted-by-design but not actually tested
...
- **`cleanup_promotions_per_pass` is decorative** — parsed in schema + loader but **never read** by `CleanupJob` (the never-promote guarantee holds structurally instead). Either consume it or document it as a documentation-only invariant.
- **`DerivedIndex.symbol_exists` is dead in production** — fully implemented and unit-tested, but no caller (only path existence is used). A direct consequence of F1/F2.
- **`EpisodeRecord` is `frozen=True` with a mutable `stage_outcomes: dict`** (`records.py`) — the dict is still mutable in place; cosmetic, but undercuts "frozen".
- **Entity merge is wholesale "latest card wins"** (`service._ingest_entity`) — a new card with fewer paths/symbols overwrites a richer one. Tracked (row 61c).
...
1. **Close F1** — wire `DerivedIndex` into `apply_delta` (resolve `scope.paths`, downgrade unverifiable cards off `repo-observed`, quarantine on a missing target), **or** decide to leave write-time validation to cleanup — and either way correct the stale `follow_ups.md` 61(a) text.
2. **Surface F2** — record "V1 lessons have no existence/contradiction rot-protection" as an explicit risk, watch-queue steady state as the trigger.
3. **Decide F3** — pass known config/env secret literals as `extra_secrets`, or record the residual structural-only risk against C1.
4. **Patch F4** — make `restore` prune snapshot-absent tier files; extend the rollback drill to start from an empty-quarantine store.
5. **Backfill F5 tests** — AC-S3 and an AC-S4 disabled-vs-baseline run at minimum.
6. **Sweep F6 nits** — consume or annotate `cleanup_promotions_per_pass`; note `symbol_exists` as F1/F2-pending; tidy the frozen-dataclass dict.

These are all additive; none blocks the existing V1 merge gate. F1 and F2 are the two that touch the subsystem's stated invariants (NFR2, anti-rot) and should land before the subsystem is exercised on a long-lived repo.
