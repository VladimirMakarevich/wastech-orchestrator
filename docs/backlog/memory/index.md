...
Status: **V1 implemented** (all five phases landed on `feat/memory-subsystem`) Date: 2026-07-01 Owner: Vladimir Makarevich
...
- ✅ **V1 is implemented.** All five phases landed on `feat/memory-subsystem`; the [definition-of-done](definition-of-done.md) holds. Next: record a **real** eval baseline once production runs accrue and tune the provisional Q1/Q3/Q5 integers against it; the V2 (SQLite/FTS) / V3 (embeddings) / V4 (graph) phases stay gated behind a measured lift (AC-O4). Deferred V1-scope-cut tightening is tracked in [../follow_ups.md](../follow_ups.md).
- 📋 **Post-implementation audit recorded** ([implementation-audit.md](implementation-audit.md), 2026-07-01). Conformance holds and the gate passes (suite green, `ruff`/`mypy` clean); six open findings remain that the gate does not catch. The two touching stated invariants — **F1** (the planned `DerivedIndex`→`apply_delta` write-time validation was never wired when phase 04 landed; `follow_ups.md` 61a is stale) and **F2** (V1 lessons have no existence/contradiction rot-protection) — should land before the subsystem runs against a long-lived repo. F3–F6 (structural-only runtime redaction, a `restore` first-created-file edge, assert-by-design-but-untested ACs, minor nits) are lower priority.
