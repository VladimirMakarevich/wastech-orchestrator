...
- Docs: add an "Install & upgrade" section (likely [docs/operations.md](../operations.md) or the top-level [docs/index.md](../index.md)) with the git-tag recipe and the `pipx upgrade`/`--force`/uv/quoting caveats. This is doc-only and can ship independently of any code.
