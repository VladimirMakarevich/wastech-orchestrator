...
**Follow-up finding (2026-06-26) — the default env allowlist was POSIX-only.** Running `worc preflight` against a real repo on Windows reported `claude: FAIL — claude was found but 'claude --version' did not succeed`, while `claude --version` worked in the operator's shell. The allowlist _mechanism_ is cross-platform-clean (the premise above holds), but the default _contents_ (`PATH, HOME, USER, USERPROFILE, CODEX_HOME, CLAUDE_CONFIG_DIR`) were POSIX-shaped and omitted `SystemRoot`. The orchestrator launches every child with only the allowlisted vars present in the parent (`build_child_env`), so the Node-based `claude.exe` started with no `%SystemRoot%` and aborted at startup with exit `0xC0000409` (STATUS_STACK_BUFFER_OVERRUN) — before any stdout/stderr — which preflight surfaced as "did not succeed". (Codex passed: it is a Rust binary and does not need `SystemRoot` to start.) **Fixed:** the default `allowed_environment` is now OS-aware (`security/env.py` `default_allowed_environment` / `os_essential_env`) — the cross-platform base plus the host OS's launch essentials (Windows: `SystemRoot`, `SystemDrive`, `windir`, `ComSpec`, `PATHEXT`, `TEMP`, `TMP`, `APPDATA`, `LOCALAPPDATA`, `HOMEDRIVE`, `HOMEPATH`, `NUMBER_OF_PROCESSORS`, `PROCESSOR_ARCHITECTURE`; POSIX: `TMPDIR`, `LD_LIBRARY_PATH`, `DYLD_LIBRARY_PATH`). The installer writes the host OS's set; a hand-written config that omits the key falls back to it; the allowlist stays the single env gate (security rule #4). **WSL** needs no special handling — it reports as `Linux` via `platform.system()`, so it gets the POSIX set, which is correct (inside WSL the agent CLIs are Linux binaries). See the deferred upgrade-config repair in [follow_ups.md](follow_ups.md).

**Deferred** (tracked in [follow_ups.md](follow_ups.md)): the Windows CI runner matrix (change #3); a `taskkill /F /PID` hard-kill backstop for a wedged Windows daemon; the `CTRL_BREAK_EVENT` Ctrl+C handler; the macOS `_read_proc_start_time` recycling guard; and a `upgrade-config` repair that re-adds OS-launch essentials to an existing config whose explicit `allowed_environment` predates the OS-aware default (the merge is add-only, so existing configs with a POSIX-only list are not auto-repaired).
