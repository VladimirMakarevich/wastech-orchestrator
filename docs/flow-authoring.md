...
| `private_control_workspace_report` | `.worc/security-reports/<task_id>/` only | `report.md` | **no — never committed** | `none` (the publish node itself takes `policy: private_control_workspace_report`) | The result is an advisory report that must stay out of git (the `security_audit` shape). |
...
A node's prompt is the content of its `role_file` — plus, on a turn that continues a session the node has already spoken on, the optional [`resume_role_file`](#continuation-prompts-resume_role_file) beside it. Both render the same variable set on the same node. Role files render only an allowlisted set of path/metadata variables — `{task_path}`, `{repo_path}` (alias `{repo}`), `{plan_path}`, `{diff_path}`, `{checks_path}`, `{review_path}`, `{memory_path}`, `{subtask_order}`/`{subtask_count}`/`{subtask_spec_path}`, `{predecessor_context}` (the intra-task subtask handoff brief), the two metadata scalars `{task_id}` and `{stage}`, plus every agent/tool node's `{<node_id>_path}` — never task bodies, diffs, env, or secrets. A variable that is empty for a given node renders as the empty string; wrap optional references in a conditional block `{?name}…{/name}` so they drop cleanly when empty. For the full variable contract and which runner populates each, see [configuration.md → Prompt templates](configuration.md#prompt-templates-no-longer-a-config-block).
...
Every violation in a layer is collected and reported together, so one pass shows you all of them. Three layers run:
...
- **Security ceiling** — no node's `permission_profile` exceeds the flow `permission_ceiling`; evaluators are forced read-only and may not use `editing_lineage`; `git_evidence` is not declared on a workspace-write node; `role_file` and the `supervisor.*` prompt paths contain no traversal; unknown fields fail closed. A node's `extra_args` is checked against a **config-independent ceiling** — one that does not consult `config.yaml` at all — so the absolutely-forbidden flags (`--dangerously*`, `--yolo`, `--ignore-rules`) **and both provider full-access selectors** (Codex `--sandbox danger-full-access`, Claude `--permission-mode bypassPermissions`, in either argv spelling) are refused here whatever the config says. A `--permission-mode` value _weaker_ than the profile the node asked for — `auto` over a stricter profile, say — is refused too, checked over the provider config and the node's `extra_args` together.
- **Config consistency** — a pinned `provider` is in `agents.allowed`, its `reasoning` is supported by that provider, Codex is not routed to a `workspace-write` node resolving `network_access: true`, at least one allowed provider can operate at the flow's `permission_ceiling`, a `tool:` node names a tool actually delivered under `.worc/tools/`, the flow's `supervisor.observe.mode` does not **widen** the config's cadence, and (under `security.strict_isolation`) no `extra_args` selects a full-access sandbox mode.
