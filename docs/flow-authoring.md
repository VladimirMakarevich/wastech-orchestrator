...
Two infrastructure shapes are lifted out of that contract before the outcome is read, and both park the task at `manual_action_required` without charging a fix iteration. **A crashed tool** — a non-zero exit with **empty stdout** and non-empty stderr — cannot be an actionable quality verdict, so a redacted stderr head goes into the stop reason instead of routing a fixer at nothing. And **a repeated non-actionable failure**: two identical `fail` results with **no `findings`** from the same tool node park on the second, before another fix edge can be charged, so a gate and a fixer cannot burn the whole budget on a verdict the fixer cannot act on. The practical consequence for a tool you write: when you return `fail`, return `findings` with it.
